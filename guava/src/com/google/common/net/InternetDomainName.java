...
public final class InternetDomainName {
     ...
     * encoding of each part occupies at least two bytes (dot plus label externally, length byte
     * plus label internally). Thus, if all labels have the minimum size of one byte, 127 of them
     * will fit.
     */
    ...
    /**
     * The index in the {@link #parts()} list at which the public suffix begins. For example, for
     * the domain name {@code www.google.co.uk}, the value would be 2 (the index of the {@code co}
     * part). The value is negative (specifically, {@link #NO_PUBLIC_SUFFIX_FOUND}) if no public
     * suffix was found.
     */
    ...
    /**
     * Returns the index of the leftmost part of the public suffix, or -1 if not found. Note that
     * the value defined as the "public suffix" may not be a public suffix according to
    ...
    private static boolean validatePart(String part, boolean isFinalPart) {
        ...
        /*
         * GWT claims to support java.lang.Character's char-classification methods, but it actually
         * only works for ASCII. So for now, assume any non-ASCII characters are valid. The only
         * place this seems to be documented is here:
         * http://osdir.com/ml/GoogleWebToolkitContributors/2010-03/msg00178.html
         *
         * <p>ASCII characters in the part are expected to be valid per RFC 1035, with underscore
         * also being allowed due to widespread practice.
         */
        ...
        /*
         * Note that we allow (in contravention of a strict interpretation of the relevant RFCs)
         * domain parts other than the last may begin with a digit (for example, "3com.com"). It's
         * important to disallow an initial digit in the last part; it's the only thing that stops
         * an IPv4 numeric address like 127.0.0.1 from looking like a valid domain name.
         */
    ...
    /**
     * Indicates whether this domain name represents a <i>public suffix</i>, as defined by the
     * Mozilla Foundation's <a href="http://publicsuffix.org/">Public Suffix List</a> (PSL). A
     * public suffix is one under which Internet users can directly register names, such as
     * {@code com}, {@code co.uk} or {@code pvt.k12.wy.us}. Examples of domain names that are
     * <i>not</i> public suffixes include {@code google}, {@code google.com} and {@code foo.co.uk}.
     *
     ...
     *
     * <p>
     * <b>Warning:</b> a {@code false} result from this method does not imply that the domain does
    ...
    /**
     * Indicates whether this domain name is composed of exactly one subdomain component followed by
     * a {@linkplain #isPublicSuffix() public suffix}. For example, returns {@code true} for
     * {@code google.com} and {@code foo.co.uk}, but not for {@code www.google.com} or
     * {@code co.uk}.
     *
     * <p>
     * <b>Warning:</b> A {@code true} result from this method does not imply that the domain is at
     * the highest level which is addressable as a host, as many public suffixes are also
     * addressable hosts. For example, the domain {@code bar.uk.com} has a public suffix of
     * {@code uk.com}, so it would return {@code true} from this method. But {@code uk.com} is
     * itself an addressable host.
     *
     * <p>
     * This method can be used to determine whether a domain is probably the highest level for which
     * cookies may be set, though even that depends on individual browsers' implementations of
     ...
     *
     * <p>
     * This method should not be used to determine the topmost parent domain which is addressable as
     * a host, as many public suffixes are also addressable hosts. For example, the domain
    ...
    /**
     * Returns an {@code InternetDomainName} that is the immediate ancestor of this one; that is,
     * the current domain with the leftmost part removed. For example, the parent of
    ...
    /**
     * Equality testing is based on the text supplied by the caller, after normalization as
     * described in the class documentation. For example, a non-ASCII Unicode domain name and the
     * Punycode version of the same domain name would not be considered equal.
     *
     */
    @Override
