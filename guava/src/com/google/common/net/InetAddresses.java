 ...
 *
 * <p>
 * <b>Important note:</b> Unlike {@code InetAddress.getByName()}, the methods of this class never
 ...
 *
 * <p>
 * Technically one <i>can</i> create a 128bit IPv6 address with the wire format of a "mapped"
 ...
 *
 * <p>
 * For added safety, it is common for IPv6 network operators to filter all packets where either the
 * source or destination address appears to be a "compat" or "mapped" address. Filtering suggestions
 * usually recommend discarding any packets with source or destination addresses in the invalid
 * range {@code ::/3}, which includes both of these bizarre address formats. For more information on
 * "bogons", including lists of IPv6 bogon space, see:
 *
 * <ul>
...
public final class InetAddresses {
     ...
     *
     * {@link InetAddress#getByAddress} is documented as throwing a checked exception "if IP address
     * is of illegal length." We replace it with an unchecked exception, for use by callers who
     * already know that addr is an array of length 4 or 16.
     *
     ...
     * addresses, the output follows <a href="http://tools.ietf.org/html/rfc5952">RFC 5952</a>
     * section 4. The main difference is that this method uses "::" for zero compression, while
     * Java's version uses the uncompressed form.
     *
     * <p>
     ...
     * <a target="_parent" href="http://tools.ietf.org/html/rfc3986#section-3.2.2">RFC 3986</a>, a
     * URI containing an IPv6 string literal is of the form
     ...
     * literal is desired. This is because {@link InetAddress#toString()} prints the hostname and
     * the IP address string joined by a "/".
     *
    ...
    /**
     * Returns an InetAddress representing the literal IPv4 or IPv6 host portion of a URL, encoded
     * in the format specified by RFC 3986 section 3.2.2.
     *
     * <p>
    ...
    public static final class TeredoInfo {
         ...
         *
         * <p>
         * Both server and client can be {@code null}, in which case the value {@code "0.0.0.0"}
         * will be assumed.
         *
        ...
        public TeredoInfo(@Nullable Inet4Address server, @Nullable Inet4Address client, int port, int flags) {
            checkArgument((port >= 0) && (port <= 0xffff), "port '%s' is out of range (0 <= port <= 0xffff)", port);
            checkArgument((flags >= 0) && (flags <= 0xffff), "flags '%s' is out of range (0 <= flags <= 0xffff)",
                    flags);
     ...
     * by concatenating the 24-bit IANA OUI (00-00-5E), the 8-bit hexadecimal value 0xFE, and a
    ...
    /**
     * Examines the Inet6Address to determine if it is an IPv6 address of one of the specified
     * address types that contain an embedded IPv4 address.
     *
     * <p>
    ...
    /**
     * Examines the Inet6Address to extract the embedded IPv4 client address if the InetAddress is
     * an IPv6 address of one of the specified address types that contain an embedded IPv4 address.
     *
     * <p>
     * NOTE: ISATAP addresses are explicitly excluded from this method due to their trivial
     * spoofability. With other transition addresses spoofing involves (at least) infection of one's
     ...
     * accounting, et cetera, it may be necessary to <b>coerce</b> IPv6 addresses into IPv4
     * addresses. This function does so by hashing the upper 64 bits into {@code 224.0.0.0/3} (64
     * bits into 29 bits).
     *
     * <p>
    ...
    /**
     * Returns an integer representing an IPv4 address regardless of whether the supplied argument
     * is an IPv4 address or not.
     *
     * <p>
     * IPv6 addresses are <b>coerced</b> to IPv4 addresses before being converted to integers.
     *
     * <p>
     * As long as there are applications that assume that all IP addresses are IPv4 addresses and
     * can therefore be converted safely to integers (for whatever purpose) this function can be
     * used to handle IPv6 addresses as well until the application is suitably fixed.
     *
     * <p>
