...
public abstract class ByteSource {
    ...
    /**
     * Returns a {@link CharSource} view of this byte source that decodes bytes read from this
     * source as characters using the given {@link Charset}.
     *
     * <p>
    ...
    /**
     * Opens a new {@link InputStream} for reading from this source. This method should return a
     ...
     * source, the returned source will be empty. If {@code offset + length} is greater than the
     * size of this source, the returned source will contain the slice starting at {@code offset}
     * and ending at the end of this source.
     *
     ...
     * {@link #sizeIfKnown} returns zero, falling back to opening a stream and checking for EOF if
     * the size is not known.
     *
     * <p>
     * Note that, in cases where {@code sizeIfKnown} returns zero, it is <i>possible</i> that bytes
     ...
     *
     * <p>
     * The default implementation returns {@link Optional#absent}. Some sources, such as a file, may
     * return a non-absent value. Note that in such cases, it is <i>possible</i> that this method
     * will return a different number of bytes than would be returned by reading all of the bytes
     * (for example, some special files may return a size of 0 despite actually having content when
     * read).
     *
     * <p>
     ...
     * {@link InputStream#skip(long) skip}, if possible) to the end of the stream and return the
    ...
    /**
     * Concatenates multiple {@link ByteSource} instances into a single source. Streams returned
     * from the source will contain the concatenated data from the streams of the underlying
     * sources.
     *
     * <p>
