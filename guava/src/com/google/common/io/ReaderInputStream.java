 ...
 *
 * <p>
 * This is an alternative to copying the data to an {@code OutputStream} via a {@code Writer}, which
 * is necessarily blocking. By implementing an {@code InputStream} it allows consumers to "pull" as
 * much data as they can handle, which is more convenient when dealing with flow controlled, async
 * APIs.
 *
...
final class ReaderInputStream extends InputStream {
    ...
    /**
     * charBuffer holds characters that have been read from the Reader but not encoded yet. The
     * buffer is perpetually "flipped" (unencoded characters between position and limit).
     */
    ...
    /**
     * Creates a new input stream that will encode the characters from {@code reader} into bytes
     * using the given character set. Malformed input and unmappable characters will be replaced.
     *
    ...
    public int read(byte[] b, int off, int len) throws IOException {
        ...
        DRAINING: while (true) {
            // We stay in draining mode until there are no bytes left in the output buffer. Then we
            // go
            ...
            while (true) {
                // We call encode until there is no more input. The last call to encode will have
                // endOfInput
                ...
                if (result.isOverflow()) {
                    // Not enough room in output buffer--drain it, creating a bigger buffer if
                    // necessary.
                    startDraining(true);
                    continue DRAINING;
