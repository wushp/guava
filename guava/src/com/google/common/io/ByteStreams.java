...
public final class ByteStreams {
     ...
     * channel have their own file descriptors. Generally this only happens when both channels are
     * files or sockets. This performs zero copies - the bytes never enter userspace.
     ...
     * implementation will effectively put the whole file in memory. On many systems with paging and
     * virtual memory, this is not a problem - because it is mapped read-only, the kernel can always
     * page it to disk "for free". However, on systems where killing processes happens all the time
     * in normal conditions (i.e., android) the OS must make a tradeoff between paging memory and
     * killing other processes - so allocating a gigantic buffer and then sequentially accessing it
     * could result in other processes dying. This is solvable via madvise(2), but that obviously
    ...
    private static final class FastByteArrayOutputStream extends ByteArrayOutputStream {
        /**
         * Writes the contents of the internal buffer to the given array starting at the given
         * offset. Assumes the array has space to hold count bytes.
         */
    ...
    /**
     * Returns a new {@link ByteArrayDataInput} instance to read from the {@code bytes} array from
     * the beginning.
     */
     ...
     * {@code ByteArrayInputStream}. The given input stream is not reset before being read from by
     * the returned {@code ByteArrayDataInput}.
     *
     * @since 17.0
     */
     ...
     * {@code ByteArrayOutputStream}. The given output stream is not reset before being written to
     * by the returned {@code ByteArrayDataOutput} and new data will be appended to any existing
     * content.
     *
     * <p>
     ...
     * {@code off}, with the same behavior as {@link DataInput#readFully(byte[], int, int)}. Does
     ...
     * {@linkplain java.io.FileInputStream#skip(long) specifies} it can do in its Javadoc despite
     * the fact that it is violating the contract of {@code InputStream.skip()}.
     */
     ...
     * {@code b}, then an {@code IndexOutOfBoundsException} is thrown. If {@code len} is zero, then
     * no bytes are read. Otherwise, the first byte read is stored into element {@code b[off]}, the
