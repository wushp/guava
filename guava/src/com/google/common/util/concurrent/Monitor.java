 ...
 *
 * <p>
 * This class is intended as a replacement for {@link ReentrantLock}. Code using {@code Monitor} is
 * less error-prone and more readable than code using {@code ReentrantLock}, without significant
 ...
 * <a href="http://en.wikipedia.org/wiki/Monitor_(synchronization)#Implicit_signaling">implicit</a>.
 * By eliminating explicit signaling, this class can guarantee that only one thread is awakened when
 * a condition becomes true (no "signaling storms" due to use of
 * {@link java.util.concurrent.locks.Condition#signalAll Condition.signalAll}) and that no signals
 * are lost (no "hangs" due to incorrect use of {@link java.util.concurrent.locks.Condition#signal
 * Condition.signal}).
 *
 * <p>
 * A thread is said to <i>occupy</i> a monitor if it has <i>entered</i> the monitor but not yet
 ...
 *
 * <p>
 * A call to any of the <i>enter</i> methods with <b>void</b> return type should always be followed
 * immediately by a <i>try/finally</i> block to ensure that the current thread leaves the monitor
 * cleanly:
 * 
 * <pre>
 *    {@code
 ...
 * current thread leaves the monitor cleanly:
 * 
 * <pre>
 *    {@code
 *
 ...
 *
 * <p>
 * This version is the fewest lines of code, largely because the synchronization mechanism used is
 * built into the language and runtime. But the programmer has to remember to avoid a couple of
 ...
 *
 * <p>
 * This version is much more verbose than the {@code synchronized} version, and still suffers from
 * the need for the programmer to remember to use {@code while} instead of {@code if}. However, one
 * advantage is that we can introduce two separate {@code Condition} objects, which allows us to use
 * {@code signal()} instead of {@code signalAll()}, which may be a performance benefit.
 * 
 * <pre>
 * {
 *     &#64;code
...
public final class Monitor {
     ...
     * guards, and leaves, calling signalNextWaiter. With traditional locks and conditions, all the
     * conditions need to be signalled because it is not known which if any of them have waiters
     * (and hasWaiters can't be used reliably because of a check-then-act race). With our Monitor
     * guards, we only signal the first active guard that is satisfied. But the corresponding thread
     * may have already been interrupted and is waiting to reacquire the lock while still registered
     * in activeGuards, in which case the signal is a no-op, and the bigger-picture signal is lost
     * unless interrupted threads take special action by participating in the signal-passing game.
     */

    /*
     * Timeout handling is intricate, especially given our ambitious goals: - Avoid underflow and
     * overflow of timeout values when specified timeouts are close to Long.MIN_VALUE or
     * Long.MAX_VALUE. - Favor responding to interrupts over timeouts. - System.nanoTime() is
     * expensive enough that we want to call it the minimum required number of times, typically once
     * before invoking a blocking method. This often requires keeping track of the first time in a
     * method that nanoTime() has been invoked, for which the special value 0L is reserved to mean
     * "uninitialized". If timeout is non-positive, then nanoTime need never be called. - Keep
     * behavior of fair and non-fair instances consistent.
     */
    ...
    public abstract static class Guard {
         ...
         * associated monitor already occupied. Implementations of this method must depend only on
         * state protected by the associated monitor, and must not modify that state.
         */
     ...
     *
     * @param fair whether this monitor should use a fair ordering policy rather than a non-fair
     *        (but fast) one
     */
     ...
     * both the time to acquire the lock and the time to wait for the guard to be satisfied, and may
     * be interrupted.
     *
    ...
    /**
     * Enters this monitor if the guard is satisfied. Blocks indefinitely acquiring the lock, but
     * does not wait for the guard to be satisfied, and may be interrupted.
     *
     * @return whether the monitor was entered, which guarantees that the guard is now satisfied
     * @throws InterruptedException if interrupted while waiting
     */
    ...
    /**
     * Enters this monitor if it is possible to do so immediately and the guard is satisfied. Does
    ...
    /**
     * Returns the number of times the current thread has entered this monitor in excess of the
     * number of times it has left. Returns 0 if the current thread is not occupying this monitor.
     */
     ...
     * cancellations may occur at any time, a {@code true} return does not guarantee that any other
     * thread will ever enter this monitor. This method is designed primarily for use in monitoring
     * of the system state.
     */
     ...
     * cancellations may occur at any time, a {@code true} return does not guarantee that this
     * thread will ever enter this monitor. This method is designed primarily for use in monitoring
     * of the system state.
     */
     ...
     * because timeouts and interrupts may occur at any time, a {@code true} return does not
     * guarantee that the guard becoming satisfied in the future will awaken any threads. This
     ...
     * Note that because timeouts and interrupts may occur at any time, the estimate serves only as
     * an upper bound on the actual number of waiters. This method is designed for use in monitoring
    ...
    /**
     * Returns System.nanoTime() unless the timeout has already elapsed. Returns 0L if and only if
    ...
    /**
     * Returns the remaining nanos until the given timeout, or 0L if the timeout has already
     * elapsed. Caller must have previously sanitized timeoutNanos using toSafeNanos.
     */
     ...
     * the current thread is about to relinquish the lock and may have changed the state protected
     * by the monitor, thereby causing some guard to be satisfied.
     *
     ...
     * recording this fact so that other threads know to check our guard and signal us. It's
     * caller's responsibility to ensure that the guard is *not* currently satisfied.
     */
