 ...
 *
 * <p>
 * Potential deadlocks detected when calling the {@code lock()}, {@code lockInterruptibly()}, or
 ...
 *
 * <p>
 * The locks created by a factory instance will detect lock acquisition cycles with locks created by
 * other {@code CycleDetectingLockFactory} instances (except those with {@code Policy.DISABLED}). A
 * lock's behavior when a cycle is detected, however, is defined by the {@code Policy} of the
 ...
 *
 * <p>
 * Applications are encouraged to use a {@code CycleDetectingLockFactory} to create any locks for
 ...
 *
 * <p>
 * Deadlocks can arise when locks are acquired in an order that forms a cycle. In a simple example
 * involving two locks and two threads, deadlock occurs when one thread acquires Lock A, and then
 * Lock B, while another thread acquires Lock B, and then Lock A:
 *
 * <pre>
 ...
 *
 * <p>
 * Neither thread will progress because each is waiting for the other. In more complex applications,
 * cycles can arise from interactions among more than 2 locks:
 *
 * <pre>
 * Thread1: acquire(LockA) --X acquire(LockB)
 ...
 *
 * <p>
 * The implementation detects cycles by constructing a directed graph in which each lock represents
 * a node and each edge represents an acquisition ordering between two locks.
 * <ul>
 ...
 *
 * <p>
 * The {@link CycleDetectingLockFactory.WithExplicitOrdering} class can be used to enforce an
 ...
 *
 * <p>
 * In order to allow proper garbage collection of unused locks, the edges of the lock graph are weak
 * references.
 *
 * <p>
 * <strong>Performance</strong>
 *
 * <p>
 * The extra bookkeeping done by cycle detecting locks comes at some cost to performance. Benchmarks
 * (as of December 2011) show that:
 *
 * <ul>
 ...
 *
 * <p>
 * As such, the CycleDetectingLockFactory may not be suitable for performance-critical applications
 * which involve tightly-looped or deeply-nested locking algorithms.
 *
 * @author Darick Tong
 * @since 13.0
 */
...
public class CycleDetectingLockFactory {
     ...
     * one of the predefined {@link Policies} or specify a custom implementation. Implementations
     * must be thread-safe.
     *
     * @since 13.0
     */
    ...
    public interface Policy {
         ...
         *
         * <p>
         * Note that the method will be called even upon an invocation of {@code tryLock()}.
         * Although {@code tryLock()} technically recovers from deadlock by eventually timing out,
         * this behavior is chosen based on the assumption that it is the application's wish to
         * prohibit any cyclical lock acquisitions.
         */
    ...
    public enum Policies implements Policy {
         ...
         * {@code PotentialDeadlockException} indicating the potential deadlock, which includes
         ...
         * {@link Level#SEVERE} message indicating the potential deadlock, which includes stack
         ...
         *
         * <p>
         * Note that locks created by a factory with this policy will <em>not</em> participate the
    ...
    /**
     * Creates a {@link ReentrantLock} with the given fairness policy. The {@code lockName} is used
     * in the warning or exception output to help identify the locks involved in the detected
     * deadlock.
     */
     ...
     * is used in the warning or exception output to help identify the locks involved in the
     * detected deadlock.
     */
    ...
    /**
     * <p>
     * A {@code CycleDetectingLockFactory.WithExplicitOrdering} provides the additional enforcement
     * of an application-specified ordering of lock acquisitions. The application defines the
     * allowed ordering with an {@code Enum} whose values each correspond to a lock type. The order
     * in which the values are declared dictates the allowed order of lock acquisition. In other
     * words, locks corresponding to smaller values of {@link Enum#ordinal()} should only be
     * acquired before locks with larger ordinals. Example:
     *
     * <pre>
     *    {@code
     * enum MyLockOrder {
     ...
     *
     * <p>
     * As with all locks created by instances of {@code CycleDetectingLockFactory} explicitly
     * ordered locks participate in general cycle detection with all other cycle detecting locks,
     * and a lock's behavior when detecting a cyclic lock acquisition is defined by the
     * {@code Policy} of the factory that created it.
     *
     * <p>
     * Note, however, that although multiple locks can be created for a given Enum value, whether it
     * be through separate factory instances or through multiple calls to the same factory,
    ...
    public static final class WithExplicitOrdering<E extends Enum<E>> extends CycleDetectingLockFactory {
        ...
        /**
         * Creates a {@link ReentrantLock} with the given fairness policy and rank. The values
         * returned by {@link Enum#getDeclaringClass()} and {@link Enum#name()} are used to describe
         * the lock in warning or exception output.
         *
        ...
        /**
         * Creates a {@link ReentrantReadWriteLock} with the given fairness policy and rank. The
         * values returned by {@link Enum#getDeclaringClass()} and {@link Enum#name()} are used to
    ...
    private static class ExampleStackTrace extends IllegalStateException {
        ...
        static final Set<String> EXCLUDED_CLASS_NAMES = ImmutableSet.of(CycleDetectingLockFactory.class.getName(),
                ExampleStackTrace.class.getName(), LockGraphNode.class.getName());
    ...
    /**
     * Represents a detected cycle in lock acquisition ordering. The exception includes a causal
     * chain of {@code ExampleStackTrace} instances to illustrate the cycle, e.g.
     *
     * <pre>
     ...
     *
     * <p>
     * Instances are logged for the {@code Policies.WARN}, and thrown for {@code Policies.THROW}.
     *
     * @since 13.0
     */
    @Beta
    ...
    /**
     * Internal Lock implementations implement the {@code CycleDetectingLock} interface, allowing
     * the detection logic to treat all locks in the same manner.
     */
    ...
    /**
     * A {@code LockGraphNode} associated with each lock instance keeps track of the directed edges
    ...
    private static class LockGraphNode {
         ...
         * associated with an example stack trace. Locks are weakly keyed to allow proper garbage
        ...
        /**
         * The map tracking lock nodes that can cause a lock acquisition cycle if acquired before
         * this node.
         */
        ...
        /**
         * Checks the acquisition-ordering between {@code this}, which is about to be acquired, and
         * the specified {@code acquiredLock}.
         *
         * <p>
         * When this method returns, the {@code acquiredLock} should be in either the
         * {@code preAcquireLocks} map, for the case in which it is safe to acquire {@code this}
         * after the {@code acquiredLock}, or in the {@code disallowedPriorLocks} map, in which case
         * it is not safe.
         */
         ...
         * {@code allowedPriorLocks} to find a path between {@code this} and the specified
         * {@code lock}.
         *
         * @return If a path was found, a chained {@link ExampleStackTrace} illustrating the path to
         *         the {@code lock}, or {@code null} if no path was found.
         */
        @Nullable
    ...
    /**
     * CycleDetectingLock implementations must call this method before attempting to acquire the
     * lock.
     */
    ...
    /**
     * CycleDetectingLock implementations must call this method in a {@code finally} clause after
     * any attempt to change the lock state, including both lock and unlock attempts. Failure to do
     * so can result in corrupting the acquireLocks set.
     */
