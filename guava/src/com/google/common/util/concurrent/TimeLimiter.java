...
public interface TimeLimiter {
     ...
     *
     * <p>
     * If the target method call finishes before the limit is reached, the return value or exception
     * is propagated to the caller exactly as-is. If, on the other hand, the time limit is reached,
     * the proxy will attempt to abort the call to the target, and will throw an
     ...
     * @param timeoutDuration with timeoutUnit, the maximum length of time that callers are willing
     *        to wait on each method call to the proxy
     * @param timeoutUnit with timeoutDuration, the maximum length of time that callers are willing
     *        to wait on each method call to the proxy
     ...
     * caller exactly as-is. If, on the other hand, the time limit is reached, we attempt to abort
     * the call to the target, and throw an {@link UncheckedTimeoutException} to the caller.
     *
