...
public final class JdkFutureAdapters {
     ...
     *
     * <p>
     * <b>Warning:</b> If the input future does not already implement {@code ListenableFuture}, the
     ...
     * {@link ListenableFutureTask}, {@link AbstractFuture}, and other utilities over creating plain
     * {@code
     ...
     * execution consists of blocking until the input future is {@linkplain Future#isDone() done},
     * so each call to this method may claim and hold a thread for an arbitrary length of time. Use
     * of bounded executors or other executors that may fail to execute a task promptly may result
     * in deadlocks.
     *
     * <p>
     * Prefer to create {@code ListenableFuture} instances with {@link SettableFuture},
     ...
     * the source future indefinitely, so if the source future never completes, the adapter will
    ...
    private static class ListenableFutureAdapter<V> extends ForwardingFuture<V> implements ListenableFuture<V> {
        ...
        private static final ThreadFactory threadFactory =
                new ThreadFactoryBuilder().setDaemon(true).setNameFormat("ListenableFutureAdapter-thread-%d").build();
        ...
        public void addListener(Runnable listener, Executor exec) {
            ...
            // When a listener is first added, we run a task that will wait for the delegate to
            // finish,
            // and when it is done will run the listeners.
            if (hasListeners.compareAndSet(false, true)) {
                if (delegate.isDone()) {
                    // If the delegate is already done, run the execution list immediately on the
                    // current
                    // thread.
                ...
                adapterExecutor.execute(new Runnable() {
                    ...
                    public void run() {
                        try {
                             ...
                             * user-supplied executor might be, but... what can we do? This is
                             * another reason to return a proper ListenableFuture instead of using
                             * listenInPoolThread.
                             */
