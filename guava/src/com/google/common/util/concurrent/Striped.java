 ...
 *
 * <p>
 * There are three flavors of this class: {@code Striped<Lock>}, {@code Striped<Semaphore>}, and
 ...
 *
 * <p>
 * Prior to this class, one might be tempted to use {@code Map<K, Lock>}, where {@code K} represents
 * the task. This maximizes concurrency by having each unique key mapped to a unique lock, but also
 * maximizes memory footprint. On the other extreme, one could use a single lock for all tasks,
 * which minimizes memory footprint but also minimizes concurrency. Instead of choosing either of
 * these extremes, {@code Striped} allows the user to trade between required concurrency and memory
 * footprint. For example, if a set of tasks are CPU-bound, one could easily create a very compact
 * {@code Striped<Lock>} of {@code availableProcessors() * 4} stripes, instead of possibly thousands
 * of locks which could be created in a {@code Map<K, Lock>} structure.
 *
...
public abstract class Striped<L> {
    /**
     * If there are at least this many stripes, we assume the memory usage of a ConcurrentMap will
     * be smaller than a large array. (This assumes that in the lazy case, most stripes are unused.
     * As always, if many stripes are in use, a non-lazy striped makes more sense.)
     */
     ...
     * shared stripes (much like the birthday paradox, where much fewer than anticipated birthdays
     ...
     * {@code bulkGet(keys)} operation. See
     * <a href="http://www.mathpages.com/home/kmath199.htm">Balls in Bins model</a> for mathematical
     * formulas that can be used to estimate the probability of collisions.
     *
    ...
    public Iterable<L> bulkGet(Iterable<?> keys) {
         ...
         * stripedLock.bulkGet(keys); for (Lock lock : locks) { lock.lock(); } operation(); for
         * (Lock lock : locks) { lock.unlock(); }
         *
    ...
    /**
     * Creates a {@code Striped<Lock>} with eagerly initialized, strongly referenced locks. Every
     * lock is reentrant.
     *
    ...
    /**
     * Creates a {@code Striped<Lock>} with lazily initialized, weakly referenced locks. Every lock
     * is reentrant.
     *
     * @param stripes the minimum number of stripes (locks) required
     * @return a new {@code Striped<Lock>}
     ...
     * AtomicReferenceArray of size 2^k. To map a user key into a stripe, we take a k-bit slice of
     * the user key's (smeared) hashCode(). The stripes are lazily initialized and are weakly
     * referenced.
     */
    @VisibleForTesting
    static class SmallLazyStriped<L> extends PowerOfTwoStriped<L> {
        ...
        // N.B. Draining the queue is only necessary to ensure that we don't accumulate empty
        // references
     ...
     * where the key domain is [0..2^k). To map a user key into a stripe, we take a k-bit slice of
     * the user key's (smeared) hashCode(). The stripes are lazily initialized and are weakly
     * referenced.
     */
    @VisibleForTesting
     ...
     *
     * As of 2010/06/11, this method is identical to the (package private) hash method in OpenJDK
     * 7's java.util.HashMap class.
     */
    ...
    private static class PaddedLock extends ReentrantLock {
        /*
         * Padding from 40 into 64 bytes, same size as cache line. Might be beneficial to add a
         * fourth long here, to minimize chance of interference between consecutive locks, but I
         * couldn't observe any benefit from that.
         */
        long unused1;
        long unused2;
        long unused3;
