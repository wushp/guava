 ...
 *
 * <p>
 * The main purpose of {@link #addListener addListener} is to support this chaining. You will rarely
 * use it directly, in part because it does not provide direct access to the {@code Future} result.
 * (If you want such access, you may prefer {@link Futures#addCallback Futures.addCallback}.) Still,
 ...
 *
 * <p>
 * We encourage you to return {@code ListenableFuture} from your methods so that your users can take
 * advantage of the {@linkplain Futures utilities built atop the class}. The way that you will
 ...
 * Futures.immediate*} family. <b>Avoid</b> creating a mock or stub {@code Future}. Mock and stub
 * implementations are fragile because they assume that only certain methods will be called and
 * because they often implement subtleties of the API improperly.
 *
 * <p>
 * <b>Custom implementation</b>: Avoid implementing {@code ListenableFuture} from scratch. If you
 ...
 *
 * <p>
 * Occasionally, an API will return a plain {@code Future} and it will be impossible to change the
 * return type. For this case, we provide a more expensive workaround in {@code
...
public interface ListenableFuture<V> extends Future<V> {
     ...
     * thread may be an internal system thread such as an RPC network thread. Blocking that thread
     * may stall progress of the whole system. It may even cause a deadlock.
     * <li>The listener may delay other listeners, even listeners that are not themselves {@code
     * directExecutor} listeners.
     * </ul>
     *
     * <p>
     * This is the most general listener interface. For common operations performed using listeners,
     * see {@link Futures}. For a simplified but general listener interface, see
     * {@link Futures#addCallback addCallback()}.
     *
     * <p>
