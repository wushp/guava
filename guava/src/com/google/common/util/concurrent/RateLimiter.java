 ...
 *
 * <p>
 * Rate limiters are often used to restrict the rate at which some physical or logical resource is
 * accessed. This is in contrast to {@link java.util.concurrent.Semaphore} which restricts the
 ...
 *
 * <p>
 * It is possible to configure a {@code RateLimiter} to have a warmup period during which time the
 * permits issued each second steadily increases until it hits the stable rate.
 *
 * <p>
 * As an example, imagine that we have a list of tasks to execute, but we don't want to submit more
 * than 2 per second:
 * 
 * <pre>
 *    {@code
 ...
 *
 * <p>
 * It is important to note that the number of permits requested <i>never</i> affects the throttling
 * of the request itself (an invocation to {@code acquire(1)} and an invocation to
...
public abstract class RateLimiter {
    ...
    public static RateLimiter create(double permitsPerSecond) {
         ...
         * This is to avoid unnecessary stalls in situations like this: A RateLimiter of 1qps, and 4
         * threads, all calling acquire() at these moments:
         *
         * T0 at 0 seconds T1 at 1.05 seconds T2 at 2 seconds T3 at 3 seconds
         *
         * Due to the slight delay of T1, T2 would have to sleep till 2.05 seconds, and T3 would
         * also have to sleep till 3.05 seconds.
         */
     ...
     * Similarly, if the {@code RateLimiter} is left <i>unused</i> for a duration of
     * {@code warmupPeriod}, it will gradually return to its "cold" state, i.e. it will go through
     * the same warming up process as when it was first created.
     *
     * <p>
    ...
    abstract static class SleepingStopwatch {
        ...
        /*
         * We always hold the mutex when calling this. TODO(cpovirk): Is that important? Perhaps we
         * need to guarantee that each call to reserveEarliestAvailable, etc. sees a value >= the
         * previous? Also, is it OK that we don't hold the mutex when sleeping?
         */
