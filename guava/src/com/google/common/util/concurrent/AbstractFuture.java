 ...
 * existing one, typically using methods like
 * {@link Futures#transform(ListenableFuture, com.google.common.base.Function) Futures.transform}
 * and
 * {@link Futures#catching(ListenableFuture, Class, com.google.common.base.Function, java.util.concurrent.Executor)
 * Futures.catching}.
 *
 * <p>
 * This class implements all methods in {@code ListenableFuture}. Subclasses should provide a way to
 * set the result of the computation through the protected methods {@link #set(Object)},
 * {@link #setFuture(ListenableFuture)} and {@link #setException(Throwable)}. Subclasses may also
 * override {@link #interruptTask()}, which will be invoked automatically if a call to
 * {@link #cancel(boolean) cancel(true)} succeeds in canceling the future. Subclasses should rarely
...
public abstract class AbstractFuture<V> implements ListenableFuture<V> {
    ...
    private static final boolean GENERATE_CANCELLATION_CAUSES =
            Boolean.parseBoolean(System.getProperty("guava.concurrent.generate_cancellation_cause", "false"));
    ...
    private static final class Waiter {
        ...
        void unpark() {
            ...
            // call
            // unpark even though the thread has already removed itself from the list. But even if
            // we did
        ...
        restart: while (true) {
            ...
            while (curr != null) {
                ...
                if (curr.thread != null) { // we aren't unlinking this node, update pred.
                    ...
                    pred.next = succ;
                    if (pred.thread == null) { // We raced with another node that unlinked pred.
                                               // Restart.
                        continue restart;
                    }
    ...
    public V get(long timeout, TimeUnit unit) throws InterruptedException, TimeoutException, ExecutionException {
                ...
                do {
                    ...
                    if (ATOMIC_HELPER.casWaiters(this, oldHead, node)) {
                        while (true) {
                            ...
                            // Check interruption first, if we woke up due to interruption we need
                            // to honor that.
                            if (Thread.interrupted()) {
                                removeWaiter(node);
                                throw new InterruptedException();
        ...
        // If we get here then we have remainingNanos < SPIN_THRESHOLD_NANOS and there is no node on
        // the
        // waiters list
    ...
    public V get() throws InterruptedException, ExecutionException {
            ...
            do {
                ...
                if (ATOMIC_HELPER.casWaiters(this, oldHead, node)) {
                    ...
                    while (true) {
                        ...
                        // Otherwise re-read and check doneness. If we loop then it must have been a
                        // spurious
                        // wakeup
                        localValue = value;
                        if (localValue != null & !(localValue instanceof SetFuture)) {
        ...
        // re-read value, if we get here then we must have observed a TOMBSTONE while trying to add
        // a
        // waiter.
        return getDoneValue(value);
    }
     ...
     * {@linkplain #setFuture set asynchronously}, then the cancellation will also be propagated to
     * the delegate {@code Future} that was supplied in the {@code setFuture} call.
     */
    @CanIgnoreReturnValue
    @Override
    public boolean cancel(boolean mayInterruptIfRunning) {
        ...
        if (localValue == null | localValue instanceof SetFuture) {
            // Try to delay allocating the exception. At this point we may still lose the CAS, but
            // it is
            // certainly less likely.
            Throwable cause =
                    GENERATE_CANCELLATION_CAUSES ? new CancellationException("Future.cancel() was called.") : null;
            ...
            while (true) {
                if (ATOMIC_HELPER.casValue(abstractFuture, localValue, valueToSet)) {
                    ...
                    if (localValue instanceof SetFuture) {
                        // propagate cancellation to the future set in setfuture, this is racy, and
                        // we don't
                        // care if we are successful or not.
                        ListenableFuture<?> futureToPropagateTo = ((SetFuture) localValue).future;
                        if (futureToPropagateTo instanceof TrustedFuture) {
                            // If the future is a TrustedFuture then we specifically avoid calling
                            // cancel()
                            // this has 2 benefits
                            ...
                            // cancellation
                            // chain
                            // We can only do this for TrustedFuture, because TrustedFuture.cancel
                            // is final and
                ...
                if (!(localValue instanceof SetFuture)) {
                    // obj cannot be null at this point, because value can only change from null to
                    // non-null.
                    // So if value changed (and it did since we lost the CAS), then it cannot be
                    // null and
                    // since it isn't a SetFuture, then the future must be done and we should exit
                    // the loop
                    break;
                }
            }
    ...
    /**
     * Sets the result of this {@code Future} unless this {@code Future} has already been cancelled
     ...
     * if</b> the call was accepted (in which case it returns {@code true}). If it returns
     * {@code false}, the {@code Future} may have previously been set asynchronously, in which case
     * its result may not be known yet. That result, though not yet known, cannot be overridden by a
     * call to a {@code set*} method, only by a call to {@link #cancel}.
     *
     ...
     * by the time this method returns. If the supplied future is not done and the call is accepted,
     * then the future will be <i>set asynchronously</i>. Note that such a result, though not yet
     * known, cannot be overridden by a call to a {@code set*} method, only by a call to
     * {@link #cancel}.
     *
     * <p>
        ...
        if (localValue == null) {
            ...
            if (ATOMIC_HELPER.casValue(this, null, valueToSet)) {
                ...
                try {
                    ...
                    // addListener has thrown an exception! SetFuture.run can't throw any exceptions
                    // so this
                    ...
                    // Note: The only way this CAS could fail is if cancel() has raced with us. That
                    // is ok.
    ...
    private static void complete(AbstractFuture<?> future) {
        ...
        outer: while (true) {
            ...
            // stack data
            // structure for them.
            // afterDone() should be generally fast and only used for cleanup work... but in theory
            // can
            ...
            while (next != null) {
                ...
                if (task instanceof SetFuture) {
                    ...
                    // We unwind setFuture specifically to avoid StackOverflowErrors in the case of
                    // long
                    // chains of SetFutures
                    // Handling this special case is important because there is no way to pass an
                    // executor to
                    // setFuture, so a user couldn't break the chain by doing this themselves. It is
                    // also
     ...
     * fields. If your task does anything heavier consider, just using a listener with an executor.
     *
     * @since 20.0
     */
     ...
     * through a call to {@link setException} or {@link setFuture}{@code (failedFuture)} but not
     * cancellation.
     *
    ...
    private Listener clearListeners(Listener onto) {
        ...
        // 2. reverse the linked list, because despite our rather clear contract, people depend on
        // us
    ...
    private static void executeListener(Runnable runnable, Executor executor) {
        try {
            ...
            // runnables if
            // we're given a bad one. We only catch RuntimeException because we want Errors to
            // propagate
            // up.
     ...
     *
     * <p>
     * Static initialization of this class will fail if the {@link sun.misc.Unsafe} object cannot be
     * accessed.
     */
    ...
    private static CancellationException cancellationExceptionWithCause(@Nullable String message,
