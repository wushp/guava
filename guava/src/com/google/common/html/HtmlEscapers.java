 ...
 * systems and high-level APIs that provide autoescaping. One Google-authored templating system
 * available for external use is <a href="https://developers.google.com/closure/templates/">Closure
 * Templates</a>.
 *
 * <p>
 * HTML escaping is particularly tricky: For example, <a href="http://goo.gl/5TgZb">some elements'
 * text contents must not be HTML escaped</a>. As a result, it is impossible to escape an HTML
 * document correctly without domain-specific knowledge beyond what {@code HtmlEscapers} provides.
...
public final class HtmlEscapers {
     ...
     * <a href="http://www.w3.org/TR/html4/">HTML 4.01</a>. The resulting strings can be used both
     * in attribute values and in <em>most</em> elements' text contents, provided that the HTML
     * document's character encoding can encode any non-ASCII code points in the input (as UTF-8 and
     * other Unicode encodings can).
     *
     *
     * <p>
     * <b>Note:</b> This escaper only performs minimal escaping to make content structurally
     * compatible with HTML. Specifically, it does not perform entity replacement (symbolic or
     * numeric), so it does not replace non-ASCII code points with character references. This
     * escaper escapes only the following five ASCII characters: {@code '"&<>}.
     */
