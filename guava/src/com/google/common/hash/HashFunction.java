 ...
 * <i>some</i> degree to make this unlikely. (Without this condition, a function that always returns
 * zero could be called a hash function. It is not.)
 * </ul>
 *
 * <p>
 * Summarizing the last two points: "equal yield equal <i>always</i>; unequal yield unequal
 ...
 * token attempt, one measure of the quality of a hash function is <i>how well</i> it succeeds at
 * this goal. Important note: it may be easy to achieve the theoretical minimum collision rate when
 * using completely <i>random</i> sample input. The true test of a hash function is how it performs
 * on representative real-world data, which tends to contain many hidden patterns and clumps. The
 * goal of a good hash function is to stamp these patterns out as thoroughly as possible.
 *
 * <li><b>bit-dispersing:</b> masking out any <i>single bit</i> from a hash code should yield only
 * the expected <i>twofold</i> increase to all collision rates. Informally, the "information" in the
 * hash code should be as evenly "spread out" through the hash code's bits as possible. The result
 * is that, for example, when choosing a bucket in a hash table of size 2^8, <i>any</i> eight bits
 * could be consistently used.
 *
 * <li><b>cryptographic:</b> certain hash functions such as {@link Hashing#sha512} are designed to
 * make it as infeasible as possible to reverse-engineer the input that produced a given hash code,
 * or even to discover <i>any</i> two distinct inputs that yield the same result. These are called
 * <i>cryptographic hash functions</i>. But, whenever it is learned that either of these feats has
 * become computationally feasible, the function is deemed "broken" and should no longer be used for
 * secure purposes. (This is the likely eventual fate of <i>all</i> cryptographic hashes.)
 *
 ...
 *
 * <p>
 * <b>Compatibility note:</b> Throughout this API, multibyte values are always interpreted in
 ...
 *
 * <p>
 * {@code Object.hashCode} implementations tend to be very fast, but have weak collision prevention
 * and <i>no</i> expectation of bit dispersion. This leaves them perfectly suitable for use in hash
 * tables, because extra collisions cause only a slight performance hit, while poor bit dispersion
 * is easily corrected using a secondary hash function (which all reasonable hash table
...
public interface HashFunction {
     ...
     * functions (hash functions that need to buffer their whole input before processing any of it).
     */
     ...
     *
     * <p>
     * <b>Warning:</b> This method will produce different output than most other languages do when
     ...
     *
     * <p>
     * <b>Warning:</b> This method, which reencodes the input before hashing it, is useful only for
     * cross-language compatibility. For other use cases, prefer {@link #hashUnencodedChars}, which
     * is faster, produces the same output across Java releases, and hashes every {@code char} in
