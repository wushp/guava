...
public final class Hashing {
     ...
     * algorithm the returned function implements is unspecified and subject to change without
     * notice.
     *
     * <p>
     * <b>Warning:</b> a new random seed for these functions is chosen each time the {@code
    ...
    /**
     * Returns a hash function implementing the
     * <a href="https://github.com/aappleby/smhasher/blob/master/src/MurmurHash3.cpp">128-bit
     * murmur3 algorithm, x64 variant</a> (little-endian variant), using the given seed value.
     *
     * <p>
     ...
     * Returns a hash function implementing the
     * <a href="https://github.com/aappleby/smhasher/blob/master/src/MurmurHash3.cpp">128-bit
     * murmur3 algorithm, x64 variant</a> (little-endian variant), using a seed value of zero.
     *
     * <p>
     * The exact C++ equivalent is the MurmurHash3_x64_128 function (Murmur3F).
     */
     ...
     *
     * <p>
     * <b>Warning:</b> MD5 is not cryptographically secure or collision-resistant and is not
     ...
     *
     * <p>
     * <b>Warning:</b> SHA1 is not cryptographically secure and is not recommended for use in new
    ...
    /**
     * Returns a hash function implementing the SHA-512 algorithm (512 hash bits) by delegating to
     * the SHA-512 {@link MessageDigest}.
     */
    ...
    /**
     * Returns a hash function implementing the Message Authentication Code (MAC) algorithm, using
     * the MD5 (128 hash bits) hash function and a {@link SecretSpecKey} created from the given byte
     * array and the MD5 algorithm.
     *
     *
    ...
    /**
     * Returns a hash function implementing the CRC32C checksum algorithm (32 hash bits) as
     * described by RFC 3720, Section 12.1.
     *
     * @since 18.0
     */
    ...
    /**
     * Returns a hash function implementing the CRC-32 checksum algorithm (32 hash bits) by
     * delegating to the {@link CRC32} {@link Checksum}.
     *
     * <p>
     ...
     * buckets will be removed first. More concretely, if you are dividing traffic among tasks, you
     * can decrease the number of tasks from 15 and 10, killing off the final 5 tasks, and {@code
     ...
     * [alpha, bravo, charlie]} to {@code [bravo, charlie]}, it will assign all the old
     * {@code alpha} traffic to {@code bravo} and all the old {@code bravo} traffic to
    ...
    /**
     * Assigns to {@code input} a "bucket" in the range {@code [0, buckets)}, in a uniform manner
     ...
     * combines the information of these hash codes in an ordered fashion. That is, whenever two
     * equal hash codes are produced by two calls to this method, it is <i>as likely as possible</i>
     * that each was computed from the <i>same</i> input hash codes in the <i>same</i> order.
     *
    ...
    /**
     * Returns a hash code, having the same bit length as each of the input hash codes, that
     * combines the information of these hash codes in an unordered fashion. That is, whenever two
     * equal hash codes are produced by two calls to this method, it is <i>as likely as possible</i>
     * that each was computed from the <i>same</i> input hash codes in <i>some</i> order.
     *
     * @throws IllegalArgumentException if {@code hashCodes} is empty, or the hash codes do not all
     *         have the same bit length
     */
     ...
     * underlying hash functions together. This can be useful if you need to generate hash codes of
     * a specific length.
     *
     * <p>
