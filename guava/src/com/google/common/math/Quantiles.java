 ...
 *
 * <p>
 * These examples all take a copy of your dataset. If you have a double array, you are okay with it
 * being arbitrarily reordered, and you want to avoid that copy, you can use {@code computeInPlace}
 * instead of {@code compute}.
 *
 * <h3>Definition and notes on interpolation</h3>
 *
 * <p>
 * The definition of the kth q-quantile of N values is as follows: define x = k * (N - 1) / q; if x
 * is an integer, the result is the value which would appear at index x in the sorted dataset
 ...
 *
 * <p>
 * Otherwise, {@link Double#NEGATIVE_INFINITY NEGATIVE_INFINITY} and {@link Double#POSITIVE_INFINITY
 * POSITIVE_INFINITY} sort to the beginning and the end of the dataset, as you would expect.
 *
 * <p>
 ...
 *
 * <p>
 * When calling {@link ScaleAndIndex#compute} (in {@linkplain ScaleAndIndexes#compute either form}),
...
public final class Quantiles {
     ...
     *
     * @param scale the scale for the quantiles to be calculated, i.e. the q of the q-quantiles,
    ...
    /**
     * Describes the point in a fluent API chain where only the scale (i.e. the q in q-quantiles)
     * has been specified.
     */
    public static final class Scale {
         ...
         *
         * @param index the quantile index, which must be in the inclusive range [0, q] for
         *        q-quantiles
         */
         ...
         * q-quantile.
         *
         * @param indexes the quantile indexes, each of which must be in the inclusive range [0, q]
         *        for q-quantiles; the order of the indexes is unimportant, duplicates will be
         *        ignored, and the set will be snapshotted when this method is called
         */
     ...
     * the q and the k in the kth q-quantile) have been specified.
     */
    public static final class ScaleAndIndex {
         ...
         *
         * @param dataset the dataset to do the calculation on, which must be non-empty, which will
         *        be cast to doubles (with any associated lost of precision), and which will not be
         ...
         *
         * @param dataset the dataset to do the calculation on, which must be non-empty, which will
         *        be cast to doubles, and which will not be mutated by this call (it is copied
         *        instead)
         * @return the quantile value
         */
         ...
         *
         * @param dataset the dataset to do the calculation on, which must be non-empty, and which
         *        will be arbitrarily reordered by this method call
         * @return the quantile value
         */
        public double computeInPlace(double... dataset) {
            ...
            // index * (dataset.length - 1) / scale. If there is no remainder, we can just find the
            // value
            ...
            // non-negative int, we can do long-arithmetic on index * (dataset.length - 1) / scale
            // to get
            // a rounded ratio and a remainder which can be expressed as ints, without risk of
            // overflow:
    ...
    /**
     * Describes the point in a fluent API chain where the scale and a multiple quantile indexes
     * (i.e. the q and a set of values for the k in the kth q-quantile) have been specified.
     */
    public static final class ScaleAndIndexes {
         ...
         *        be cast to doubles (with any associated lost of precision), and which will not be
         *        mutated by this call (it is copied instead)
         * @return an unmodifiable map of results: the keys will be the specified quantile indexes,
         *         and the values the corresponding quantile values
         */
        ...
        public Map<Integer, Double> computeInPlace(double... dataset) {
            ...
            // Calculate the quotients and remainders in the integer division x = k * (N - 1) / q,
            // i.e.
            // index * (dataset.length - 1) / scale for each index in indexes. For each, if there is
            // no
            ...
            for (int i = 0; i < indexes.length; i++) {
                ...
                // Since scale is a positive int, index is in [0, scale], and (dataset.length - 1)
                // is a
                // non-negative int, we can do long-arithmetic on index * (dataset.length - 1) /
                // scale to
                // get a rounded ratio and a remainder which can be expressed as ints, without risk
                // of
                // overflow:
                int quotient = (int) LongMath.divide(numerator, scale, RoundingMode.DOWN);
                int remainder = (int) (numerator - (long) quotient * scale);
     ...
     * that all the values with indexes in the range [{@code from}, {@code required}) are less than
     * or equal to the value with index {@code required}, and all the values with indexes in the
     * range ({@code required}, {@code to}] are greater than or equal to that value. Therefore, the
     * value at {@code required} is the value which would appear at that index in the sorted
     * dataset.
     */
    private static void selectInPlace(int required, double[] array, int from, int to) {
        // If we are looking for the least element in the range, we can just do a linear search for
        // it.
        // (We will hit this whenever we are doing quantile interpolation: our first selection finds
        // the lower value, our second one finds the upper value by looking for the next least
        // element.)
        if (required == from) {
            int min = from;
    ...
    private static int partition(double[] array, int from, int to) {
        ...
        // Move all elements with indexes in (from, to] which are greater than the pivot to the end
        // of
        ...
        // We now know that all elements with indexes in (from, partitionPoint] are less than or
        // equal
        // to the pivot at from, and all elements with indexes in (partitionPoint, to] are greater
        // than
        // it. We swap the pivot into partitionPoint and we know the array is partitioned around
        // that.
     ...
     * halfway between the two (rounded down), from {@code array}, and ensure (by swapping elements
     * if necessary) that that pivot value appears at the start of the slice i.e. at {@code from}.
    ...
    private static int chooseNextSelection(int[] allRequired, int requiredFrom, int requiredTo, int from, int to) {
        ...
        // Do a binary search until we're down to the range of two which encloses centerFloor
        // (unless
        // all values are lower or higher than centerFloor, in which case we find the two highest or
        // lowest respectively). If centerFloor is in allRequired, we will definitely find it. If
        // not,
        // but centerFloor + 1 is, we'll definitely find that. The closest value to the true
        // (unrounded)
