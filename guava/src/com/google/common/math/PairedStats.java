...
public final class PairedStats implements Serializable {
     ...
     * is not guaranteed to return zero when the dataset consists of the same pair of values
     ...
     * product-moment correlation coefficient</a> of the values. The count must greater than one,
     * and the {@code x} and {@code y} values must both have non-zero population variance (i.e.
     * {@code xStats().populationVariance() > 0.0 && yStats().populationVariance() > 0.0}). The
     * result is not guaranteed to be exactly +/-1 even when the data are perfectly
     * (anti-)correlated, due to numerical errors. However, it is guaranteed to be in the inclusive
     * range [-1, +1].
     *
     * <h3>Non-finite values</h3>
     *
     ...
     * regression</a> of {@code y} as a function of {@code x}. The count must be greater than one,
     * and either the {@code x} or {@code y} data must have a non-zero population variance (i.e.
     * {@code xStats().populationVariance() > 0.0 || yStats().populationVariance() > 0.0}). The
     * result is guaranteed to be horizontal if there is variance in the {@code x} data but not the
     * {@code y} data, and vertical if there is variance in the {@code y} data but not the {@code x}
     * data.
     *
     * <p>
     ...
     * values (i.e. it is the square root of the mean of the squares of the vertical distances
     * between the data points and the best fit line). For this fit, this error is a fraction
     * {@code sqrt(1 - R*R)} of the population standard deviation of {@code y}, where {@code R} is
     * the Pearson's correlation coefficient (as given by
     * {@link #pearsonsCorrelationCoefficient()}).
     *
     * <p>
     * The corresponding root-mean-square error in {@code x} as a function of {@code y} is a
     * fraction {@code sqrt(1/(R*R) - 1)} of the population standard deviation of {@code x}. This
     * fit does not normally minimize that error: to do that, you should swap the roles of {@code x}
     * and {@code y}.
     *
     * <h3>Non-finite values</h3>
     *
     ...
     *
     * <p>
     * <b>Note:</b> This tests exact equality of the calculated statistics, including the floating
     ...
     * However, floating point rounding errors mean that it may be false for some instances where
     * the statistics are mathematically equal, including the same values in a different order.
     */
    @Override
     ...
     *
     * <p>
     * <b>Note:</b> This hash code is consistent with exact equality of the calculated statistics,
     ...
     *
     * <p>
     * <b>Note:</b> No guarantees are made regarding stability of the representation between
     * versions.
     */
    ...
    /**
     * Creates a {@link PairedStats} instance from the given byte representation which was obtained
     * by {@link #toByteArray}.
     *
     * <p>
     * <b>Note:</b> No guarantees are made regarding stability of the representation between
