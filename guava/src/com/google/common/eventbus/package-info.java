 ...
 * sole argument, and mark it with the {@link com.google.common.eventbus.Subscribe} annotation.
 * </ul>
 *
 * <p>
 ...
 * {@code registerCustomerChangeEventListener} method. These methods are rarely defined in common
 ...
 * {@link com.google.common.eventbus.EventBus#register(Object)} method on an EventBus. You'll need
 * to make sure that your object shares an EventBus instance with the event producers.
 * </ul>
 *
 * <p>
 ...
 *
 * <p>
 * Of course, if you'd like to have a process-wide EventBus singleton, there's nothing stopping you
 * from doing it that way. Simply have your container (such as Guice) create the EventBus as a
 * singleton at global scope (or stash it in a static field, if you're into that sort of thing).
 *
 * <p>
 * In short, the EventBus is not a singleton because we'd rather not make that decision for you. Use
 * it how you like.
 *
 ...
 *
 * <p>
 * Traditional Java Events use a listener interface which typically sports only a handful of methods
 * -- typically one. This has a number of disadvantages:
 * <ul>
 ...
 *
 * <p>
 * The difficulties in implementing this cleanly has given rise to a pattern, particularly common in
 * Swing apps, of using tiny anonymous classes to implement event listener interfaces.
 *
 * <p>
 ...
 *
 * <p>
 * Some have proposed a generic {@code Subscriber<T>} interface for EventBus listeners. This runs
 ...
 *
 * <p>
 * Due to erasure, no single class can implement a generic interface more than once with different
 * type parameters. This is a giant step backwards from traditional Java Events, where even if
 * {@code actionPerformed} and {@code keyPressed} aren't very meaningful names, at least you can
 * implement both methods!
 *
 * <h3>Doesn't EventBus destroy static typing and eliminate automated refactoring support?</h3>
 *
 * <p>
 * Some have freaked out about EventBus's {@code register(Object)} and {@code post(Object)} methods'
 * use of the {@code Object} type.
 *
 * <p>
 ...
 *
 * <p>
 * Event subscriber methods, on the other hand, must explicitly declare their argument type -- the
 * type of event desired (or one of its supertypes). Thus, searching for references to an event
 ...
 *
 * <p>
 * Immediately upon invoking {@code register(Object)}, the listener being registered is checked for
 * the <i>well-formedness</i> of its subscriber methods. Specifically, any methods marked with
 ...
 *
 * <p>
 * If a component posts events with no registered listeners, it <i>may</i> indicate an error
 * (typically an indication that you missed a {@code @Subscribe} annotation, or that the listening
 * component is not loaded).
 *
 * <p>
 * (Note that this is <i>not necessarily</i> indicative of a problem. There are many cases where an
 * application will deliberately ignore a posted event, particularly if the event is coming from
 ...
 *
 * <p>
 * Because subscriber methods on your listener classes are normal methods, you can simply call them
 * from your test code to simulate the EventBus.
 */
@CheckReturnValue
