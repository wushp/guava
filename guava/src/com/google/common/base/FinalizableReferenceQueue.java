 ...
 * If this object is garbage collected earlier, the backing thread will not invoke {@code
...
public class FinalizableReferenceQueue implements Closeable {
    /*
     * The Finalizer thread keeps a phantom reference to this object. When the client (for example,
     * a map built by MapMaker) no longer has a strong reference to this object, the garbage
     * collector will reclaim it and enqueue the phantom reference. The enqueued reference will
     * trigger the Finalizer to stop.
     *
     ...
     * have a strong reference back to the class loader. Otherwise, you could have a graph like
     * this:
     *
     ...
     *
     * Even if no other references to classes from the application class loader remain, the
     * Finalizer thread keeps an indirect strong reference to the queue in ReferenceMap, which keeps
     * the Finalizer running, and as a result, the application class loader can never be reclaimed.
     *
     ...
     * FinalizableReferenceQueue field in ReferenceMap. The application class loader can be
     * reclaimed at which point the Finalizer thread will stop and its decoupled class loader can
     * also be reclaimed.
     *
    ...
    /**
     * Tries to load Finalizer from the system class loader. If Finalizer is in the system class
     * path, we needn't create a separate loader.
     */
    ...
    /**
     * Try to load Finalizer in its own class loader. If Finalizer's thread had a direct reference
     * to our class loader (which could be that of a dynamically loaded web application or OSGi
     * bundle), it would prevent our class loader from getting garbage collected.
     */
    static class DecoupledLoader implements FinalizerLoader {
        ...
        public Class<?> loadFinalizer() {
            try {
                /*
                 * We use URLClassLoader because it's the only concrete class loader implementation
                 * in the JDK. If we used our own ClassLoader subclass, Finalizer would indirectly
                 * reference this class loader:
                 *
