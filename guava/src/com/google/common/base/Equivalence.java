 ...
 *
 * <p>
 * A future version of this class will implement {@code BiPredicate<T, T>}. In the meantime, to use
 * an equivalence (say, named {@code equivalence}) as a bi-predicate, use the method reference
...
public abstract class Equivalence<T> implements BiPredicate<T, T> {
     ...
     *
     * <p>
     * Called by {@link #equivalent}. {@code a} and {@code b} are not the same object and are not
     * nulls.
     *
     ...
     * according to the definition of the equivalence. The hash need not remain consistent from one
     ...
     * necessary that the hash be distributable across <i>inequivalence</i>. If
     * {@code equivalence(x, y)} is false, {@code hash(x) == hash(y)} may still be true.
    ...
    /**
     * Returns a new equivalence relation for {@code F} which evaluates equivalence by first
     * applying {@code function} to the argument, then evaluating using {@code this}. That is, for
     * any pair of non-null objects {@code x} and {@code y}, {@code
     ...
     * {@code Equivalence.identity().onResultOf(Functions.toStringFunction())} is broken because
     * it's not guaranteed that {@link Object#toString}) always returns the same string instance.
     *
     * @since 10.0
     */
     ...
     *
     * <p>
     * For example, given an {@link Equivalence} for {@link String strings} named {@code equiv} that
    ...
    public static final class Wrapper<T> implements Serializable {
        ...
        /**
         * Returns {@code true} if {@link Equivalence#equivalent(Object, Object)} applied to the
         * wrapped references is {@code true} and both wrappers use the {@link Object#equals(Object)
         * same} equivalence.
         */
        @Override
     ...
     * elements, and each pair of corresponding elements is equivalent according to {@code this}.
    ...
    public final <S extends T> Equivalence<Iterable<S>> pairwise() {
        ...
        // the need for this is so rare that it's not worth making callers deal with the ugly
        // wildcard.
     ...
     * value is null and {@link Object#equals} returns {@code true}. {@link Equivalence#hash}
