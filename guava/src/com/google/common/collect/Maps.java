 ...
 * {@link SortedMap}, {@link BiMap}, etc.). Also see this class's counterparts {@link Lists},
 * {@link Sets} and {@link Queues}.
 *
 * <p>
...
public final class Maps {
     ...
     * The resulting implementation is specialized for enum key types. The returned map and its
     * views will iterate over keys in their enum definition order, not encounter order.
     *
     * <p>
     * If the mapped keys contain duplicates, an {@code IllegalArgumentException} is thrown when the
     * collection operation is performed. (This differs from the {@code Collector} returned by
     ...
     *
     * <p>
     * <b>Note:</b> if mutability is not required, use {@link ImmutableMap#of()} instead.
     *
     * <p>
     ...
     * deprecated. Instead, use the {@code HashMap} constructor directly, taking advantage of the
     ...
     * broadly guaranteed, but it is observed to be true for OpenJDK 1.7. It also can't be
     * guaranteed that the method isn't inadvertently <i>oversizing</i> the returned map.
     *
    ...
    /**
     * Creates a <i>mutable</i> {@code HashMap} instance with the same mappings as the specified
     * map.
     *
     * <p>
     * <b>Note:</b> if mutability is not required, use {@link ImmutableMap#copyOf(Map)} instead.
     *
     * <p>
    ...
    /**
     * Returns a general-purpose instance of {@code ConcurrentMap}, which supports all optional
     * operations of the ConcurrentMap interface. It does not permit null keys or values. It is
     * serializable.
     *
     * <p>
     ...
     *
     * <p>
     * It is preferable to use {@code MapMaker} directly (rather than through this method), as it
     * presents numerous useful configuration options, such as the concurrency level, load factor,
     * key/value reference types, and value computation.
     *
     ...
     *
     * <p>
     * <b>Note:</b> if mutability is not required, use {@link ImmutableSortedMap#of()} instead.
     *
     * <p>
     * <b>Note for Java 7 and later:</b> this method is now unnecessary and should be treated as
     * deprecated. Instead, use the {@code TreeMap} constructor directly, taking advantage of the
     * new <a href="http://goo.gl/iz2Wi">"diamond" syntax</a>.
     *
     ...
     * <p>
     * <b>Note for Java 7 and later:</b> this method is now unnecessary and should be treated as
     * deprecated. Instead, use the {@code EnumMap} constructor directly, taking advantage of the
     * new <a href="http://goo.gl/iz2Wi">"diamond" syntax</a>.
     *
     ...
     * <p>
     * <b>Note for Java 7 and later:</b> this method is now unnecessary and should be treated as
     * deprecated. Instead, use the {@code IdentityHashMap} constructor directly, taking advantage
     ...
     *
     * <p>
     * Values are compared using a provided equivalence, in the case of equality, the value on the
     * 'left' is returned in the difference.
     *
     * <p>
     * Since this method uses {@code HashMap} instances internally, the keys of the supplied maps
     * must be well-behaved with respect to {@link Object#equals} and {@link Object#hashCode}.
     ...
     * difference is an immutable snapshot of the state of the maps at the time this method is
     * called. It will never change, even if the maps change at a later time.
     *
     * <p>
     ...
     *
     * <p>
     * <b>Note:</b>If you only need to know whether two sorted maps have the same mappings, call
     * {@code left.equals(right)} instead of this method.
     *
     * @param left the map to treat as the "left" map for purposes of comparison
     * @param right the map to treat as the "right" map for purposes of comparison
     ...
     * Ordering.natural()}. This method is an abomination of generics; the only purpose of this
     * method is to contain the ugly type-casting in one place.
     */
    @SuppressWarnings("unchecked")
    ...
    /**
     * Returns a live {@link Map} view whose keys are the contents of {@code set} and whose values
     * are computed on demand using {@code function}. To get an immutable <i>copy</i> instead, use
     * {@link #toMap(Iterable, Function)}.
     *
     * <p>
     * Specifically, for each {@code k} in the backing set, the returned map has an entry mapping
     * {@code k} to {@code function.apply(k)}. The {@code
     * keySet}, {@code values}, and {@code entrySet} views of the returned map iterate in the same
     * order as the backing set.
     *
     * <p>
     ...
     *
     * <p>
     * <b>Warning:</b> If the function rejects {@code null}, caution is required to make sure the
     * set does not contain {@code null}, because the view cannot stop {@code null} from being added
     * to the set.
     *
     * <p>
     * <b>Warning:</b> This method assumes that for any instance {@code k} of key type {@code K},
     * {@code k.equals(k2)} implies that {@code k2} is also of type {@code K}. Using a key type for
     * which this may not hold, such as {@code ArrayList}, may risk a {@code ClassCastException}
     * when calling methods on the resulting map view.
     *
     * @since 14.0
     */
    ...
    /**
     * Returns a view of the sorted set as a map, mapping keys from the set according to the
     * specified function.
     *
     * <p>
     * Specifically, for each {@code k} in the backing set, the returned map has an entry mapping
    ...
    private static final class NavigableAsMapView<K, V> extends AbstractNavigableMap<K, V> {
        /*
         * Using AbstractNavigableMap is simpler than extending SortedAsMapView and rewriting all
     ...
     * keys} and whose value for each key was computed by {@code valueFunction}. The map's iteration
     * order is the order of the first appearance of each key in {@code keys}.
     *
     * <p>
     * When there are multiple instances of a key in {@code keys}, it is unspecified whether
     ...
     *
     * <p>
     * If {@code keys} is a {@link Set}, a live view can be obtained instead of a copy using
     * {@link Maps#asMap(Set, Function)}.
     *
     ...
     * other words, each input value produces an entry in the map whose key is the result of
     * applying {@code keyFunction} to that value. These entries appear in the same order as the
     * input values. Example usage:
     * 
     * <pre>
     *    {@code
     *
     ...
     * @throws NullPointerException if any elements of {@code values} is null, or if
     ...
     * Properties normally derive from {@code Map<Object, Object>}, but they typically contain
     * strings, which is awkward. This method lets you get a plain-old-{@code Map} out of a
     * {@code Properties}.
     *
     ...
     * {@code equals} to comply with the Entry contract, to avoid a possible nefarious
    ...
    /**
     * Returns a {@link Converter} that converts values using {@link BiMap#get bimap.get()}, and
     * whose inverse view converts values using {@link BiMap#inverse bimap.inverse()}{@code .get()}.
     *
     * <p>
    ...
    private static final class BiMapConverter<A, B> extends Converter<A, B> implements Serializable {
        ...
        // There's really no good way to implement toString() without printing the entire BiMap,
        // right?
        @Override
        public String toString() {
    ...
    /**
     * Returns a synchronized (thread-safe) bimap backed by the specified bimap. In order to
     * guarantee serial access, it is critical that <b>all</b> access to the backing bimap is
     * accomplished through the returned bimap.
     *
     * <p>
     * It is imperative that the user manually synchronize on the returned map when accessing any of
     ...
     * "read through" to the specified bimap, and attempts to modify the returned map, whether
    ...
    /**
     * Returns a view of a map where each value is transformed by a function. All other properties
     * of the map, such as iteration order, are left intact. For example, the code:
     * 
     * <pre>
     * {
     *     &#64;code
     ...
     * like {@link Map#containsValue} and {@code Map.toString()}. For this to perform well,
     * {@code function} should be fast. To avoid lazy evaluation when the returned map doesn't need
     * to be a view, copy the returned map into a new map of your choosing.
     */
     ...
     * to {@link #transformValues}, this method's entry-transformation logic may depend on the key
     ...
     *
     * <p>
     * It's acceptable for the underlying map to contain null keys and null values provided that the
     * transformer is capable of accepting null inputs. The transformed map might contain null
     ...
     * {@code EntryTransformer} key type {@code K}, {@code k.equals(k2)} implies that {@code k2} is
     * also of type {@code K}. Using an {@code
     * EntryTransformer} key type for which this may not hold, such as {@code
     * ArrayList}, may risk a {@code ClassCastException} when calling methods on the transformed
     * map.
     *
     * @since 7.0
     */
    ...
    /**
     * Returns a view of a sorted map whose values are derived from the original sorted map's
     * entries. In contrast to {@link #transformValues}, this method's entry-transformation logic
    ...
    /**
     * A transformation of the value of a key-value pair, using both key and value as inputs. To
     * apply the transformation to a map, use {@link Maps#transformEntries(Map, EntryTransformer)}.
     *
    ...
    public interface EntryTransformer<K, V1, V2> {
         ...
         * </ul>
         *
         * @throws NullPointerException if the key or value is null and this transformer does not
     ...
     * values()} views have iterators that don't support {@code remove()}, but all other methods are
     * supported by the map and its views. When given a key that doesn't satisfy the predicate, the
     * map's {@code put()} and {@code putAll()} methods throw an {@link IllegalArgumentException}.
     *
     * <p>
     * When methods such as {@code removeAll()} and {@code clear()} are called on the filtered map
     ...
     *
     * <p>
     * Many of the filtered map's methods, such as {@code size()}, iterate across every key/value
     ...
     *
     * <p>
     * <b>Warning:</b> {@code keyPredicate} must be <i>consistent with equals</i>, as documented at
     * {@link Predicate#apply}. Do not provide a predicate such as
     * {@code Predicates.instanceOf(ArrayList.class)}, which is inconsistent with equals.
     */
     ...
     * predicate. The returned map is a live view of {@code
     ...
     *
     * <p>
     * The resulting bimap's {@code keySet()}, {@code entrySet()}, and {@code values()} views have
     ...
     *
     * <p>
     * When methods such as {@code removeAll()} and {@code clear()} are called on the filtered bimap
     * or its views, only mappings that satisfy the filter will be removed from the underlying
     * bimap.
     *
     * <p>
     ...
     *
     * <p>
     * Many of the filtered bimap's methods, such as {@code size()}, iterate across every key in the
     * underlying bimap and determine which satisfy the filter. When a live view is <i>not</i>
     * needed, it may be faster to copy the filtered bimap and use the copy.
     *
     * <p>
     * <b>Warning:</b> {@code entryPredicate} must be <i>consistent with equals </i>, as documented
     ...
     * The resulting map's {@code keySet()}, {@code entrySet()}, and {@code
     * values()} views have iterators that don't support {@code remove()}, but all other methods are
     * supported by the map and its views. When given a value that doesn't satisfy the predicate,
     * the map's {@code put()}, {@code
     * putAll()}, and {@link Entry#setValue} methods throw an {@link IllegalArgumentException}.
     *
     * <p>
     * When methods such as {@code removeAll()} and {@code clear()} are called on the filtered map
     * or its views, only mappings whose values satisfy the filter will be removed from the
     * underlying map.
     *
     * <p>
     * The returned map isn't threadsafe or serializable, even if {@code
     ...
     *
     * <p>
     * <b>Warning:</b> {@code valuePredicate} must be <i>consistent with equals</i>, as documented
     * at {@link Predicate#apply}. Do not provide a predicate such as
     * {@code Predicates.instanceOf(ArrayList.class)}, which is inconsistent with equals.
     */
     ...
     * {@link IllegalArgumentException}. Similarly, the map's entries have a {@link Entry#setValue}
     * method that throws an {@link IllegalArgumentException} when the provided value doesn't
     ...
     *
     * <p>
     * Many of the filtered bimap's methods, such as {@code size()}, iterate across every value in
     ...
     * The resulting map's {@code keySet()}, {@code entrySet()}, and {@code
     * values()} views have iterators that don't support {@code remove()}, but all other methods are
     * supported by the map and its views. When given a key/value pair that doesn't satisfy the
     ...
     * The resulting bimap's {@code keySet()}, {@code entrySet()}, and {@code values()} views have
     * iterators that don't support {@code remove()}, but all other methods are supported by the
     * bimap and its views. When given a key/value pair that doesn't satisfy the predicate, the
     * bimap's {@code put()}, {@code forcePut()} and {@code putAll()} methods throw an
     ...
     *
     * <p>
     * Many of the filtered bimap's methods, such as {@code size()}, iterate across every key/value
    ...
    /**
     * Support {@code clear()}, {@code removeAll()}, and {@code retainAll()} when filtering a
     * filtered map.
     */
    ...
    private static class FilteredEntryNavigableMap<K, V> extends AbstractNavigableMap<K, V> {
         ...
         * FilteredEntryMap than to extend FilteredEntrySortedMap and reimplement all the
    ...
    static final class FilteredEntryBiMap<K, V> extends FilteredEntryMap<K, V> implements BiMap<K, V> {
        ...
        public void replaceAll(BiFunction<? super K, ? super V, ? extends V> function) {
            unfiltered().replaceAll((key,
                    value) -> predicate.apply(Maps.immutableEntry(key, value)) ? function.apply(key, value) : value);
        }
     ...
     * map read through to the specified map, and attempts to modify the returned map, whether
     * direct or via its views, result in an {@code UnsupportedOperationException}.
     *
     * <p>
     ...
     *
     * <p>
     * This method's signature will not permit you to convert a {@code NavigableMap<? extends K,
     ...
     * {@code descendingMap}, {@code subMap}, {@code headMap} or {@code tailMap} views.
     * 
     * <pre>
     * {
     *     &#64;code
    ...
    abstract static class ViewCachingAbstractMap<K, V> extends AbstractMap<K, V> {
        /**
         * Creates the entry set to be returned by {@link #entrySet()}. This method is invoked at
         * most once on a given map, at the time when {@code entrySet} is first called.
         */
     ...
     * {@code o} is an instance of {@code Map.Entry}, it is wrapped using {@link #unmodifiableEntry}
     * to protect against a possible nefarious equals method.
     *
     * <p>
     * Note that {@code c} is the backing (delegate) collection, rather than the forwarding
     * collection.
     *
     ...
     *
     * <p>
     * Note that {@code c} is backing (delegate) collection, rather than the forwarding collection.
     *
     * @param c the delegate (unwrapped) collection of map entries
     ...
     *
     * <p>
     * <b>Warning:</b> {@code Range}s always represent a range of values using the values' natural
