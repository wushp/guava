...
public final class Queues {
    ...
    /**
     * Creates an empty {@code ArrayBlockingQueue} with the given (fixed) capacity and nonfair
     * access policy.
     */
     ...
     * containing the elements of the specified iterable, in the order they are returned by the
     * iterable's iterator.
     *
     * @since 12.0
     */
    ...
    /**
     * Creates an empty {@code PriorityBlockingQueue} with the ordering given by its elements'
     * natural ordering.
     *
    ...
    public static <E> int drain(BlockingQueue<E> q, Collection<? super E> buffer, int numElements, long timeout,
        ...
        while (added < numElements) {
            ...
            if (added < numElements) { // not enough elements immediately available; will have to
                                       // poll
     ...
     * with a different behavior in case it is interrupted while waiting. In that case, the
    ...
    public static <E> int drainUninterruptibly(BlockingQueue<E> q, Collection<? super E> buffer, int numElements,
        ...
        try {
            while (added < numElements) {
                // we could rely solely on #poll, but #drainTo might be more efficient when there
                // are
                // multiple elements already available (e.g. LinkedBlockingQueue#drainTo locks only
                // once)
                added += q.drainTo(buffer, numElements - added);
                if (added < numElements) { // not enough elements immediately available; will have
                                           // to poll
                    E e; // written exactly once, by a successful (uninterrupted) invocation of
                         // #poll
                    while (true) {
                        try {
