...
public final class Multimaps {
     ...
     * specified supplier. The keys and values of the entries are the result of applying the
     * provided mapping functions to the input elements, accumulated in the encounter order of the
     * stream.
     *
     * <p>
     * Example:
    ...
    /**
     * Returns a {@code Collector} accumulating entries into a {@code Multimap} generated from the
     * specified supplier. Each input element is mapped to a key and a stream of values, each of
     * which are put into the resulting {@code Multimap}, in the encounter order of the stream and
     * the encounter order of the streams of values.
     *
     * <p>
     * Example:
     *
     ...
     *
     * <p>
     * <b>Warning: do not use</b> this method when the collections returned by {@code factory}
     * implement either {@link List} or {@code Set}! Use the more specific method
     * {@link #newListMultimap}, {@link #newSetMultimap} or {@link #newSortedSetMultimap} instead,
     * to avoid very surprising behavior from {@link Multimap#equals}.
     *
     * <p>
     * The {@code factory}-generated and {@code map} classes determine the multimap iteration order.
     * They also specify the behavior of the {@code equals}, {@code hashCode}, and {@code
     * toString} methods for the multimap and its returned views. However, the multimap's
     * {@code get} method returns instances of a different class than {@code factory.get()} does.
     *
     * <p>
     ...
     * {@code map} and the instances generated by {@code factory} are. Concurrent read operations
     * will work correctly. To allow concurrent update operations, wrap the multimap with a call to
     ...
     * <p>
     * The {@code factory}-generated and {@code map} classes determine the multimap iteration order.
     * They also specify the behavior of the {@code equals}, {@code hashCode}, and {@code toString}
     * methods for the multimap and its returned views. The multimap's {@code get}, {@code
     * removeAll}, and {@code replaceValues} methods return {@code RandomAccess} lists if the
     * factory does. However, the multimap's {@code get} method returns instances of a different
     * class than does {@code factory.get()}.
     *
     * <p>
     * The multimap is serializable if {@code map}, {@code factory}, the lists generated by
     * {@code factory}, and the multimap contents are all serializable.
     *
     * <p>
     * The multimap is not threadsafe when any concurrent operations update the multimap, even if
     ...
     *
     * <p>
     * Call this method only when the simpler methods {@link ArrayListMultimap#create()} and
     ...
     * The {@code factory}-generated and {@code map} classes determine the multimap iteration order.
     * They also specify the behavior of the {@code equals}, {@code hashCode}, and {@code toString}
     * methods for the multimap and its returned views. However, the multimap's {@code get} method
    ...
    /**
     * Creates a new {@code SortedSetMultimap} that uses the provided map and factory. It can
     * generate a multimap based on arbitrary {@link Map} and {@link SortedSet} classes.
     *
     * <p>
     * The {@code factory}-generated and {@code map} classes determine the multimap iteration order.
     * They also specify the behavior of the {@code equals}, {@code hashCode}, and {@code toString}
     ...
     * <p>
     * Note that the generated multimap's {@link Multimap#removeAll} and
     * {@link Multimap#replaceValues} methods return collections that are modifiable.
     *
     * <p>
     * The returned multimap will be serializable if the specified multimap is serializable.
     *
    ...
    private static class UnmodifiableSetMultimap<K, V> extends UnmodifiableMultimap<K, V> implements SetMultimap<K, V> {
        ...
        public Set<V> get(K key) {
            /*
             * Note that this doesn't return a SortedSet when delegate is a SortedSetMultiset,
             * unlike (SortedSet<V>) super.get().
             */
     ...
     * returned multimap "read through" to the specified multimap, and attempts to modify the
     ...
     * instances of {@code SortedSet}, {@code Set}, {@code List} and {@code Collection}, in that
     * order of preference.
     *
     ...
     * {@link Entry#setValue} operation throws an {@link UnsupportedOperationException}. If the
     * specified collection is a {@code
     ...
     * {@code Map<K, Collection<V>>} to {@code Map<K, List<V>>}.
     *
     * @since 15.0
     */
    @Beta
     ...
     * {@code Map<K, Collection<V>>} to {@code Map<K, Set<V>>}.
     *
     * @since 15.0
     */
    @Beta
    ...
    /**
     * Returns {@link SortedSetMultimap#asMap multimap.asMap()}, with its type corrected from
     * {@code Map<K, Collection<V>>} to {@code Map<K, SortedSet<V>>}.
     *
     * @since 15.0
     */
    @Beta
    ...
    /**
     * Returns a multimap view of the specified map. The multimap is backed by the map, so changes
     * to the map are reflected in the multimap, and vice versa. If the map is modified while an
     ...
     * It does not support any operations which might add mappings, such as {@code put},
     * {@code putAll} or {@code replaceValues}.
     *
     * <p>
     ...
     *
     * <p>
     * Changes in the underlying multimap are reflected in this view. Conversely, this view supports
     * removal operations, and these are reflected in the underlying multimap.
     *
     * <p>
     ...
     * The {@code equals} and {@code hashCode} methods of the returned multimap are meaningless,
     * since there is not a definition of {@code equals} or {@code hashCode} for general
     * collections, and {@code get()} will return a general {@code Collection} as opposed to a
     ...
     * In contrast to {@link #transformValues}, this method's entry-transformation logic may depend
     * on the key as well as the value.
     *
     * <p>
     ...
     * operations like {@link Multimap#containsValue} and {@link Object#toString}. For this to
     * perform well, {@code transformer} should be fast. To avoid lazy evaluation when the returned
     * multimap doesn't need to be a view, copy the returned multimap into a new multimap of your
     * choosing.
     *
     * <p>
     ...
     * multimap's entries. In contrast to {@link #transformValues(ListMultimap, Function)}, this
     ...
     * specified function to each item in an {@code Iterable} of values. Each value will be stored
     * as a value in the resulting multimap, yielding a multimap with the same size as the input
    ...
    /**
     * Creates an index {@code ImmutableListMultimap} that contains the results of applying a
     * specified function to each item in an {@code Iterator} of values. Each value will be stored
     * as a value in the resulting multimap, yielding a multimap with the same size as the input
     ...
     *
     * <p>
     * The resulting multimap's views have iterators that don't support {@code remove()}, but all
     * other methods are supported by the multimap and its views. When adding a key that doesn't
     * satisfy the predicate, the multimap's {@code put()}, {@code putAll()}, and
     * {@code replaceValues()} methods throw an {@link IllegalArgumentException}.
     *
     * <p>
     * When methods such as {@code removeAll()} and {@code clear()} are called on the filtered
     ...
     *
     * <p>
     * Many of the filtered multimap's methods, such as {@code size()}, iterate across every
    ...
    public static <K, V> Multimap<K, V> filterKeys(Multimap<K, V> unfiltered, final Predicate<? super K> keyPredicate) {
        if (unfiltered instanceof SetMultimap) {
            ...
            return new FilteredKeyMultimap<K, V>(prev.unfiltered, Predicates.<K>and(prev.keyPredicate, keyPredicate));
    ...
    public static <K, V> SetMultimap<K, V> filterKeys(SetMultimap<K, V> unfiltered,
        ...
        if (unfiltered instanceof FilteredKeySetMultimap) {
            ...
            return new FilteredKeySetMultimap<K, V>(prev.unfiltered(),
                    Predicates.<K>and(prev.keyPredicate, keyPredicate));
     ...
     * The returned multimap is a live view of {@code unfiltered}; changes to one affect the other.
     *
     * <p>
     * The resulting multimap's views have iterators that don't support {@code remove()}, but all
     * other methods are supported by the multimap and its views. When adding a key/value pair that
     * doesn't satisfy the predicate, multimap's {@code put()}, {@code putAll()}, and
     * {@code replaceValues()} methods throw an {@link IllegalArgumentException}.
     *
     * <p>
     * When methods such as {@code removeAll()} and {@code clear()} are called on the filtered
     ...
     * iterators that don't support remove, passing one to the FilteredEntryMultimap constructor
     * would lead to a multimap whose removal operations would fail. This method combines the
     * predicates to avoid that problem.
     */
