...
public class TreeBasedTable<R, C, V> extends StandardRowSortedTable<R, C, V> {
    ...
    /**
     * Creates an empty {@code TreeBasedTable} that uses the natural orderings of both row and
     * column keys.
     *
     * <p>
     ...
     * {@link Ordering#natural()} is returned.
     *
     * @deprecated Store the {@link Comparator} alongside the {@link Table}. Or, if you know that
     *             the {@link Table} contains at least one value, you can retrieve the
     *             {@link Comparator} with:
     *             {@code ((SortedMap<C, V>) table.rowMap().values().iterator().next()).comparator();}.
     *             This method is scheduled for removal in April 2019.
     */
    @Deprecated
    ...
    private class TreeRow extends Row implements SortedMap<C, V> {
        ...
        /*
         * If the row was previously empty, we check if there's a new row here every time we're
         * queried.
         */
    ...
    Iterator<C> createColumnKeyIterator() {
        ...
        final Iterator<C> merged =
                Iterators.mergeSorted(Iterables.transform(backingMap.values(), new Function<Map<C, V>, Iterator<C>>() {
                    @Override
