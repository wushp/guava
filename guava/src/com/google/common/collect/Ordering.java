 ...
 *
 * <p>
 * Finally, use the resulting {@code Ordering} anywhere a {@link Comparator} is required, or use any
 * of its special operations, such as:
 *
 * <ul>
 ...
 *
 * <p>
 * Alas, {@link #reverse} is a little different. As you read backwards through a chain and encounter
 * a call to {@code reverse}, continue working backwards until a result is determined, and then
 * reverse that result.
 *
 * <h3>Additional notes</h3>
 *
 * <p>
 * Except as noted, the orderings returned by the factory methods of this class are serializable if
 * and only if the provided instances that back them are. For example, if {@code ordering} and
 ...
 *
 * <p>
 * If you are using Java 8, this class is now obsolete <i>(pending a few August 2016 updates)</i>.
 * Most of its functionality is now provided by {@link java.util.stream.Stream Stream} and by
 * {@link Comparator} itself, and the rest can now be found as static methods in our new
 ...
 *
 * <p>
 * Many replacements involve adopting {@code Stream}, and these changes can sometimes make your code
 * verbose. Whenever following this advice, you should check whether {@code Stream} could be adopted
 * more comprehensively in your code; the end result may be quite a bit simpler.
 *
 * <h3>See also</h3>
 *
 * <p>
...
public abstract class Ordering<T> implements Comparator<T> {
    ...
    /**
     * Returns a serializable ordering that uses the natural order of the values. The ordering
     * throws a {@link NullPointerException} when passed a null parameter.
     *
     * <p>
     * The type specification is {@code <C extends Comparable>}, instead of the technically correct
     ...
     * to pass it in here. Instead, simply subclass {@code Ordering} and implement its
     * {@code compare} method directly.
     *
     * <p>
     * <b>Java 8 users:</b> this class is now obsolete as explained in the class documentation, so
     ...
     * changes to the {@code valuesInOrder} list will have no effect on the returned comparator.
     ...
     * this method. Only objects present in the argument list (according to {@link Object#equals})
     * may be compared. This comparator imposes a "partial ordering" over the type {@code T}. Null
     * values in the argument list are not supported.
     *
     * <p>
     * The returned comparator throws a {@link ClassCastException} when it receives an input
     * parameter that isn't among the provided values.
     ...
     *        values
     * @param remainingValuesInOrder the rest of the values that the returned comparator will be
     ...
     *
     * <p>
     * Assuming {@code t}, {@code e} and {@code s} are non-null, this returns {@code [t, e, s, t,
     ...
     *
     * <p>
     * <b>Warning:</b> by definition, this comparator is not <i>consistent with equals</i> (as
     * defined {@linkplain Comparator here}). Avoid its use in APIs, such as
     ...
     *
     * <p>
     * <b>Java 8 users:</b> Use the lambda expression {@code (a, b) -> 0} instead (in certain cases
     ...
     * {@code a == b} (identity equality). There is no meaning whatsoever to the order imposed, but
     * it is constant for the life of the VM.
     *
     * <p>
     * Because the ordering is identity-based, it is not "consistent with
     * {@link Object#equals(Object)}" as defined by {@link Comparator}. Use caution when building a
     * {@link SortedSet} or {@link SortedMap} from it, as the resulting collection will not behave
    ...
    static class ArbitraryOrdering extends Ordering<Object> {
        ...
        /*
         * We need to be able to mock identityHashCode() calls for tests, because it can take 1-10
         * seconds to find colliding objects. Mocking frameworks that can do magic to mock static
         * method calls still can't do so for a system class, so we need the indirection. In
         * production, Hotspot should still recognize that the call is 1-morphic and should still be
         * willing to inline it if necessary.
         */
    ...
    /**
     * Constructs a new instance of this class (only invokable by the subclass constructor,
     * typically implicit).
     */
    protected Ordering() {}
    ...
    /**
     * Returns a new ordering on {@code F} which orders elements by first applying a function to
     * them, then comparing those results using {@code this}. For example, to compare objects by
     * their string forms, in a case-insensitive manner, use:
     *
     * <pre>
     * {@code
     ...
     *
     * <p>
     * <b>Java 8 users:</b> Use {@code Comparator.comparing(function, thisComparator)} instead (you
     ...
     * "tie", then delegates to {@code secondaryComparator}. For example, to sort a bug list first
     * by status and second by priority, you might use {@code byStatus.compound(byPriority)}. For a
     * compound ordering with three or more components, simply chain multiple calls to this method.
     *
     * <p>
     * An ordering produced by this method, or a chain of calls to this method, is equivalent to one
     * created using {@link Ordering#compound(Iterable)} on the same component comparators.
     *
     * <p>
     * <b>Java 8 users:</b> Use {@code thisComparator.thenComparing(secondaryComparator)} instead.
     ...
     * returned ordering is based on the state of the {@code comparators} iterable at the time it
     * was provided to this method.
     *
     * <p>
     ...
     *
     * <p>
     * <b>Java 8 users:</b> Use a chain of calls to {@link Comparator#thenComparing(Comparator)}, or
     * {@code comparatorCollection.stream().reduce(Comparator::thenComparing).get()} (if the
     * collection might be empty, also provide a default comparator as the {@code identity}
     * parameter to {@code reduce}).
     *
    ...
    /**
     * Returns a new ordering which sorts iterables by comparing corresponding elements pairwise
     * until a nonzero result is found; imposes "dictionary order". If the end of one iterable is
     * reached, but not the other, the shorter iterable is considered to be less than the longer
     * one. For example, a lexicographical natural ordering over integers considers
     * {@code [] < [1] < [1, 1] <
     * [1, 2] < [2]}.
     *
     * <p>
     * Note that {@code ordering.lexicographical().reverse()} is not equivalent to {@code
     * ordering.reverse().lexicographical()} (consider how each would order {@code [1]} and
     * {@code [1,
     * 1]}).
     *
     * <p>
     * <b>Java 8 users:</b> Use {@link Comparators#lexicographical(Comparator)} instead.
     *
     * @since 2.0
     */
    @GwtCompatible(serializable = true)
    ...
    public <S extends T> Ordering<Iterable<S>> lexicographical() {
        /*
         * Note that technically the returned ordering should be capable of handling not just {@code
         * Iterable<S>} instances, but also any {@code Iterable<? extends S>}. However, the need for
         * this comes up so rarely that it doesn't justify making everyone else deal with the very
         * ugly wildcard.
         */
     ...
     *
     * <p>
     * <b>Java 8 users:</b> Continue to use this method for now. After the next release of Guava,
     ...
     *
     * <p>
     * <b>Implementation note:</b> this method is invoked by the default implementations of the
     * other {@code min} overloads, so overriding it will affect their behavior.
     *
     * <p>
     * <b>Java 8 users:</b> Use {@code Collections.min(Arrays.asList(a, b), thisComparator)} instead
     ...
     *
     * <p>
     * <b>Java 8 users:</b> Use {@code Collections.min(Arrays.asList(a, b, c...), thisComparator)}
    ...
    /**
     * Returns the greatest of the specified values according to this ordering. If there are
     * multiple greatest values, the first of those is returned. The iterator will be left
    ...
    /**
     * Returns the greatest of the specified values according to this ordering. If there are
     * multiple greatest values, the first of those is returned.
     *
     * <p>
     * <b>Java 8 users:</b> If {@code iterable} is a {@link Collection}, use {@code
     ...
     *
     * <p>
     * <b>Java 8 users:</b> Use {@code Collections.max(Arrays.asList(a, b), thisComparator)} instead
     ...
     *
     * <p>
     * <b>Java 8 users:</b> Use {@code Collections.max(Arrays.asList(a, b, c...), thisComparator)}
    ...
    /**
     * Returns the {@code k} least elements of the given iterable according to this ordering, in
     * order from least to greatest. If there are fewer than {@code k} elements present, all will be
     * included.
     *
     * <p>
     * The implementation does not necessarily use a <i>stable</i> sorting algorithm; when multiple
     * elements are equivalent, it is undefined which will come first.
     *
     * <p>
     * <b>Java 8 users:</b> Use {@code Streams.stream(iterable).collect(Comparators.least(k,
     ...
     *
     * <p>
     * <b>Java 8 users:</b> Use {@code Streams.stream(iterable).collect(Comparators.greatest(k,
     * thisComparator))} instead.
     *
    ...
    /**
     * Returns the {@code k} greatest elements from the given iterator according to this ordering,
     * in order from greatest to least. If there are fewer than {@code k} elements present, all will
     * be included.
     *
     * <p>
     * The implementation does not necessarily use a <i>stable</i> sorting algorithm; when multiple
     ...
     * <p>
     * <b>Java 8 users:</b> Continue to use this method for now. After the next release of Guava,
     * use {@code Streams.stream(iterator).collect(Comparators.greatest(k, thisComparator))}
     * instead.
     *
     * @return an immutable {@code RandomAccess} list of the {@code k} greatest elements in
     *         <i>descending order</i>
     ...
     * duplicates according to the comparator. The sort performed is <i>stable</i>, meaning that
     * such elements will appear in the returned list in the same order they appeared in
     * {@code elements}.
     *
     * <p>
     * <b>Performance note:</b> According to our benchmarking on Open JDK 7,
     * {@link #immutableSortedCopy} generally performs better (in both time and space) than this
     * method, and this method in turn generally performs better than copying the list and calling
    ...
    /**
     * Returns an <b>immutable</b> list containing {@code elements} sorted by this ordering. The
     ...
     *
     * <p>
     * <b>Performance note:</b> According to our benchmarking on Open JDK 7, this method is the most
     ...
     *
     * <p>
     * <b>Java 8 users:</b> Use the equivalent {@link Comparators#isInOrder(Iterable, Comparator)}
     ...
     * greater than the element that preceded it, according to this ordering. Note that this is
     * always true when the iterable has fewer than two elements.
     *
     * <p>
     * <b>Java 8 users:</b> Use the equivalent
     * {@link Comparators#isInStrictOrder(Iterable, Comparator)} instead, since the rest of
     * {@code Ordering} is mostly obsolete (as explained in the class documentation).
     */
     ...
     * {@link Ordering#explicit(Object, Object[])} comparator when comparing a value outside the set
     * of values it can compare. Extending {@link ClassCastException} may seem odd, but it is
     * required.
     */
    @VisibleForTesting
