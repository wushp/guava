 ...
 *
 * <p>
 * Elements of a multiset that are equal to one another are referred to as <i>occurrences</i> of the
 * same single element. The total number of occurrences of an element in a multiset is called the
 * <i>count</i> of that element (the terms "frequency" and "multiplicity" are equivalent, but not
 * used in this API). Since the count of an element is represented as an {@code
 * int}, a multiset may never contain more than {@link Integer#MAX_VALUE} occurrences of any one
 * element.
 *
 * <p>
 * {@code Multiset} refines the specifications of several methods from {@code Collection}. It also
 * defines an additional query operation, {@link #count}, which returns the count of an element.
 * There are five new bulk-modification operations, for example {@link #add(Object, int)}, to add or
 * remove multiple occurrences of an element at once, or to set the count of an element to a
 * specific value. These modification operations are optional, but implementations which support the
 * standard collection operations {@link #add(Object)} or {@link #remove(Object)} are encouraged to
 * implement the related methods as well. Finally, two collection views are provided:
 * {@link #elementSet} contains the distinct elements of the multiset "with duplicates collapsed",
 * and {@link #entrySet} is similar but contains {@link Entry Multiset.Entry} instances, each
 ...
 * Multiset} are expected to provide two {@code static} creation methods: {@code create()},
 * returning an empty multiset, and {@code
 * create(Iterable<? extends E>)}, returning a multiset containing the given initial elements. This
 * is simply a refinement of {@code Collection}'s constructor recommendations, reflecting the new
 * developments of Java 5.
 *
 * <p>
...
public interface Multiset<E> extends Collection<E> {
     ...
     *
     * <p>
     * <b>Note:</b> this method does not return the number of <i>distinct elements</i> in the
     ...
     * element). Note that for an {@link Object#equals}-based multiset, this gives the same result
     * as {@link Collections#frequency} (which would presumably perform more poorly).
     *
     * <p>
     * <b>Note:</b> the utility method {@link Iterables#frequency} generalizes this operation; it
     ...
     * {@code occurrences == 1}, this method has the identical effect to {@link #add(Object)}. This
     * method is functionally equivalent (except in the case of overflow) to the call
     * {@code addAll(Collections.nCopies(element,
     * occurrences))}, which would presumably perform much more poorly.
     *
     * @param element the element to add occurrences of; may be null only if explicitly allowed by
     *        the implementation
     ...
     * @throws NullPointerException if {@code element} is null and this implementation does not
     ...
     * removed. Note that if {@code occurrences == 1}, this is functionally equivalent to the call
     * {@code
     * remove(element)}.
     *
     ...
     * {@link #setCount(Object, int)}, provided that the element has the expected current count. If
     * the current count is not {@code oldCount}, no change is made.
     *
     ...
     * @return {@code true} if the condition for modification was met. This implies that the
     *         multiset was indeed modified, unless {@code oldCount == newCount}.
     * @throws IllegalArgumentException if {@code oldCount} or {@code newCount} is negative
     * @throws NullPointerException if {@code element} is null and the implementation does not
     *         permit null elements. Note that if {@code
     *     oldCount} and {@code newCount} are both zero, the implementor may optionally return
     *         {@code true} instead.
     */
    @CanIgnoreReturnValue
     ...
     *
     * <p>
     * If the element set supports any removal operations, these necessarily cause <b>all</b>
     * occurrences of the removed element(s) to be removed from the multiset. Implementations are
     * not expected to support the add operations, although this is possible.
     *
     * <p>
     ...
     * Multiset.Entry} instances, each providing an element of the multiset and the count of that
     * element. This set contains exactly one entry for each distinct element in the multiset (thus
     * it always has the same size as the {@link #elementSet}). The order of the elements in the
    ...
    /**
     * An unmodifiable element-count pair for a multiset. The {@link Multiset#entrySet} method
     * returns a view of the multiset whose elements are of this class. A multiset implementation
     * may return Entry instances that are either live "read-through" views to the Multiset, or
     * immutable snapshots. Note that this type is unrelated to the similarly-named type
     * {@code Map.Entry}.
     *
     * @since 2.0
     */
    interface Entry<E> {
        ...
        /**
         * Returns the count of the associated element in the underlying multiset. This count may
         * either be an unchanging snapshot of the count at the time the entry was retrieved, or a
         * live view of the current count of the element in the multiset, depending on the
         * implementation. Note that in the former case, this method can never return zero, while in
         * the latter, it will return zero if all occurrences of the element were since removed from
         * the multiset.
         *
         ...
         * represent the same element and count. That is, two entries {@code a} and {@code b} are
         * equal if:
         * 
         * <pre>
         *    {@code
        ...
        /**
         * Returns the canonical string representation of this entry, defined as follows. If the
         * count for this entry is one, this is simply the string representation of the
         * corresponding element. Otherwise, it is the string representation of the element,
         * followed by the three characters {@code
     ...
     * {@link #toString} on the {@link #entrySet}, yielding a result such as
     * {@code [a x 3, c, d x 2, e]}.
     */
    @Override
    String toString();
     ...
     *
     * <p>
     * This method refines {@link Collection#contains} to further specify that it <b>may not</b>
     ...
     *
     * <p>
     * This method refines {@link Collection#containsAll} to further specify that it <b>may not</b>
     * throw an exception in response to any of {@code
     * elements} being null or of the wrong type.
     *
     * <p>
     * <b>Note:</b> this method does not take into account the occurrence count of an element in the
     * two collections; it may still return {@code
     * true} even if {@code elements} contains several occurrences of an element and this multiset
     * contains only one. This is no different than any other collection type like {@link List}, but
     ...
     *
     * <p>
     * This method refines {@link Collection#add}, which only <i>ensures</i> the presence of the
     ...
     * {@link #add(E, int) add}{@code (element, 1)} instead.
     *
     * @param element the element to add one occurrence of; may be null only if explicitly allowed
     ...
     *
     * <p>
     * This method refines {@link Collection#remove} to further specify that it <b>may not</b> throw
     * an exception in response to {@code element} being null or of the wrong type.
     *
     * <p>
     * To both remove the element and obtain the previous count of that element, use
     * {@link #remove(E, int) remove}{@code (element, 1)} instead.
     *
     ...
     *
     * <p>
     * <b>Note:</b> This method ignores how often any element might appear in {@code c}, and only
     * cares whether or not an element appears at all. If you wish to remove one occurrence in this
     * multiset for every occurrence in {@code c}, see
     * {@link Multisets#removeOccurrences(Multiset, Multiset)}.
     *
     * <p>
     * This method refines {@link Collection#removeAll} to further specify that it <b>may not</b>
     ...
     * cares whether or not an element appears at all. If you wish to remove one occurrence in this
     * multiset for every occurrence in {@code c}, see
     * {@link Multisets#retainOccurrences(Multiset, Multiset)}.
     *
     * <p>
     * This method refines {@link Collection#retainAll} to further specify that it <b>may not</b>
     * throw an exception in response to any of {@code elements} being null or of the wrong type.
     *
     ...
     *
     * <p>
     * Elements that occur multiple times in the multiset will be passed to the {@code Consumer}
