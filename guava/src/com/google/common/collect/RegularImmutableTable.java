...
abstract class RegularImmutableTable<R, C, V> extends ImmutableTable<R, C, V> {
    ...
    static <R, C, V> RegularImmutableTable<R, C, V> forCells(List<Cell<R, C, V>> cells,
        ...
        if (rowComparator != null || columnComparator != null) {
            /*
             * This sorting logic leads to a cellSet() ordering that may not be expected and that
             * isn't documented in the Javadoc. If a row Comparator is provided, cellSet() iterates
             * across the columns in the first row, the columns in the second row, etc. If a column
             * Comparator is provided but a row Comparator isn't, cellSet() iterates across the rows
             * in the first column, the rows in the second column, etc.
             */
            Comparator<Cell<R, C, V>> comparator = new Comparator<Cell<R, C, V>>() {
                @Override
                public int compare(Cell<R, C, V> cell1, Cell<R, C, V> cell2) {
                    int rowCompare =
                            (rowComparator == null) ? 0 : rowComparator.compare(cell1.getRowKey(), cell2.getRowKey());
                    if (rowCompare != 0) {
                        return rowCompare;
                    }
