 ...
 * a regular priority queue, the methods {@link #peekLast}, {@link #pollLast} and
 * {@link #removeLast} are also provided, to act on the <i>greatest</i> element in the queue
 * instead.
 *
 * <p>
 * A min-max priority queue can be configured with a maximum size. If so, each time the size of the
 * queue exceeds that value, the queue automatically removes its greatest element according to its
 * comparator (which might be the element that was just added). This is different from conventional
 * bounded queues, which either block or reject new elements when full.
 *
 * <p>
 * This implementation is based on the <a href="http://portal.acm.org/citation.cfm?id=6621">min-max
 * heap</a> developed by Atkinson, et al. Unlike many other double-ended priority queues, it stores
 * elements in a single array, as compact as the traditional heap data structure used in
 * {@link PriorityQueue}.
 *
 * <p>
 ...
 * significantly worse than a {@code PriorityQueue} with manual eviction above the maximum size. In
 * many cases {@link Ordering#leastOf} may work for your use case with significantly improved (and
 ...
 * {@link #poll} and {@link #remove()}) run in {@code
 *     O(log n) time}.
...
public final class MinMaxPriorityQueue<E> extends AbstractQueue<E> {
    ...
    /**
     * Creates a new min-max priority queue with default settings: natural order, no maximum size,
     * no initial contents, and an initial expected size of 11.
     */
     ...
     * MinMaxPriorityQueue} instances that use {@code comparator} to determine the least and
     * greatest elements.
     */
    ...
    /**
     * Creates and returns a new builder, configured to build {@code
     * MinMaxPriorityQueue} instances that are limited to {@code maximumSize} elements. Each time a
     * queue grows beyond this bound, it immediately removes its greatest element (according to its
     ...
     * directly, use {@link MinMaxPriorityQueue#orderedBy(Comparator)},
     ...
     *        example, a {@code Builder<Number>} can produce a {@code Queue<Number>} or
     *        {@code Queue<Integer>} but not a {@code
     *     Queue<Object>}).
     * @since 8.0
     */
    @Beta
    public static final class Builder<B> {
        /*
         * TODO(kevinb): when the dust settles, see if we still need this or can just default to
         * DEFAULT_CAPACITY.
         */
        ...
        /**
         * Configures this builder to build {@code MinMaxPriorityQueue} instances that are limited
         * to {@code maximumSize} elements. Each time a queue grows beyond this bound, it
         * immediately removes its greatest element (according to its comparator), which might be
        ...
        /**
         * Builds a new min-max priority queue using the previously specified options, and having
         * the given initial elements.
         */
        public <T extends B> MinMaxPriorityQueue<T> create(Iterable<? extends T> initialContents) {
            MinMaxPriorityQueue<T> queue =
                    new MinMaxPriorityQueue<T>(this, initialQueueSize(expectedSize, maximumSize, initialContents));
     ...
     * {@code element} the queue will automatically evict its greatest element (according to its
     * comparator), which may be {@code
     * element} itself.
     *
    ...
    /**
     * Retrieves, but does not remove, the least element of this queue, or returns {@code null} if
     * the queue is empty.
     */
    public E peekFirst() {
        return peek();
    ...
    /**
     * Retrieves, but does not remove, the greatest element of this queue, or returns {@code null}
     ...
     * list with one before {@code index}. Under these circumstances it returns a pair of elements
     * as a {@link MoveDesc}. The first one is the element that was previously at the end of the
     * heap and is now at some position before {@code index}. The second element is the one that was
     * swapped down to replace the element at {@code index}. This fact is used by iterator.remove so
     * as to visit elements during a traversal once and only once.
     */
    @VisibleForTesting
    @CanIgnoreReturnValue
    MoveDesc<E> removeAt(int index) {
        ...
        if (lastElementAt == index) {
            ...
            // 'lastElementAt'
            // is now at the end of queue. If that's the element we wanted to remove in the first
            // place,
     ...
     * max-heap. Conceptually, these might each have their own array for storage, but for
     * efficiency's sake they are stored interleaved on alternate heap levels in the same array
     * (MMPQ.queue).
     */
    @WeakOuter
    private class Heap {
        ...
        /**
         * Tries to move {@code toTrickle} from a min to a max level and bubble up there. If it
         * moved before {@code removeIndex} this method returns a pair as described in
         * {@link #removeAt}.
         */
        ...
        /**
         * Returns the index of minimum value between {@code index} and {@code index + len}, or
         * {@code -1} if {@code index} is greater than {@code size}.
         */
        ...
        /**
         * Moves an element one level up from a min level to a max level (or vice versa). Returns
         ...
         * becomes the new parent of the uncle. In that case, we first switch the last element with
         * its uncle, before returning.
         */
        ...
        /**
         * Crosses an element over to the opposite heap by moving it one level down (or up if there
        ...
        /**
         * Fills the hole at {@code index} by moving in the least of its grandchildren to this
         * position, then recursively filling the new hole created.
         *
    ...
    private class QueueIterator implements Iterator<E> {
        ...
        // The same element is not allowed in both forgetMeNot and skipMe, but duplicates are
        // allowed in
     ...
     *
     * <p>
     * The iterator is <i>fail-fast</i>: If the MinMaxPriorityQueue is modified at any time after
    ...
    /**
     * Returns the comparator used to order the elements in this queue. Obeys the general contract
     * of {@link PriorityQueue#comparator}, but returns {@link Ordering#natural} instead of
     * {@code null} to indicate natural ordering.
     */
    ...
    static int initialQueueSize(int configuredExpectedSize, int maximumSize, Iterable<?> initialContents) {
        ...
        int result =
                (configuredExpectedSize == Builder.UNSET_EXPECTED_SIZE) ? DEFAULT_CAPACITY : configuredExpectedSize;
