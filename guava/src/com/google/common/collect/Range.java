 ...
 * if at all possible. Otherwise, be aware that concepts used throughout this documentation such as
 * "equal", "same", "unique" and so on actually refer to whether {@link Comparable#compareTo
 ...
 * undefined horrible things to happen in {@code Range}. For now, the Range API does not prevent its
 * use, because this would also rule out all ungenerified (pre-JDK1.5) data types. <b>This may
 ...
 * also be contained. More formally, for any {@code c1 <= c2 <= c3} of type {@code C},
 * {@code r.contains(c1) && r.contains(c3)} implies {@code r.contains(c2)}). This means that a
 * {@code Range<Integer>} can never be used to represent, say, "all <i>prime</i> numbers from 1 to
 * 100."
...
public final class Range<C extends Comparable> extends RangeGwtSerializationDependencies
     ...
     *
     * <p>
     * Note that certain discrete ranges such as the integer range {@code (3..4)} are <b>not</b>
    ...
    /**
     * Returns {@code true} if every element in {@code values} is {@linkplain #contains contained}
     * in this range.
     */
     ...
     * {@linkplain #intersection intersection} (as a single, possibly-empty range) if and only if
    ...
    /**
     * Returns {@code true} if {@code object} is a range having the same endpoints and bound types
     * as this range. Note that discrete ranges such as {@code (1..4)} and {@code [2..3]} are
     * <b>not</b> equal to one another, despite the fact that they each contain precisely the same
     * set of values. Similarly, empty ranges are not equal unless they have exactly the same
     * representation, so {@code [3..3)}, {@code (3..3]}, {@code (4..4]} are all unequal.
     */
    @Override
