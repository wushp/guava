 ...
 *
 * <p>
 * In contrast to {@link RangeSet}, no "coalescing" is done of {@linkplain Range#isConnected(Range)
 * connected} ranges, even if they are mapped to the same value.
 *
...
public interface RangeMap<K extends Comparable, V> {
     ...
     *
     * <p>
     * Specifically, if any range in this range map contains the specified key, the value associated
     * with that range is returned.
     */
    @Nullable
    V get(K key);
     ...
     *
     * <p>
     * Specifically, after a call to {@code put(range, value)}, if {@link Range#contains(Comparable)
     * range.contains(k)}, then {@link #get(Comparable) get(k)} will return {@code value}.
     *
     * <p>
    ...
    /**
     * Maps a range to a specified value, coalescing this range with any existing ranges with the
     * same value that are {@linkplain Range#isConnected connected} to this range.
     *
     * <p>
     * The behavior of {@link #get(Comparable) get(k)} after calling this method is identical to the
     * behavior described in {@link #put(Range, Object) put(range, value)}, however the ranges
     ...
     *
     * <p>
     * <b>Note:</b> coalescing requires calling {@code .equals()} on any connected values, which may
     * be expensive depending on the value type. Using this method on range maps with large values
     ...
     * {@code rangeMap.subRangeMap(Range.open(3, 12))} would return a range map with the entries
     ...
     *
     * <p>
     * The returned range map will throw an {@link IllegalArgumentException} on an attempt to insert
     * a range not {@linkplain Range#encloses(Range) enclosed} by {@code range}.
     */
