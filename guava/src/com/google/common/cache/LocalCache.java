...
class LocalCache<K, V> extends AbstractMap<K, V> implements ConcurrentMap<K, V> {
     ...
     * constructors with arguments. MUST be a power of two {@code <= 1<<30} to ensure that entries
     * are indexable using ints.
     */
    ...
    /**
     * A listener that is invoked when an entry is removed due to expiration or garbage collection
     * of soft/weak entries.
     */
    ...
    LocalCache(CacheBuilder<? super K, ? super V> builder, @Nullable CacheLoader<? super K, V> loader) {
        ...
        removalNotificationQueue =
                (removalListener == NullListener.INSTANCE) ? LocalCache.<RemovalNotification<K, V>>discardingQueue()
        ...
        // entries. The special casing for size-based eviction is only necessary because that
        // eviction
        // happens per segment instead of globally, so too many segments compared to the maximum
        // size
    ...
    interface ValueReference<K, V> {
        ...
        /**
         * Returns true if a new value is currently loading, regardless of whether or not there is
         * an existing value. It is assumed that the return value of this method is constant for any
         * given ValueReference instance.
         */
        boolean isLoading();

        /**
         * Returns true if this reference contains an active value, meaning one that is still
         * considered present in the cache. Active values consist of live values, which are returned
         * by cache lookups, and dead values, which have been evicted but awaiting removal.
         * Non-active values consist strictly of loading values, though during refresh a value may
    ...
    interface ReferenceEntry<K, V> {
        ...
        /*
         * Used by entries that use access order. Access entries are maintained in a doubly-linked
         * list. New entries are added at the tail of the list at write time; stale entries are
    ...
    /*
     * Note: All of this duplicate code sucks, but it saves a lot of memory. If only Java had
     * mixins! To maintain this code, make a change for the strong reference type. Then, cut and
     * paste, and replace "Strong" with "Soft" or "Weak" within the pasted text. The primary
     * difference is that strong entries store the key reference directly while soft and weak
     * entries delegate to their respective superclasses.
     */
    ...
    static class WeakEntry<K, V> extends WeakReference<K> implements ReferenceEntry<K, V> {
        ...
        /*
         * It'd be nice to get these for free from AbstractReferenceEntry, but we're already
         * extending WeakReference<K>.
         */
    ...
    static class Segment<K, V> extends ReentrantLock {
        ...
        /*
         * TODO(fry): Consider copying variables (like evictsBySize) from outer class into this
         * class. It will require more memory but will reduce indirection.
         */
         ...
         * segments loading size or checking containsValue, then we might have an inconsistent view
        ...
        /**
         * The key reference queue contains entries whose keys have been garbage collected, and
         * which need to be cleaned up internally.
         */
        final ReferenceQueue<K> keyReferenceQueue;

        /**
         * The value reference queue contains value references whose values have been garbage
        ...
        /**
         * A queue of elements currently in the map, ordered by write time. Elements are added to
         * the tail of the queue on write.
         */
        @GuardedBy("this")
        final Queue<ReferenceEntry<K, V>> writeQueue;

        /**
         * A queue of elements currently in the map, ordered by access time. Elements are added to
         * the tail of the queue on access (note that writes count as accesses).
         */
        @GuardedBy("this")
        ...
        Segment(LocalCache<K, V> map, int initialCapacity, long maxSegmentWeight, StatsCounter statsCounter) {
            ...
            writeQueue =
                    map.usesWriteQueue() ? new WriteQueue<K, V>() : LocalCache.<ReferenceEntry<K, V>>discardingQueue();
        ...
        /**
         * Copies {@code original} into a new entry chained to {@code newNext}. Returns the new
         * entry, or {@code null} if {@code original} was already garbage collected.
         */
        @GuardedBy("this")
        ReferenceEntry<K, V> copyEntry(ReferenceEntry<K, V> original, ReferenceEntry<K, V> newNext) {
        ...
        V compute(K key, int hash, BiFunction<? super K, ? super V, ? extends V> function) {
            ...
            try {
                ...
                // note valueReference can be an existing value or even itself another loading value
                // if
        ...
        /**
         * Refreshes the value associated with {@code key}, unless another thread is already doing
         * so. Returns the newly refreshed value associated with {@code key} if it was refreshed
         * inline, or {@code null} if another thread is performing the refresh or if an error occurs
         * during refresh.
         */
        @Nullable
        ...
        /**
         * Returns a newly inserted {@code LoadingValueReference}, or null if the live value
         * reference is already loading.
         */
        @Nullable
                ...
                for (e = first; e != null; e = e.getNext()) {
                    ...
                    if (e.getHash() == hash && entryKey != null && map.keyEquivalence.equivalent(key, entryKey)) {
                        ...
                        if (valueReference.isLoading() || (checkTime && (now - e.getWriteTime() < map.refreshNanos))) {
                            ...
                            // if checkTime, we want to check *after* acquiring the lock if refresh
                            // still needs
                            // to be scheduled
                            return null;
                        }
        ...
        /**
         * Records the relative order in which this read was performed by adding {@code entry} to
         * the recency queue. At write-time, or when the queue is full past the threshold, the queue
         * will be drained and the entries therein processed.
         *
         * <p>
        ...
        /**
         * Drains the recency queue, updating eviction metadata that the entries therein were read
         * in the specified relative order. This currently amounts to adding them to relevant
         * eviction lists (accounting for the fact that they could have been removed from the map
         * since being added to the recency queue).
         */
        @GuardedBy("this")
        ...
        /**
         * Performs eviction if the segment is over capacity. Avoids flushing the entire cache if
         * the newest entry exceeds the maximum weight all on its own.
         *
        ...
        /**
         * Performs routine cleanup following a read. Normally cleanup happens during writes. If
         * cleanup is not observed after a sufficient number of reads, try cleaning up from the read
         * thread.
         */
     ...
     *
     * <p>
     * Note that this entire implementation makes the assumption that all elements which are in the
     * map are also in this queue, and that all elements not in the queue are not in the map.
     *
     * <p>
     * The benefits of creating our own queue are that (1) we can replace elements in the middle of
     ...
     * writeReplace of LocalLoadingCache.
     *
     * Unfortunately, readResolve() doesn't get called when a circular dependency is present, so the
     * proxy must be able to behave as the cache itself.
     */
