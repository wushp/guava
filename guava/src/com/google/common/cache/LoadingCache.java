...
public interface LoadingCache<K, V> extends Cache<K, V>, Function<K, V> {
     ...
     * {@code Cache.asMap().putIfAbsent} after loading has completed; if another value was
     * associated with {@code key} while the new value was loading then a removal notification will
     * be sent for the new value.
     *
     * <p>
     * If the cache loader associated with this cache is known not to throw checked exceptions, then
     * prefer {@link #getUnchecked} over this method.
     *
     ...
     * Returns the value associated with {@code key} in this cache, first loading that value if
     * necessary. No observable state associated with this cache is modified until loading
     * completes. Unlike {@link #get}, this method does not throw a checked exception, and thus
     * should only be used in situations where checked exceptions are not thrown by the cache
     * loader.
     *
     * <p>
     * If another call to {@link #get} or {@link #getUnchecked} is currently loading the value for
     ...
     *
     * <p>
     * <b>Warning:</b> this method silently converts checked exceptions to unchecked exceptions, and
     * should not be used with cache loaders which throw checked exceptions. In such cases use
     * {@link #get} instead.
     *
     ...
     * returns {@code null}, returns a map containing null keys or values, or fails to return an
     * entry for each requested key.
     *
     * <p>
     ...
     * @throws UncheckedExecutionException if an exception was thrown while loading the value. (As
     *         described in the documentation for {@link #getUnchecked}, {@code LoadingCache} should
     *         be used as a {@code Function} only with cache loaders that throw only unchecked
     *         exceptions.)
     */
    @Deprecated
    @Override
    ...
    /**
     * Loads a new value for key {@code key}, possibly asynchronously. While the new value is
     * loading the previous value (if any) will continue to be returned by {@code get(key)} unless
     * it is evicted. If the new value is loaded successfully it will replace the previous value in
     * the cache; if an exception is thrown while refreshing the previous value will remain, <i>and
     * the exception will be logged (using {@link java.util.logging.Logger}) and swallowed</i>.
     *
     * <p>
     * Caches loaded by a {@link CacheLoader} will call {@link CacheLoader#reload} if the cache
     * currently contains a value for {@code key}, and {@link CacheLoader#load} otherwise. Loading
     * is asynchronous only if {@link CacheLoader#reload} was overridden with an asynchronous
     * implementation.
     *
     * <p>
     ...
     *
     * <p>
     * <b>Note that although the view <i>is</i> modifiable, no method on the returned map will ever
