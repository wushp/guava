...
public abstract class AbstractCache<K, V> implements Cache<K, V> {
     ...
     * structure, and is thus forced to return the query keys instead of the cached keys. This is
     * only possible with an unsafe cast which requires {@code keys} to actually be of type
     * {@code K}.
     *
     * {@inheritDoc}
     *
    ...
    public interface StatsCounter {
         ...
         * threads blocking on the load. Multiple concurrent calls to {@link Cache} lookup methods
         * with the same key on an absent value should result in a single call to either
         * {@code recordLoadSuccess} or {@code recordLoadException} and multiple calls to this
        ...
        /**
         * Records the eviction of an entry from the cache. This should only been called when an
         * entry is evicted due to the cache's eviction strategy, and not as a result of manual
         * {@linkplain Cache#invalidate invalidations}.
         */
        void recordEviction();

        /**
         * Returns a snapshot of this counter's values. Note that this may be an inconsistent view,
