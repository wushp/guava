 ...
 *             CacheBuilder.newBuilder().maximumSize(10000).expireAfterWrite(10, TimeUnit.MINUTES)
 ...
 *
 * <p>
 * <b>Note:</b> by default, the returned cache uses equality comparisons (the {@link Object#equals
 * equals} method) to determine equality for keys or values. However, if {@link #weakKeys} was
 * specified, the cache uses identity ({@code ==}) comparisons instead for keys. Likewise, if
 * {@link #weakValues} or {@link #softValues} was specified, the cache uses identity comparisons for
 * values.
 *
 * <p>
 * Entries are automatically evicted from the cache when any of {@linkplain #maximumSize(long)
 ...
 *
 * <p>
 * Certain cache configurations will result in the accrual of periodic maintenance tasks which will
 * be performed during write operations, or during occasional read operations in the absence of
 ...
 *
 * <p>
 * The caches produced by {@code CacheBuilder} are serializable, and the deserialized caches retain
 * all the configuration properties of the original cache. Note that the serialized form does
...
public final class CacheBuilder<K, V> {
     ...
     * created, each having a hash table of size eight. Providing a large enough estimate at
     ...
     * The table is internally partitioned to try to permit the indicated number of concurrent
     * updates without contention. Because assignment of entries to these partitions is not
     * necessarily uniform, the actual concurrency observed may vary. Ideally, you should choose a
     * value to accommodate as many threads as will ever concurrently modify the table. Using a
     * significantly higher value than you need can waste space and time, and a significantly lower
     * value can lead to thread contention. But overestimates and underestimates within an order of
     * magnitude do not usually have much noticeable impact. A value of one permits only one thread
     * to modify the cache at a time, but since read operations and cache loading computations can
     * proceed concurrently, this still yields higher concurrency than full synchronization.
     *
     * <p>
     * Defaults to 4. <b>Note:</b>The default may change in the future. If you care about this
     ...
     * explicit write, and twice for each cache loading computation (once prior to loading the new
     * value, and once after loading completes). Much internal cache management is performed at the
     * segment granularity. For example, access queues and write queues are kept per segment when
     * they are required by the selected eviction algorithm. As such, when writing unit tests it is
     * not uncommon to specify {@code concurrencyLevel(1)} in order to achieve more deterministic
     * eviction behavior.
     *
     * <p>
    ...
    /**
     * Specifies the maximum number of entries the cache may contain. Note that the cache <b>may
     * evict an entry before this limit is exceeded</b>. As the cache size grows close to the
     * maximum, the cache evicts entries that are less likely to be used again. For example, the
     ...
     *
     * <p>
     * Note that the cache <b>may evict an entry before this limit is exceeded</b>. As the cache
     ...
     * consideration by {@link #maximumWeight(long)} when determining which entries to evict, and
     ...
     *
     * <p>
     * <b>Important note:</b> Instead of returning <em>this</em> as a {@code CacheBuilder} instance,
     * this method returns {@code CacheBuilder<K1, V1>}. From this point on, either the original
     * reference or the returned reference may be used to complete configuration and build the
     * cache, but only the "generic" one is type-safe. That is, it will properly prevent you from
     ...
     *
     * <p>
     * <b>Warning:</b> if you ignore the above advice, and use this {@code CacheBuilder} to build a
    ...
    /**
     * Specifies that each value (not key) stored in the cache should be wrapped in a
     * {@link SoftReference} (by default, strong references are used). Softly-referenced objects
     * will be garbage-collected in a <i>globally</i> least-recently-used manner, in response to
     * memory demand.
     *
     * <p>
     * <b>Warning:</b> in most circumstances it is better to set a per-cache
     * {@linkplain #maximumSize(long) maximum size} instead of using soft references. You should
     * only use this method if you are well familiar with the practical consequences of soft
     * references.
     *
     * <p>
     * <b>Note:</b> when this method is used, the resulting cache will use identity ({@code ==})
     ...
     * its last access. Access time is reset by all cache read and write operations (including
     ...
     * elapsed after the entry's creation, or the most recent replacement of its value. The
     * semantics of refreshes are specified in {@link LoadingCache#refresh}, and are performed by
     * calling {@link CacheLoader#reload}.
     *
     * <p>
     ...
     *
     * <p>
     * The primary intent of this method is to facilitate testing of caches with a fake or mock time
     * source.
     *
     * @return this {@code CacheBuilder} instance (for chaining)
     ...
     * {@linkplain RemovalCause reason}. Each cache created by this builder will invoke this
     * listener as part of the routine maintenance described in the class documentation above.
     *
     * <p>
     * <b>Warning:</b> after invoking this method, do not continue to use <i>this</i> cache builder
     * reference; instead use the reference this method <i>returns</i>. At runtime, these point to
     * the same instance, but only the returned reference has the correct generic type information
     * so as to ensure type safety. For best results, use the standard method-chaining idiom
     * illustrated in the class documentation above, configuring a builder and building your cache
     * in a single statement. Failure to heed this advice can result in a {@link ClassCastException}
     * being thrown by a cache operation at some <i>undefined</i> point in the future.
     *
     * <p>
     ...
     * computes or retrieves it using the supplied {@code CacheLoader}. If another thread is
     * currently loading the value for this key, simply waits for that thread to finish and returns
     * its loaded value. Note that multiple threads can concurrently load values for distinct keys.
     *
     * <p>
    ...
    /**
     * Returns a string representation for this CacheBuilder instance. The exact form of the
