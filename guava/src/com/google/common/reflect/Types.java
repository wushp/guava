...
final class Types {
    ...
    private static <D extends GenericDeclaration> TypeVariable<D> newTypeVariableImpl(D genericDeclaration, String name,
        ...
        TypeVariable<D> typeVariable =
                Reflection.newProxy(TypeVariable.class, new TypeVariableInvocationHandler(typeVariableImpl));
        return typeVariable;
    }
     ...
     *
     * <p>
     * Java 8 introduced a new method {@code getAnnotatedBounds()} in the {@link TypeVariable}
     ...
     * method on its instance of {@code TypeVariableImpl}. Otherwise it throws
     * {@link UnsupportedOperationException}; this should only apply to
     * {@code getAnnotatedBounds()}. This does mean that users on Java 8 who obtain an instance of
     * {@code TypeVariable} from {@link TypeResolver#resolveType} will not be able to call
            ...
            for (Method method : TypeVariableImpl.class.getMethods()) {
                if (method.getDeclaringClass().equals(TypeVariableImpl.class)) {
                    try {
                        ...
                        // OK: the method is accessible to us anyway. The setAccessible call is only
                        // for
     ...
     * implementation respects symmetry. Moreover, we don't want to reconstruct a native type
     * variable {@code <A>} using our implementation unless some of its bounds have changed in
     * resolution. This avoids creating unequal TypeVariable implementation unnecessarily. When the
     * bounds do change, however, it's fine for the synthetic TypeVariable to be unequal to any
     * native TypeVariable anyway.
     */
