...
public final class TypeResolver {
     ...
     *
     * <p>
     * For example, if {@code formal} is a {@code TypeVariable T}, and {@code actual} is {@code
     ...
     *        always a bug if {@code formal} isn't a type variable and contains no type variable.
     ...
     *        yet other type variables, in which case these type variables will be further resolved
     *        if corresponding mappings exist in the current {@code TypeResolver} instance.
     */
    ...
    private static void populateTypeMappings(final Map<TypeVariableKey, Type> mappings, Type from, final Type to) {
        ...
        new TypeVisitor() {
            ...
            void visitParameterizedType(ParameterizedType fromParameterizedType) {
                ...
                checkArgument(fromArgs.length == toArgs.length, "%s not compatible with %s", fromParameterizedType,
                        toParameterizedType);
            ...
            void visitClass(Class<?> fromClass) {
                ...
                // And we don't support "assuming String is T"; user has to say "assuming T is
                // String".
    ...
    private static class TypeTable {
         ...
         * non-reified type or has bounds, {@code forDependants} is used to do further resolution,
         * which doesn't try to resolve any type variable on generic declarations that are already
         * being resolved.
         *
         * <p>
        ...
        Type resolveInternal(TypeVariable<?> var, TypeTable forDependants) {
            ...
            if (type == null) {
                ...
                /*
                 * We'd like to simply create our own TypeVariable with the newly resolved bounds.
                 * There's just one problem: Starting with JDK 7u51, the JDK TypeVariable's equals()
                 * method doesn't recognize instances of our TypeVariable implementation. This is a
                 * problem because users compare TypeVariables from the JDK against TypeVariables
                 * returned by TypeResolver. To work with all JDK versions, TypeResolver must return
                 * the appropriate TypeVariable implementation in each of the three possible cases:
                 *
                 * 1. Prior to JDK 7u51, the JDK TypeVariable implementation interoperates with
                 * ours. Therefore, we can always create our own TypeVariable.
                 *
                 * 2. Starting with JDK 7u51, the JDK TypeVariable implementations does not
                 * interoperate with ours. Therefore, we have to be careful about whether we create
                 * our own TypeVariable:
                 *
                 * 2a. If the resolved types are identical to the original types, then we can return
                 * the original, identical JDK TypeVariable. By doing so, we sidestep the problem
                 * entirely.
                 *
                 * 2b. If the resolved types are different from the original types, things are
                 * trickier. The only way to get a TypeVariable instance for the resolved types is
                 * to create our own. The created TypeVariable will not interoperate with any JDK
                 * TypeVariable. But this is OK: We don't _want_ our new TypeVariable to be equal to
                 * the JDK TypeVariable because it has _different bounds_ than the JDK TypeVariable.
                 * And it wouldn't make sense for our new TypeVariable to be equal to any _other_
                 * JDK TypeVariable, either, because any other JDK TypeVariable must have a
                 * different declaration or name. The only TypeVariable that our new TypeVariable
                 * _will_ be equal to is an equivalent TypeVariable that was also created by us. And
                 * that equality is guaranteed to hold because it doesn't involve the JDK
    ...
    private static final class WildcardCapturer {
        ...
        Type capture(Type type) {
            ...
            if (type instanceof ParameterizedType) {
                ...
                return Types.newParameterizedTypeWithOwner(captureNullable(parameterizedType.getOwnerType()),
                        (Class<?>) parameterizedType.getRawType(), capture(parameterizedType.getActualTypeArguments()));
            }
    ...
    /**
     * Wraps around {@code TypeVariable<?>} to ensure that any two type variables are equal as long
     * as they are declared by the same {@link java.lang.reflect.GenericDeclaration} and have the
     * same name, even if their bounds differ.
     *
     * <p>
     * While resolving a type variable from a {@code var -> type} map, we don't care whether the
     * type variable's bound has been partially resolved. As long as the type variable "identity"
     * matches.
     *
     * <p>
     * On the other hand, if for example we are resolving {@code List<A extends B>} to {@code
     * List<A extends String>}, we need to compare that {@code <A extends B>} is unequal to
     * {@code <A
