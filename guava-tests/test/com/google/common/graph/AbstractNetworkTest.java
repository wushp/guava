...
public abstract class AbstractNetworkTest {
     ...
     *
     * <p>
     * This method should be used in tests of specific implementations if you want to ensure uniform
     * behavior (including side effects) with how edges are added elsewhere in the tests. For
     * example, the existing implementations of this method explicitly add the supplied nodes to the
     * graph, and then call {@code graph.addEdge()} to connect the edge to the nodes; this is not
     * part of the contract of {@code graph.addEdge()} and is done for convenience. In cases where
     * you want to avoid such side effects (e.g., if you're testing what happens in your
     * implementation if you add an edge whose end-points don't already exist in the graph), you
     * should <b>not</b> use this method.
     *
     * @return {@code true} iff the graph was modified as a result of this call
     */
    @CanIgnoreReturnValue
    ...
    static <N, E> void validateNetwork(Network<N, E> network) {
        ...
        for (N node : sanityCheckSet(network.nodes())) {
            ...
            for (E incidentEdge : sanityCheckSet(network.incidentEdges(node))) {
                assertTrue(
                        network.inEdges(node).contains(incidentEdge) || network.outEdges(node).contains(incidentEdge));
    ...
    /**
     * Verifies that the {@code Set} returned by {@code inEdges} has the expected mutability
     * property (see the {@code Network} documentation for more information).
     */
    @Test
    ...
    /**
     * Verifies that the {@code Set} returned by {@code outEdges} has the expected mutability
     * property (see the {@code Network} documentation for more information).
     */
    @Test
    ...
    public void removeEdge_queryAfterRemoval() {
        addEdge(N1, N2, E12);
        @SuppressWarnings("unused")
        EndpointPair<Integer> unused = network.incidentNodes(E12); // ensure cache (if any) is
                                                                   // populated
        assertTrue(network.removeEdge(E12));
        try {
