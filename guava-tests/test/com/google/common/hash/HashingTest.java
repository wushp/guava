 ...
 *
 * <p>
 * TODO(b/33919189): Migrate repeated testing methods to {@link #HashTestUtils} and tweak unit tests
...
public class HashingTest extends TestCase {
    ...
    public void testCombineOrdered_differentBitLengths() {
        try {
            HashCode unused = Hashing.combineOrdered(ImmutableList.of(HashCode.fromInt(32), HashCode.fromLong(32L)));
            fail();
        } catch (IllegalArgumentException expected) {
        }
    }
    ...
    public void testCombineOrdered() {
        ...
        assertFalse(Hashing.combineOrdered(ImmutableList.of(hash31, hash32))
    ...
    public void testCombineUnordered_differentBitLengths() {
        try {
            HashCode unused = Hashing.combineUnordered(ImmutableList.of(HashCode.fromInt(32), HashCode.fromLong(32L)));
            fail();
        } catch (IllegalArgumentException expected) {
        }
    }
    ...
    public void testCombineUnordered() {
        ...
        assertEquals(Hashing.combineUnordered(ImmutableList.of(hash31, hash32)),
    ...
    public void testConcatenating_equals() {
        new EqualsTester().addEqualityGroup(Hashing.concatenating(asList(Hashing.md5())))
                .addEqualityGroup(Hashing.concatenating(asList(Hashing.murmur3_32())))
                .addEqualityGroup(Hashing.concatenating(Hashing.md5(), Hashing.md5()),
                        Hashing.concatenating(asList(Hashing.md5(), Hashing.md5())))
                .addEqualityGroup(Hashing.concatenating(Hashing.murmur3_32(), Hashing.md5()),
                        Hashing.concatenating(asList(Hashing.murmur3_32(), Hashing.md5())))
                .addEqualityGroup(Hashing.concatenating(Hashing.md5(), Hashing.murmur3_32()),
                        Hashing.concatenating(asList(Hashing.md5(), Hashing.murmur3_32())))
                .testEquals();
    }

    public void testConcatenatingIterable_bits() {
        assertEquals(Hashing.md5().bits() + Hashing.md5().bits(),
        ...
        assertEquals(Hashing.md5().bits() + Hashing.murmur3_32().bits() + Hashing.murmur3_128().bits(),
                Hashing.concatenating(asList(Hashing.md5(), Hashing.murmur3_32(), Hashing.murmur3_128())).bits());
    }
    ...
    public void testHashIntReverseBytesVsHashBytesIntsToByteArray() {
        int input = 42;
        assertEquals(Hashing.md5().hashBytes(Ints.toByteArray(input)),
    ...
    private static final ImmutableTable<HashFunction, String, String> KNOWN_HASHES = ImmutableTable
            .<HashFunction, String, String>builder().put(Hashing.adler32(), EMPTY_STRING, "01000000")
            .put(Hashing.adler32(), TQBFJOTLD, "da0fdc5b").put(Hashing.adler32(), TQBFJOTLDP, "0810e46b")
            ...
            .put(Hashing.murmur3_32(), EMPTY_STRING, "00000000").put(Hashing.murmur3_32(), TQBFJOTLD, "23f74f2e")
            ...
            .put(Hashing.sha256(), TQBFJOTLDP, "ef537f25c895bfa782526529a9b63d97aa631564d5d789c2b765448c8635fb6c")
            ...
            .put(Hashing.crc32(), EMPTY_STRING, "00000000").put(Hashing.crc32(), TQBFJOTLD, "39a34f41")
            .put(Hashing.crc32(), TQBFJOTLDP, "e9259051").put(Hashing.sipHash24(), EMPTY_STRING, "310e0edd47db6f72")
            .put(Hashing.sipHash24(), TQBFJOTLD, "e46f1fdc05612752")
            .put(Hashing.sipHash24(), TQBFJOTLDP, "9b602581fce4d4f8").put(Hashing.crc32c(), EMPTY_STRING, "00000000")
            .put(Hashing.crc32c(), TQBFJOTLD, "04046222").put(Hashing.crc32c(), TQBFJOTLDP, "b3970019")
            ...
            .put(Hashing.farmHashFingerprint64(), TQBFJOTLDP, "737d7e5f8660653e").build();
    ...
    public void testAllHashFunctionsHaveKnownHashes() throws Exception {
        ...
        for (Method method : Hashing.class.getDeclaredMethods()) {
            if (method.getReturnType().equals(HashFunction.class) // must return HashFunction
                    ...
                    && method.getParameterTypes().length == 0 // only the seed-less grapes^W hash
                                                              // functions
    ...
    public void testKnownUtf8Hashing() {
        for (Cell<HashFunction, String, String> cell : KNOWN_HASHES.cellSet()) {
            ...
            assertEquals(String.format(Locale.ROOT, "Known hash for hash(%s, UTF_8) failed", input), expected,
                    func.hashString(input, UTF_8).toString());
        }
    }

    public void testNullPointers() {
        NullPointerTester tester = new NullPointerTester().setDefault(byte[].class, "secret key".getBytes(UTF_8))
     ...
     * {@link #testSeededHashFunctionEquals} because the parameter to {@code goodFastHash} is a
     * size, not a seed, and because that size is rounded up. Thus, {@code goodFastHash} instances
     * with different parameters can be equal. That fact is a problem for {@code
    ...
    public void testGoodFastHashEquals() throws Exception {
        ...
        new EqualsTester().addEqualityGroup(hashFunction1a, hashFunction1b)
                .addEqualityGroup(hashFunction2a, hashFunction2b).addEqualityGroup(hashFunction3a, hashFunction3b)
                .addEqualityGroup(hashFunction4a, hashFunction4b).testEquals();
    ...
    static void assertSeedlessHashFunctionEquals(Class<?> clazz) throws Exception {
        for (Method method : clazz.getDeclaredMethods()) {
            if (method.getReturnType().equals(HashFunction.class) // must return HashFunction
                    && Modifier.isPublic(method.getModifiers()) // only the public methods
                    && method.getParameterTypes().length == 0) { // only the seed-less hash
                                                                 // functions
    ...
    static void assertSeededHashFunctionEquals(Class<?> clazz) throws Exception {
        ...
        for (Method method : clazz.getDeclaredMethods()) {
            if (method.getReturnType().equals(HashFunction.class) // must return HashFunction
                    ...
                    && !method.getName().equals("concatenating") // don't test
                ...
                new EqualsTester().addEqualityGroup(hashFunction1a, hashFunction1b).addEqualityGroup(hashFunction2)
                        .testEquals();
