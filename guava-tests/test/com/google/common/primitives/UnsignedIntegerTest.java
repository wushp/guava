...
public class UnsignedIntegerTest extends TestCase {
    ...
    static {
        ...
        for (int i = -3; i <= 3; i++) {
            testIntsBuilder.add(i).add(force32(Integer.MIN_VALUE + i)).add(force32(Integer.MAX_VALUE + i));
            testLongsBuilder.add((long) i).add((long) Integer.MIN_VALUE + i).add((long) Integer.MAX_VALUE + i)
                    .add((1L << 32) + i);
        }
    ...
    public void testFromIntBitsAndIntValueAreInverses() {
        for (int value : TEST_INTS) {
            assertEquals(UnsignedInts.toString(value), value, UnsignedInteger.fromIntBits(value).intValue());
        }
    }
