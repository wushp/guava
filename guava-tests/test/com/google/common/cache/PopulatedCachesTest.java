...
public class PopulatedCachesTest extends TestCase {
    ...
    public void testKeySet_populated() {
        for (LoadingCache<Object, Object> cache : caches()) {
            ...
            new EqualsTester().addEqualityGroup(cache.asMap().keySet(), keys).addEqualityGroup(ImmutableSet.of())
                    .testEquals();
    ...
    public void testEntrySet_populated() {
        for (LoadingCache<Object, Object> cache : caches()) {
            ...
            new EqualsTester().addEqualityGroup(cache.asMap().entrySet(), entries).addEqualityGroup(ImmutableSet.of())
                    .testEquals();
    ...
    private CacheBuilderFactory cacheFactory() {
        ...
        // WARMUP_MAX), but the tests assume no values get evicted. Even with a maximumSize of 100,
        // one
        // of the values gets evicted. With weak keys, we use identity equality, which means using
        // System.identityHashCode, which means the assignment of keys to segments is
        // nondeterministic,
        // so more than (maximumSize / #segments) keys could get assigned to the same segment, which
        // would cause one to be evicted.
        return new CacheBuilderFactory().withKeyStrengths(ImmutableSet.of(Strength.STRONG, Strength.WEAK))
                .withValueStrengths(ImmutableSet.copyOf(Strength.values()))
                .withConcurrencyLevels(ImmutableSet.of(1, 4, 16, 64)).withMaximumSizes(ImmutableSet.of(400, 1000))
                .withInitialCapacities(ImmutableSet.of(0, 1, 10, 100, 1000)).withExpireAfterWrites(ImmutableSet.of(
