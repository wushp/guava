...
class CacheTesting {
    ...
    /**
     * Assuming the given cache has maximum size {@code maxSize}, this method populates the cache
     * (by getting a bunch of different keys), then makes sure all the items in the cache are also
     * in the eviction queue. It will invoke the given {@code operation} on the first element in the
    ...
    static void checkEmpty(Collection<?> collection) {
        ...
        if (collection instanceof Set) {
            new EqualsTester().addEqualityGroup(ImmutableSet.of(), collection).addEqualityGroup(ImmutableSet.of(""))
                    .testEquals();
        } else if (collection instanceof List) {
            new EqualsTester().addEqualityGroup(ImmutableList.of(), collection).addEqualityGroup(ImmutableList.of(""))
                    .testEquals();
        }
    }
}
