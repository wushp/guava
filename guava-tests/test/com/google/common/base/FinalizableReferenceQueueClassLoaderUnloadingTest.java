...
public class FinalizableReferenceQueueClassLoaderUnloadingTest extends TestCase {
    ...
    private WeakReference<ClassLoader> useFrqInSeparateLoader() throws Exception {
        ...
        // sepLoader is the loader that we will use to load the parallel FinalizableReferenceQueue
        // (FRQ)
        ...
        // rather than delegating to a parent ClassLoader. If this assumption is violated the test
        // will
        ...
        // FRQ tries to load the Finalizer class (for the reference-collecting thread) in a few
        // ways.
        // If the class is accessible to the system ClassLoader (ClassLoader.getSystemClassLoader())
        // then FRQ does not bother to load Finalizer.class through a separate ClassLoader. That
        // happens
        // in our test environment, which foils the purpose of this test, so we disable the logic
        // for
        // our test by setting a static field. We are changing the field in the parallel version of
        // FRQ
        ...
        // Now make a parallel FRQ and an associated FinalizableWeakReference to an object, in order
        // to
    ...
    // happens. But the thread's supposedly separate ClassLoader actually has a reference to the
    // app's
    ...
    // gc'd even if there is a still a FinalizableReferenceQueue in a static field. (Setting the
    // field
