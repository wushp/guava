...
public class SplitterTest extends TestCase {
     ...
     *
     * <p>
     * TODO(user): It would be good to make all the relevant tests run on both split and
    ...
    public void testCharacterSplitWithTrim() {
        ...
        Iterable<String> family =
                COMMA_SPLITTER.trimResults(CharMatcher.anyOf("afro").or(CharMatcher.whitespace())).split(jacksons);
    ...
    public void testStringSplitWithTrim() {
        String jacksons = "arfo(Marlon)aorf, (Michael)orfa, afro(Jackie)orfa, " + "ofar(Jemaine), aff(Tito)";
        Iterable<String> family =
                Splitter.on(',').trimResults(CharMatcher.anyOf("afro").or(CharMatcher.whitespace())).split(jacksons);
        assertThat(family).containsExactly("(Marlon)", "(Michael)", "(Jackie)", "(Jemaine)", "(Tito)").inOrder();
    }
    ...
    /**
     * This test really pushes the boundaries of what we support. In general the splitter's
     * behaviour is not well defined if the char sequence it's splitting is mutated during
     * iteration.
     */
