...
public class EventBusTest extends TestCase {
    ...
    /**
     * Tests that events are distributed to any subscribers to their type or any supertype,
    ...
    public void testRegisterThreadSafety() throws Exception {
        ...
        for (StringCatcher catcher : catchers) {
            assertEquals("One of the registered catchers did not receive an event.", expectedEvents,
                    catcher.getEvents());
        }
    }

    /**
     * Tests that bridge methods are not subscribed to events. In Java 8, annotations are included
     * on the bridge method in addition to the original method, which causes both the original and
     * bridge methods to be subscribed (since both are annotated @Subscribe) without specifically
     * checking for bridge methods.
     */
