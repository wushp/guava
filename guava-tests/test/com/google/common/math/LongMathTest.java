...
public class LongMathTest extends TestCase {
    ...
    public void testMaxLog10ForLeadingZeros() {
        for (int i = 0; i < Long.SIZE; i++) {
            assertEquals(BigIntegerMath.log10(BigInteger.ONE.shiftLeft(Long.SIZE - i), FLOOR),
    ...
    public void testConstantsBiggestBinomials() {
        for (int k = 0; k < LongMath.biggestBinomials.length; k++) {
            ...
            // In the first case, any long is valid; in the second, we want to test that the
            // next-bigger
            // long overflows.
        }
    ...
    // Throws an ArithmeticException if "the simple implementation" of binomial coefficients
    // overflows
    @GwtIncompatible // TODO
    ...
    public void testSqrtMatchesBigInteger() {
        for (long x : POSITIVE_LONG_CANDIDATES) {
            for (RoundingMode mode : ALL_SAFE_ROUNDING_MODES) {
                // Promote the long value (rather than using longValue() on the expected value) to
                // avoid
        ...
        for (long a : ALL_LONG_CANDIDATES) {
            for (long b : ALL_LONG_CANDIDATES) {
                ...
                try {
                    ...
                    if (expectedSuccess) {
                        failFormat("expected checkedSubtract(%s, %s) = %s; got ArithmeticException", a, b,
                                expectedResult);
                    }
                }
            }
        ...
        for (long a : ALL_LONG_CANDIDATES) {
            for (long b : ALL_LONG_CANDIDATES) {
                if (isAndroid && a == -4294967296L && b == 2147483648L) {
                    /*
                     * Bug in older versions of Android we test against, since fixed:
                     * -9223372036854775808L / -4294967296L = -9223372036854775808L!
                     *
                     * To be clear, this bug affects not the test's computation of the expected
                     * result but the _actual prod code_. But it probably affects only unusual
                     * cases.
                     */
                    continue;
                }
                ...
                try {
                    ...
                    if (expectedSuccess) {
                        failFormat("expected checkedMultiply(%s, %s) = %s; got ArithmeticException", a, b,
                                expectedResult);
                    }
                }
            }
    ...
    public void testSaturatedAdd() {
        for (long a : ALL_LONG_CANDIDATES) {
            for (long b : ALL_LONG_CANDIDATES) {
                assertOperationEquals(a, b, "s+", saturatedCast(valueOf(a).add(valueOf(b))),
                        LongMath.saturatedAdd(a, b));
            }
        }
    }
    ...
    public void testSaturatedSubtract() {
        for (long a : ALL_LONG_CANDIDATES) {
            for (long b : ALL_LONG_CANDIDATES) {
                assertOperationEquals(a, b, "s-", saturatedCast(valueOf(a).subtract(valueOf(b))),
    ...
    public void testSaturatedMultiply() {
        for (long a : ALL_LONG_CANDIDATES) {
            for (long b : ALL_LONG_CANDIDATES) {
                assertOperationEquals(a, b, "s*", saturatedCast(valueOf(a).multiply(valueOf(b))),
    ...
    public void testBinomial_exhaustiveNotOverflowing() {
        // Tests all of the inputs to LongMath.binomial that won't cause it to overflow, that
        // weren't
    ...
    private static void assertMean(long expectedMean, long x, long y) {
        assertEquals("The expectedMean should be the same as computeMeanSafely", expectedMean, computeMeanSafely(x, y));
        assertMean(x, y);
    }
