...
public class PairedStatsTest extends TestCase {
    ...
    public void testSampleCovariance() {
        ...
        assertThat(TWO_VALUES_PAIRED_STATS.sampleCovariance()).isWithin(ALLOWED_ERROR)
                .of(TWO_VALUES_SUM_OF_PRODUCTS_OF_DELTAS);
        assertThat(MANY_VALUES_PAIRED_STATS.sampleCovariance()).isWithin(ALLOWED_ERROR)
    ...
    public void testPearsonsCorrelationCoefficient() {
        ...
        assertThat(TWO_VALUES_PAIRED_STATS.pearsonsCorrelationCoefficient()).isWithin(ALLOWED_ERROR)
                .of(TWO_VALUES_PAIRED_STATS.populationCovariance()
        ...
        for (ManyValues values : ALL_MANY_VALUES) {
            ...
            if (values.hasAnyNonFinite()) {
                ...
                assertThat(pearsonsCorrelationCoefficient).named("Pearson's correlation coefficient of " + values)
                        ...
                        .of(stats.populationCovariance() / (stats.xStats().populationStandardDeviation()
    ...
    public void testLeastSquaresFit() {
        ...
        assertDiagonalLinearTransformation(TWO_VALUES_PAIRED_STATS.leastSquaresFit(),
                TWO_VALUES_PAIRED_STATS.xStats().mean(), TWO_VALUES_PAIRED_STATS.yStats().mean(),
                TWO_VALUES_PAIRED_STATS.xStats().populationVariance(), TWO_VALUES_PAIRED_STATS.populationCovariance());
        // For datasets of many double values, we test many combinations of finite and non-finite
        // x-values:
        for (ManyValues values : ALL_MANY_VALUES) {
            PairedStats stats = createPairedStatsOf(values.asIterable(), OTHER_MANY_VALUES);
            ...
            if (values.hasAnyNonFinite()) {
                ...
                assertDiagonalLinearTransformation(fit, stats.xStats().mean(), stats.yStats().mean(),
                        stats.xStats().populationVariance(), stats.populationCovariance());
            }
        }
        assertHorizontalLinearTransformation(HORIZONTAL_VALUES_PAIRED_STATS.leastSquaresFit(),
                HORIZONTAL_VALUES_PAIRED_STATS.yStats().mean());
        assertVerticalLinearTransformation(VERTICAL_VALUES_PAIRED_STATS.leastSquaresFit(),
    ...
    public void testEqualsAndHashCode() {
        new EqualsTester()
                .addEqualityGroup(MANY_VALUES_PAIRED_STATS, DUPLICATE_MANY_VALUES_PAIRED_STATS,
                        SerializableTester.reserialize(MANY_VALUES_PAIRED_STATS))
                .addEqualityGroup(new PairedStats(MANY_VALUES_STATS_ITERABLE, OTHER_MANY_VALUES_STATS, 1.23),
                        new PairedStats(MANY_VALUES_STATS_VARARGS, OTHER_MANY_VALUES_STATS, 1.23))
                .addEqualityGroup(new PairedStats(OTHER_MANY_VALUES_STATS, MANY_VALUES_STATS_ITERABLE, 1.23))
                .addEqualityGroup(new PairedStats(MANY_VALUES_STATS_ITERABLE, MANY_VALUES_STATS_ITERABLE, 1.23))
                ...
                .addEqualityGroup(new PairedStats(MANY_VALUES_STATS_ITERABLE, MANY_VALUES_STATS_ITERABLE, 1.234))
                .testEquals();
    }
    ...
    public void testToString() {
        ...
        assertThat(MANY_VALUES_PAIRED_STATS.toString()).isEqualTo("PairedStats{xStats="
                + MANY_VALUES_PAIRED_STATS.xStats() + ", yStats=" + MANY_VALUES_PAIRED_STATS.yStats()
                + ", populationCovariance=" + MANY_VALUES_PAIRED_STATS.populationCovariance() + "}");
    }
    ...
    public void testFromByteArrayWithTooShortArrayInputThrowsIllegalArgumentException() {
        ...
        byte[] tooShortByteArray = ByteBuffer.allocate(buffer.length - 1).order(ByteOrder.LITTLE_ENDIAN)
                .put(buffer, 0, buffer.length - 1).array();
        try {
