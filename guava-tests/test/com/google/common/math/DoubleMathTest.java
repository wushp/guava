...
public class DoubleMathTest extends TestCase {
    ...
    public void testConstantsMaxFactorial() {
        ...
        assertTrue(BigIntegerMath.factorial(DoubleMath.MAX_FACTORIAL + 1).compareTo(maxDoubleValue) > 0);
    }

    public void testConstantsEverySixteenthFactorial() {
        for (int i = 0, n = 0; n <= DoubleMath.MAX_FACTORIAL; i++, n += 16) {
            assertEquals(BigIntegerMath.factorial(n).doubleValue(), DoubleMath.everySixteenthFactorial[i]);
        }
    }
    ...
    public void testRoundFractionalDoubleToInt() {
        for (double d : FRACTIONAL_DOUBLE_CANDIDATES) {
            for (RoundingMode mode : ALL_SAFE_ROUNDING_MODES) {
                ...
                try {
                    assertEquals("Rounding " + d + " with mode " + mode, expected.intValue(),
    ...
    public void testRoundExactIntegralDoubleToInt() {
        for (double d : INTEGRAL_DOUBLE_CANDIDATES) {
            ...
            boolean isInBounds =
                    expected.compareTo(MAX_INT_AS_BIG_DECIMAL) <= 0 & expected.compareTo(MIN_INT_AS_BIG_DECIMAL) >= 0;
    ...
    public void testRoundExactIntegralDoubleToLong() {
        for (double d : INTEGRAL_DOUBLE_CANDIDATES) {
            ...
            BigDecimal expected = new BigDecimal(d).setScale(0, UNNECESSARY);
            boolean isInBounds =
                    expected.compareTo(MAX_LONG_AS_BIG_DECIMAL) <= 0 & expected.compareTo(MIN_LONG_AS_BIG_DECIMAL) >= 0;
    ...
    private strictfp double trueLog2(double d) {
        ...
        if (StrictMath.abs(StrictMath.pow(2.0, trueLog2) - d) > StrictMath
                .abs(StrictMath.pow(2.0, StrictMath.nextUp(trueLog2)) - d)) {
            trueLog2 = StrictMath.nextUp(trueLog2);
        }
        return trueLog2;
    }
    ...
    private static final List<Double> BAD_TOLERANCE_CANDIDATES = Doubles.asList(-Double.MIN_VALUE, -Double.MIN_NORMAL,
            -1, -20, Double.NaN, Double.NEGATIVE_INFINITY, -0.001);
    ...
    /*
     * We've split testFuzzyCompare() into multiple tests so that our internal Android test runner
     * has a better chance of completing each within its per-test-method timeout.
     */
