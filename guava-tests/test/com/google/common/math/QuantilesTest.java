...
public class QuantilesTest extends TestCase {
     ...
     * tests are divided into three sections: 1. Tests on a hardcoded dataset for chains starting
     * with median(), quartiles(), and scale(10); 2. Tests on hardcoded datasets include non-finite
     * values for chains starting with scale(10); 3. Tests on a mechanically generated dataset for
     * chains starting with percentiles(); 4. Tests of illegal usages of the API.
     */

    /*
     * Covering every combination would lead to an explosion in the number of tests. So we cover
     * only: - median with compute taking a double-collection and with computeInPlace; - quartiles
     * with index and with indexes taking int-varargs, and with compute taking a double-collection
     * and with computeInPlace; - scale with index and with indexes taking int-varargs, and with all
     * overloads of compute taking a double-collection and with computeInPlace; - scale with indexes
     * taking integer-collection with compute taking a double-collection and with computeInPlace; -
    ...
    // 1. Tests on a hardcoded dataset for chains starting with median(), quartiles(), and
    // scale(10):

    /**
     * The squares of the 16 integers from 0 to 15, in an arbitrary order.
     */
    private static final ImmutableList<Double> SIXTEEN_SQUARES_DOUBLES = ImmutableList.of(25.0, 100.0, 0.0, 144.0, 9.0,
            121.0, 4.0, 225.0, 169.0, 64.0, 49.0, 16.0, 36.0, 1.0, 81.0, 196.0);
    private static final ImmutableList<Long> SIXTEEN_SQUARES_LONGS =
            ImmutableList.of(25L, 100L, 0L, 144L, 9L, 121L, 4L, 225L, 169L, 64L, 49L, 16L, 36L, 1L, 81L, 196L);
    ...
    public void testQuartiles_index_compute_doubleCollection() {
        assertQuantile(1, SIXTEEN_SQUARES_QUARTILE_1, quartiles().index(1).compute(SIXTEEN_SQUARES_DOUBLES));
    }
    ...
    public void testQuartiles_indexes_varargs_compute_doubleCollection() {
        ImmutableMap<Integer, Double> expected =
                ImmutableMap.of(1, SIXTEEN_SQUARES_QUARTILE_1, 3, SIXTEEN_SQUARES_QUARTILE_3);
    ...
    public void testScale_index_computeInPlace() {
        double[] dataset = Doubles.toArray(SIXTEEN_SQUARES_DOUBLES);
        assertQuantile(1, SIXTEEN_SQUARES_DECILE_1, Quantiles.scale(10).index(1).computeInPlace(dataset));
        assertDatasetAnyOrder(SIXTEEN_SQUARES_DOUBLES, dataset);
    }
    ...
    public void testScale_indexes_varargs_compute_doubleCollection() {
        ImmutableMap<Integer, Double> expected = ImmutableMap.of(0, SIXTEEN_SQUARES_MIN, 10, SIXTEEN_SQUARES_MAX, 5,
                SIXTEEN_SQUARES_MEDIAN, 1, SIXTEEN_SQUARES_DECILE_1, 8, SIXTEEN_SQUARES_DECILE_8);
        // Note that we specify index 1 twice, which by the method contract should be ignored.
        assertQuantilesMap(expected, Quantiles.scale(10).indexes(0, 10, 5, 1, 8, 1).compute(SIXTEEN_SQUARES_DOUBLES));
    }

    public void testScale_indexes_varargs_compute_doubleCollection_snapshotsIndexes() {
        // This test is the same as testScale_indexes_varargs_compute_doubleCollection except that
        // the
        // array of indexes to be calculated is modified between the calls to indexes and compute:
        // since
    ...
    public void testScale_indexes_largeVarargs_compute_doubleCollection() {
        ...
        // For the otherIndex calculation, we have q=Integer.MAX_VALUE, k=(Integer.MAX_VALUE-1)/3,
        // and
        ...
        ImmutableMap<Integer, Double> expected =
                ImmutableMap.of(0, SIXTEEN_SQUARES_MIN, scale, SIXTEEN_SQUARES_MAX, otherIndex, otherValue);
    ...
    public void testScale_indexes_varargs_compute_longCollection() {
        ImmutableMap<Integer, Double> expected = ImmutableMap.of(0, SIXTEEN_SQUARES_MIN, 10, SIXTEEN_SQUARES_MAX, 5,
                SIXTEEN_SQUARES_MEDIAN, 1, SIXTEEN_SQUARES_DECILE_1, 8, SIXTEEN_SQUARES_DECILE_8);
        // Note that we specify index 1 twice, which by the method contract should be ignored.
        assertQuantilesMap(expected, Quantiles.scale(10).indexes(0, 10, 5, 1, 8, 1).compute(SIXTEEN_SQUARES_LONGS));
    }
    ...
    public void testScale_indexes_varargs_compute_integerCollection() {
        ImmutableMap<Integer, Double> expected = ImmutableMap.of(0, SIXTEEN_SQUARES_MIN, 10, SIXTEEN_SQUARES_MAX, 5,
                SIXTEEN_SQUARES_MEDIAN, 1, SIXTEEN_SQUARES_DECILE_1, 8, SIXTEEN_SQUARES_DECILE_8);
        // Note that we specify index 1 twice, which by the method contract should be ignored.
        assertQuantilesMap(expected, Quantiles.scale(10).indexes(0, 10, 5, 1, 8, 1).compute(SIXTEEN_SQUARES_INTEGERS));
    }
    ...
    public void testScale_indexes_varargs_computeInPlace() {
        ...
        ImmutableMap<Integer, Double> expected = ImmutableMap.of(0, SIXTEEN_SQUARES_MIN, 10, SIXTEEN_SQUARES_MAX, 5,
                SIXTEEN_SQUARES_MEDIAN, 1, SIXTEEN_SQUARES_DECILE_1, 8, SIXTEEN_SQUARES_DECILE_8);
        assertQuantilesMap(expected, Quantiles.scale(10).indexes(0, 10, 5, 1, 8, 1).computeInPlace(dataset));
        assertDatasetAnyOrder(SIXTEEN_SQUARES_DOUBLES, dataset);
    }

    public void testScale_indexes_varargs_computeInPlace_explicitVarargs() {
        ImmutableMap<Integer, Double> expected = ImmutableMap.of(0, 12.3, 10, 78.9);
        assertQuantilesMap(expected, Quantiles.scale(10).indexes(0, 10).computeInPlace(78.9, 12.3, 45.6));
    }
    ...
    public void testScale_indexes_collection_compute_doubleCollection() {
        ...
        // Note that we specify index 1 twice, which by the method contract should be ignored.
        assertQuantilesMap(expected,
                Quantiles.scale(10).indexes(ImmutableList.of(0, 10, 5, 1, 8, 1)).compute(SIXTEEN_SQUARES_DOUBLES));
    }
    ...
    private static final ImmutableList<Double> ONE_TO_FIVE_AND_NAN = ImmutableList.of(3.0, 5.0, NaN, 1.0, 4.0, 2.0);
    ...
    public void testScale_indexes_varargs_compute_doubleCollection_positiveInfinity() {
        ...
        Map<Integer, Double> expected = ImmutableMap.<Integer, Double>builder().put(0, 1.0).put(1, 1.5).put(2, 2.0)
                .put(8, 5.0).put(9, POSITIVE_INFINITY) // interpolating between 5.0 and
                                                       // POSITIVE_INFNINITY
                .put(10, POSITIVE_INFINITY).build();
    ...
    public void testScale_indexes_varargs_compute_doubleCollection_negativeInfinity() {
        ...
        Map<Integer, Double> expected =
                ImmutableMap.<Integer, Double>builder().put(0, NEGATIVE_INFINITY).put(1, NEGATIVE_INFINITY) // interpolating
                                                                                                            // between
                                                                                                            // NEGATIVE_INFNINITY
                                                                                                            // and
                                                                                                            // 1.0
                        .put(2, 1.0).put(8, 4.0).put(9, 4.5).put(10, 5.0).build();
        assertQuantilesMap(expected, actual);
    }
    ...
    public void testScale_indexes_varargs_compute_doubleCollection_bothInfinities() {
        Map<Integer, Double> actual =
                Quantiles.scale(10).indexes(0, 1, 2, 8, 9, 10).compute(NEGATIVE_INFINITY_AND_FIVE_POSITIVE_INFINITIES);
        Map<Integer, Double> expected = ImmutableMap.<Integer, Double>builder().put(0, NEGATIVE_INFINITY).put(1, NaN) // interpolating
                                                                                                                      // between
                                                                                                                      // NEGATIVE_
                                                                                                                      // and
                                                                                                                      // POSITIVE_INFINITY
                                                                                                                      // values
                .put(2, POSITIVE_INFINITY).put(8, POSITIVE_INFINITY).put(9, POSITIVE_INFINITY) // interpolating
                                                                                               // between
                                                                                               // two
                                                                                               // POSITIVE_INFINITY
                                                                                               // values
    ...
    public void testScale_indexes_varargs_compute_doubleCollection_nan() {
        ...
        Map<Integer, Double> expected = ImmutableMap.<Integer, Double>builder().put(0, NaN).put(1, NaN).put(2, NaN)
                .put(8, NaN).put(9, NaN).put(10, NaN).build();
        assertQuantilesMap(expected, actual);
    }
    ...
    private static double expectedLargeDatasetPercentile(int index) {
        // We have q=100, k=index, and N=9951. Therefore k*(N-1)/q is 99.5*index. If index is even,
        // that
        // is an integer 199*index/2. If index is odd, that is halfway between floor(199*index/2)
        // and
        // ceil(199*index/2).
        if (index % 2 == 0) {
    ...
    /**
     * Asserts that the actual quantile value returned matches the expected value, allowing the
     * margin of error {@link #ALLOWED_ERROR}. The assertion passes when the expected and actual
