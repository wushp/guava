...
public class ByteSourceTester extends SourceSinkTester<ByteSource, byte[], ByteSourceFactory> {
    ...
    static TestSuite tests(String name, ByteSourceFactory factory, boolean testAsCharSource) {
        ...
        for (Map.Entry<String, String> entry : TEST_STRINGS.entrySet()) {
            if (testAsCharSource) {
                ...
                suite.addTest(
                        suiteForBytes(factory, entry.getValue().getBytes(Charsets.UTF_8), name, entry.getKey(), true));
            }
        }
        return suite;
    }

    static TestSuite suiteForString(ByteSourceFactory factory, String string, String name, String desc) {
        ...
        suite.addTest(
                CharSourceTester.suiteForString(charSourceFactory, string, name + ".asCharSource[Charset]", desc));
        return suite;
    }

    static TestSuite suiteForBytes(ByteSourceFactory factory, byte[] bytes, String name, String desc, boolean slice) {
        ...
        if (slice && bytes.length > 0) {
            ...
            // if expected.length == 0, off has to be 0 but length doesn't matter--result will be
            // empty
            ...
            ByteSourceFactory slicedOffsetPastEnd =
                    SourceSinkFactories.asSlicedByteSourceFactory(factory, expected.length + 2, expected.length + 10);
    ...
    public ByteSourceTester(ByteSourceFactory factory, byte[] bytes, String suiteName, String caseDesc, Method method) {
