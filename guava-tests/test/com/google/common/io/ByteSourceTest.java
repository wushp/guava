...
public class ByteSourceTest extends IoTestCase {
    ...
    public static TestSuite suite() {
        ...
        for (boolean asCharSource : new boolean[] {false, true}) {
            ...
            suite.addTest(ByteSourceTester.tests("ByteSource.empty[]", SourceSinkFactories.emptyByteSourceFactory(),
                    asCharSource));
        }
     ...
     *
     * <p>
     * Without special handling, it's possible to have reads of the open stream start <i>before</i>
    ...
    public void testSlice_appendingAfterSlicing() throws IOException {
        ...
        // We could use a custom InputStream instead to make the read start at index 10, but since
        // this
    ...
    public void testClosesOnErrors_copyingToByteSinkThatThrows() {
        for (TestOption option : EnumSet.of(OPEN_THROWS, WRITE_THROWS, CLOSE_THROWS)) {
            ...
            // ensure stream was closed IF it was opened (depends on implementation whether or not
            // it's
    ...
    private static final ByteSource BROKEN_OPEN_SOURCE = new TestByteSource(new byte[10], OPEN_THROWS);
    private static final ByteSource BROKEN_READ_SOURCE = new TestByteSource(new byte[10], READ_THROWS);
