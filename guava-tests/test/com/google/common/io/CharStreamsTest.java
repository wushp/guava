...
public class CharStreamsTest extends IoTestCase {
     ...
     * reduced the available size of the buffer each time a call to read didn't fill up the
     * available space in the buffer completely. In general this is a performance problem since the
     * buffer size is permanently reduced, but with certain Reader implementations it could also
     * cause the buffer size to reach 0, causing an infinite loop.
     */
    public void testCopyWithReaderThatDoesNotFillBuffer() throws IOException {
        ...
        // the main assertion of this test is here... the copy will fail if the buffer size goes
        // down
    ...
    private static Reader newNonBufferFillingReader(Reader reader) {
        return new FilterReader(reader) {
            ...
            public int read(char[] cbuf, int off, int len) throws IOException {
                // if a buffer isn't being cleared correctly, this method will eventually start
                // being called
