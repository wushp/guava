...
public class MoreFilesTest extends TestCase {
    ...
    public static TestSuite suite() {
        ...
        suite.addTest(ByteSinkTester.tests("MoreFiles.asByteSink[Path]", SourceSinkFactories.pathByteSinkFactory()));
        ...
        suite.addTest(
                CharSinkTester.tests("MoreFiles.asCharSink[Path, Charset]", SourceSinkFactories.pathCharSinkFactory()));
    ...
    public void testEqual() throws IOException {
        try (FileSystem fs = Jimfs.newFileSystem(Configuration.unix())) {
            ...
            assertThat(MoreFiles.asByteSource(fooPath).contentEquals(MoreFiles.asByteSource(fooPath))).isTrue();
            ...
            assertThat(MoreFiles.asByteSource(fooPath).size()).isEqualTo(MoreFiles.asByteSource(fooCopy).size());
            assertThat(MoreFiles.equal(fooPath, fooCopy)).isFalse();

            // should also assert that a Path that erroneously reports a size 0 can still be
            // compared,
    ...
    static FileSystem newTestFileSystem(Feature... supportedFeatures) throws IOException {
        FileSystem fs = Jimfs.newFileSystem(Configuration.unix().toBuilder()
                .setSupportedFeatures(ObjectArrays.concat(SYMBOLIC_LINKS, supportedFeatures)).build());
    ...
    public void testDirectoryDeletion_basic() throws IOException {
        for (DirectoryDeleteMethod method : EnumSet.allOf(DirectoryDeleteMethod.class)) {
            try (FileSystem fs = newTestFileSystem(SECURE_DIRECTORY_STREAM)) {
                ...
                assertEquals("contents of /dontdelete deleted by delete method " + method, 3,
     ...
     *
     * <p>
     * We can only test this with a file system that supports SecureDirectoryStream, because it's
                ...
                try {
                    for (int i = 0; i < 5000; i++) {
                        ...
                        try {
                            ...
                            // the delete method may or may not throw an exception, but if it does
                            // that's fine
                            // and expected
                        }

                        // this test is mainly checking that the contents of /dontdelete aren't
                        // deleted under
                        // any circumstances
    ...
    private enum DirectoryDeleteMethod {
        DELETE_DIRECTORY_CONTENTS {
            ...
            public void assertDeleteSucceeded(Path path) throws IOException {
                assertEquals("contents of directory " + path + " not deleted with delete method " + this, 0,
