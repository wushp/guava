 ...
 *
 * <p>
 * A single {@code SourceSinkFactory} implementation generally corresponds to one specific way of
...
public interface SourceSinkFactory<S, T> {
    ...
    /**
     * Returns the data to expect the source or sink to contain given the data that was used to
     * create the source or written to the sink. Typically, this will just return the input
     * directly, but in some cases it may alter the input. For example, if the factory returns a
     * sliced view of a source created with some given bytes, this method would return a subsequence
     * of the given (byte[]) data.
     */
