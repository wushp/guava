...
public class MultimapsCollectionTest extends TestCase {
    ...
    private static final Feature<?>[] FOR_MAP_FEATURES_ONE =
            {CollectionSize.ONE, ALLOWS_NULL_VALUES, SUPPORTS_REMOVE, CollectionFeature.SUPPORTS_ITERATOR_REMOVE};

    private static final Feature<?>[] FOR_MAP_FEATURES_ANY = {CollectionSize.ANY, ALLOWS_NULL_VALUES, SUPPORTS_REMOVE,
            CollectionFeature.SUPPORTS_ITERATOR_REMOVE, MultisetTestSuiteBuilder.NoRecurse.NO_ENTRY_SET, // Cannot
                                                                                                         // create
                                                                                                         // entries
                                                                                                         // with
                                                                                                         // count
     ...
     * Map} by ignoring all but the latest value for each key. This class exists only so that we can
     * use {@link MultimapsCollectionTest#populateMultimapForGet(Multimap, String[])} and similar
     * methods to populate a map to be passed to {@link Multimaps#forMap(Map)}. All tests should run
     * against the result of {@link #build()}.
     */
    private static final class PopulatableMapAsMultimap<K, V> extends ForwardingMultimap<K, V> {
        ...
        PopulatableMapAsMultimap() {
            this.map = newHashMap();
            this.unusableDelegate = (SetMultimap<K, V>) newProxyInstance(SetMultimap.class.getClassLoader(),
    ...
    abstract static class TestEntriesGenerator implements TestCollectionGenerator<Entry<String, Integer>> {
        @Override
        public SampleElements<Entry<String, Integer>> samples() {
            return new SampleElements<Entry<String, Integer>>(Maps.immutableEntry("bar", 1),
                    Maps.immutableEntry("bar", 2), Maps.immutableEntry("foo", 3), Maps.immutableEntry("bar", 3),
    ...
    static abstract class TransformedMultimapGenerator<M extends Multimap<String, String>>
        ...
        public SampleElements<Entry<String, String>> samples() {
            return new SampleElements<Entry<String, String>>(mapEntry("one", "january"), mapEntry("two", "february"),
                    mapEntry("three", "march"), mapEntry("four", "april"), mapEntry("five", "may"));
        }
    ...
    private static Test filterSuite() {
        ...
        suite.addTest(SetMultimapTestSuiteBuilder.using(new FilteredSetMultimapGenerator() {
            ...
            SetMultimap<String, Integer> filter(SetMultimap<String, Integer> multimap) {
                ...
                .withFeatures(CollectionSize.ANY, MultimapFeature.VALUE_COLLECTIONS_SUPPORT_ITERATOR_REMOVE,
        ...
        suite.addTest(SetMultimapTestSuiteBuilder.using(new FilteredSetMultimapGenerator() {
            ...
            SetMultimap<String, Integer> filter(SetMultimap<String, Integer> multimap) {
                ...
                .withFeatures(CollectionSize.ANY, MapFeature.GENERAL_PURPOSE, MapFeature.ALLOWS_NULL_KEYS,
                        MapFeature.ALLOWS_NULL_VALUES, MapFeature.ALLOWS_ANY_NULL_QUERIES)
                .createTestSuite());
        suite.addTest(SetMultimapTestSuiteBuilder.using(new FilteredSetMultimapGenerator() {
            @Override
            SetMultimap<String, Integer> filter(SetMultimap<String, Integer> multimap) {
