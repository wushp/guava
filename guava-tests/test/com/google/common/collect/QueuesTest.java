...
public class QueuesTest extends TestCase {
    ...
    public static List<BlockingQueue<Object>> blockingQueues() {
        return ImmutableList.<BlockingQueue<Object>>of(new LinkedBlockingQueue<Object>(),
                new LinkedBlockingQueue<Object>(10), new SynchronousQueue<Object>(), new ArrayBlockingQueue<Object>(10),
                new LinkedBlockingDeque<Object>(), new LinkedBlockingDeque<Object>(10),
                new PriorityBlockingQueue<Object>(10, Ordering.arbitrary()));
    }

    /*
     * We need to perform operations in a thread pool, even for simple cases, because the queue
     * might be a SynchronousQueue.
     */
    ...
    private static <T> int drain(BlockingQueue<T> q, Collection<? super T> buffer, int maxElements, long timeout,
            TimeUnit unit, boolean interruptibly) throws InterruptedException {
        return interruptibly ? Queues.drain(q, buffer, maxElements, timeout, unit)
    ...
    private void testDrainTimesOut(BlockingQueue<Object> q) throws Exception {
        for (boolean interruptibly : new boolean[] {true, false}) {
            ...
            // If even the first one wasn't there, clean up so that the next test doesn't see an
            // element.
