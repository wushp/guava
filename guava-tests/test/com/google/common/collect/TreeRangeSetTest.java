...
public class TreeRangeSetTest extends AbstractRangeSetTest {
    ...
    <K, V> void testNavigationAgainstExpected(NavigableMap<K, V> expected, NavigableMap<K, V> navigableMap,
            Iterable<K> keysToTest) {
        for (K key : keysToTest) {
            ...
            for (boolean inclusive : new boolean[] {false, true}) {
                assertThat(navigableMap.headMap(key, inclusive).entrySet())
                        .containsExactlyElementsIn(expected.headMap(key, inclusive).entrySet()).inOrder();
                assertThat(navigableMap.tailMap(key, inclusive).entrySet())
                        .containsExactlyElementsIn(expected.tailMap(key, inclusive).entrySet()).inOrder();
    ...
    public void testEnclosing(RangeSet<Integer> rangeSet) {
        ...
        for (Range<Integer> query : QUERY_RANGES) {
            ...
            assertEquals(rangeSet + " was incorrect on enclosesAll([" + query + "])", expectEnclose,
        ...
        for (Range<Integer> range1 : QUERY_RANGES) {
            for (Range<Integer> range2 : QUERY_RANGES) {
                ...
                for (Range<Integer> subRange : QUERY_RANGES) {
                    testViewAgainstExpected(expectedSubRangeSet(rangeSet, subRange), rangeSet.subRangeSet(subRange));
                }
            }
        }
    }
    ...
    public void testSubRangeSetOfComplement() {
        for (Range<Integer> range1 : QUERY_RANGES) {
            for (Range<Integer> range2 : QUERY_RANGES) {
                ...
                rangeSet.add(range2);
                for (Range<Integer> subRange : QUERY_RANGES) {
                    testViewAgainstExpected(expectedSubRangeSet(expectedComplement(rangeSet), subRange),
    ...
    public void testComplementOfSubRangeSet() {
        for (Range<Integer> range1 : QUERY_RANGES) {
            for (Range<Integer> range2 : QUERY_RANGES) {
                ...
                rangeSet.add(range2);
                for (Range<Integer> subRange : QUERY_RANGES) {
                    testViewAgainstExpected(expectedComplement(expectedSubRangeSet(rangeSet, subRange)),
        ...
        for (Range<Integer> range1 : QUERY_RANGES) {
            for (Range<Integer> range2 : QUERY_RANGES) {
                ...
                testNavigationAgainstExpected(expectedRangesByUpperBound,
                        new TreeRangeSet.RangesByUpperBound<Integer>(rangeSet.rangesByLowerBound), CUTS_TO_TEST);
            }
        }
    }
    ...
    public void testRemoveMiddle() {
        ...
        assertThat(rangeSet.complement().asRanges()).containsExactly(Range.closedOpen(3, 4), Range.greaterThan(6))
                .inOrder();
    }
    ...
    public void testRemoveAll() {
        ...
        assertThat(rangeSet.asRanges()).containsExactly(Range.closedOpen(3, 5), Range.open(8, 9)).inOrder();
    }
