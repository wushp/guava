...
public class MinMaxPriorityQueueTest extends TestCase {
    ...
    public static Test suite() {
        ...
        }).named("MinMaxPriorityQueue").withFeatures(CollectionSize.ANY, CollectionFeature.GENERAL_PURPOSE)
                .createTestSuite());
        return suite;
    }
    ...
    public void testCreation_comparator_maximumSize() {
        MinMaxPriorityQueue<Integer> queue = MinMaxPriorityQueue.orderedBy(SOME_COMPARATOR).maximumSize(42).create();
        assertEquals(11, queue.capacity());
        assertEquals(42, queue.maximumSize);
        assertSame(SOME_COMPARATOR, queue.comparator());
    }
    ...
    public void testCreation_expectedSize_maximumSize() {
        MinMaxPriorityQueue<Integer> queue = MinMaxPriorityQueue.expectedSize(8).maximumSize(42).create();
        assertEquals(8, queue.capacity());
        assertEquals(42, queue.maximumSize);
        checkNatural(queue);
    }
    ...
    public void testCreation_comparator_withContents() {
        MinMaxPriorityQueue<Integer> queue = MinMaxPriorityQueue.orderedBy(SOME_COMPARATOR).create(NUMBERS);
        assertEquals(6, queue.size());
        assertEquals(11, queue.capacity());
        checkUnbounded(queue);
        assertSame(SOME_COMPARATOR, queue.comparator());
    ...
    public void testCreation_allOptions() {
        MinMaxPriorityQueue<Integer> queue =
                MinMaxPriorityQueue.orderedBy(SOME_COMPARATOR).expectedSize(8).maximumSize(42).create(NUMBERS);
        assertEquals(6, queue.size());
        assertEquals(8, queue.capacity());
        assertEquals(42, queue.maximumSize);
        assertSame(SOME_COMPARATOR, queue.comparator());
    ...
    public void testHeapIntact() {
        ...
        /*
         * this map would contain the same exact elements as the MinMaxHeap; the value in the map is
        ...
        assertEquals(currentHeapSize, mmHeap.size());
        assertTrue("Heap not intact after " + numberOfModifications + " random mixture of operations",
                mmHeap.isIntact());
    }
    ...
    /**
     * This tests a special case of the removeAt() call. Moving an element sideways on the heap
     * could break the invariants. Sometimes we need to bubble an element up instead of trickling
     * down. See implementation.
     */
    ...
    public void testInvalidatingRemove2() {
        MinMaxPriorityQueue<Integer> mmHeap = MinMaxPriorityQueue.create();
        List<Integer> values = Lists.newArrayList(1, 20, 1000, 2, 3, 30, 40, 10, 11, 12, 13, 300, 400, 500, 600, 4, 5,
                6, 7, 8, 9, 4, 5, 200, 250);
    ...
    /**
     * This tests a special case where removeAt has to trickle an element first down one level from
     * a min to a max level, then up one level above the index of the removed element. It also tests
    ...
    private <T extends Comparable<T>> void runIterator(final List<T> values, int steps) throws Exception {
        IteratorTester<T> tester = new IteratorTester<T>(steps, IteratorFeature.MODIFIABLE, Lists.newLinkedList(values),
    ...
    public void testRandomAddsAndRemoves() {
        ...
        MinMaxPriorityQueue<Integer> queue = MinMaxPriorityQueue.create();
        int range = 10_000; // range should be small enough that equal elements occur
                            // semi-frequently
    ...
    private static void insertRandomly(ArrayList<Integer> elements, MinMaxPriorityQueue<Integer> q, Random random) {
