...
public class ImmutableSetTest extends AbstractImmutableSetTest {
    ...
    public static Test suite() {
        ...
        suite.addTest(SetTestSuiteBuilder.using(new ImmutableSetCopyOfGenerator())
                .named(ImmutableSetTest.class.getName()).withFeatures(CollectionSize.ANY, CollectionFeature.KNOWN_ORDER,
        ...
        suite.addTest(
                SetTestSuiteBuilder.using(new ImmutableSetWithBadHashesGenerator())
                        .named(ImmutableSetTest.class.getName() + ", with bad hashes").withFeatures(CollectionSize.ANY,
        ...
        suite.addTest(
                SetTestSuiteBuilder.using(new DegeneratedImmutableSetGenerator())
                        .named(ImmutableSetTest.class.getName() + ", degenerate").withFeatures(CollectionSize.ONE,
                                CollectionFeature.KNOWN_ORDER, CollectionFeature.ALLOWS_NULL_QUERIES)
                        .createTestSuite());

        suite.addTest(
                ListTestSuiteBuilder.using(new ImmutableSetAsListGenerator()).named("ImmutableSet.asList")
    ...
    public void testCreation_oneDuplicate() {
        ...
        assertEquals(Lists.newArrayList("a", "b", "c", "d", "e", "f", "g", "h", "i", "j", "k", "l", "m"),
    ...
    public void testToImmutableSet() {
        ...
        Equivalence<ImmutableSet<String>> equivalence = Equivalence.equals().onResultOf(ImmutableSet::asList);
        CollectorTester.of(collector, equivalence).expectCollects(ImmutableSet.of("a", "b", "c", "d"), "a", "b", "a",
                "c", "b", "b", "d");
    }
    ...
    public void testToImmutableSet_duplicates() {
        ...
        Collector<TypeWithDuplicates, ?, ImmutableSet<TypeWithDuplicates>> collector = ImmutableSet.toImmutableSet();
        BiPredicate<ImmutableSet<TypeWithDuplicates>, ImmutableSet<TypeWithDuplicates>> equivalence = (set1, set2) -> {
            if (!set1.equals(set2)) {
                return false;
            }
        ...
        CollectorTester.of(collector, equivalence).expectCollects(ImmutableSet.of(a, b1, c), a, b1, c, b2);
    }
