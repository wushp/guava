...
public class ImmutableSortedSetTest extends AbstractImmutableSetTest {
    ...
    public static Test suite() {
        ...
        suite.addTest(NavigableSetTestSuiteBuilder.using(new ImmutableSortedSetCopyOfGenerator())
                .named(ImmutableSortedSetTest.class.getName())
                .withFeatures(CollectionSize.ANY, CollectionFeature.KNOWN_ORDER, CollectionFeature.SERIALIZABLE,
                        CollectionFeature.ALLOWS_NULL_QUERIES)
                .createTestSuite());

        suite.addTest(NavigableSetTestSuiteBuilder.using(new ImmutableSortedSetExplicitComparator())
                .named(ImmutableSortedSetTest.class.getName() + ", explicit comparator, vararg")
                .withFeatures(CollectionSize.ANY, CollectionFeature.KNOWN_ORDER, CollectionFeature.SERIALIZABLE,
                        CollectionFeature.ALLOWS_NULL_QUERIES)
                .createTestSuite());

        suite.addTest(NavigableSetTestSuiteBuilder.using(new ImmutableSortedSetExplicitSuperclassComparatorGenerator())
                .named(ImmutableSortedSetTest.class.getName() + ", explicit superclass comparator, iterable")
                .withFeatures(CollectionSize.ANY, CollectionFeature.KNOWN_ORDER, CollectionFeature.SERIALIZABLE,
                        CollectionFeature.ALLOWS_NULL_QUERIES)
                .createTestSuite());
        ...
        suite.addTest(
                NavigableSetTestSuiteBuilder.using(new ImmutableSortedSetUnhashableGenerator())
                        ...
                        .named(ImmutableSortedSetTest.class.getName() + ", unhashable").withFeatures(CollectionSize.ANY,
                                CollectionFeature.KNOWN_ORDER, CollectionFeature.ALLOWS_NULL_QUERIES)
                        .createTestSuite());

        suite.addTest(NavigableSetTestSuiteBuilder.using(new ImmutableSortedSetDescendingGenerator())
        ...
        suite.addTest(
                ListTestSuiteBuilder.using(new ImmutableSortedSetAsListGenerator()).named("ImmutableSortedSet.asList")
     ...
     *
     * A call to of() with a parameter that is not a plain Object[] (here, Interface[]) creates a
     * RegularImmutableSortedSet backed by an array of that type. Later,
     * RegularImmutableSortedSet.toArray() calls System.arraycopy() to copy from that array to the
     * destination array. This would be fine, but GWT has a bug: It refuses to copy from an E[] to
     * an Object[] when E is an interface type.
     */
    ...
    public void testOf_gwtArraycopyBug() {
         ...
         * The test requires:
         *
         * 1) An interface I extending Comparable<I> so that the created array is of an interface
         * type. 2) An instance of a class implementing that interface so that we can pass non-null
         * instances of the interface.
         *
         * (Currently it's safe to pass instances for which compareTo() always returns 0, but if we
         * had a SingletonImmutableSortedSet, this might no longer be the case.)
         *
         * javax.naming.Name and java.util.concurrent.Delayed might work, but they're fairly
         * obscure, we've invented our own interface and class.
         */
    ...
    public void testExplicit_ordering_dupes() {
        SortedSet<String> set = ImmutableSortedSet.orderedBy(STRING_LENGTH)
                .add("in", "the", "quick", "brown", "fox", "jumped", "over", "a", "lazy", "dog").build();
        assertThat(set).containsExactly("a", "in", "the", "over", "quick", "jumped").inOrder();
    }
    ...
    public void testExplicit_subSet() {
        ...
        assertThat(set.subSet("a", "california")).containsExactly("a", "in", "the", "over", "quick", "jumped")
                .inOrder();
    ...
    public void testCopyOfExplicit_ordering() {
        SortedSet<String> set =
                ImmutableSortedSet.copyOf(STRING_LENGTH, asList("in", "the", "quick", "jumped", "over", "a"));
        assertThat(set).containsExactly("a", "in", "the", "over", "quick", "jumped").inOrder();
    }

    public void testCopyOfExplicit_ordering_dupes() {
        SortedSet<String> set = ImmutableSortedSet.copyOf(STRING_LENGTH,
    ...
    public void testCopyOfExplicit_iterator_ordering() {
        SortedSet<String> set =
                ImmutableSortedSet.copyOf(STRING_LENGTH, asIterator("in", "the", "quick", "jumped", "over", "a"));
        assertThat(set).containsExactly("a", "in", "the", "over", "quick", "jumped").inOrder();
    }
    ...
    public void testToImmutableSortedSet() {
        ...
        BiPredicate<ImmutableSortedSet<String>, ImmutableSortedSet<String>> equivalence =
                Equivalence.equals().onResultOf(ImmutableSortedSet<String>::comparator)
                        .and(Equivalence.equals().onResultOf(ImmutableSortedSet::asList)).and(Equivalence.equals());
        CollectorTester.of(collector, equivalence).expectCollects(ImmutableSortedSet.of("a", "b", "c", "d"), "a", "b",
                "a", "c", "b", "b", "d");
    }
    ...
    public void testToImmutableSortedSet_customComparator() {
        ...
        CollectorTester.of(collector, equivalence).expectCollects(expected, "a", "B", "a", "c", "b", "b", "d");
    }
    ...
    public void testToImmutableSortedSet_duplicates() {
        ...
        Collector<TypeWithDuplicates, ?, ImmutableSortedSet<TypeWithDuplicates>> collector =
                ImmutableSortedSet.toImmutableSortedSet(Ordering.natural());
        BiPredicate<ImmutableSortedSet<TypeWithDuplicates>, ImmutableSortedSet<TypeWithDuplicates>> equivalence =
                (set1, set2) -> {
                    if (!set1.equals(set2)) {
                        return false;
                    }
        ...
        CollectorTester.of(collector, equivalence).expectCollects(ImmutableSortedSet.of(a, b1, c), a, b1, c, b2);
    }
    ...
    public void testSupertypeComparator() {
        SortedSet<Integer> set = new ImmutableSortedSet.Builder<Integer>(TO_STRING).add(3, 12, 101, 44).build();
        assertThat(set).containsExactly(101, 12, 3, 44).inOrder();
    }

    public void testSupertypeComparatorSubtypeElements() {
        SortedSet<Number> set = new ImmutableSortedSet.Builder<Number>(TO_STRING).add(3, 12, 101, 44).build();
        assertThat(set).containsExactly(101, 12, 3, 44).inOrder();
    }
    ...
    public void testLegacyComparable_of() {
        ...
        @SuppressWarnings("unchecked") // using a legacy comparable
        ImmutableSortedSet<LegacyComparable> set2 = ImmutableSortedSet.of(LegacyComparable.Z, LegacyComparable.Y);
    }

    public void testLegacyComparable_copyOf_collection() {
        ImmutableSortedSet<LegacyComparable> set = ImmutableSortedSet.copyOf(LegacyComparable.VALUES_BACKWARD);
    ...
    public void testLegacyComparable_builder_natural() {
        ...
        ImmutableSortedSet.Builder<LegacyComparable> builder = ImmutableSortedSet.<LegacyComparable>naturalOrder();
    ...
    public void testLegacyComparable_builder_reverse() {
        @SuppressWarnings("unchecked")
        // Note: IntelliJ wrongly reports an error for this statement
        ImmutableSortedSet.Builder<LegacyComparable> builder = ImmutableSortedSet.<LegacyComparable>reverseOrder();
    ...
    public void testTailSetInclusive() {
        ...
        Arrays.sort(strings);
        for (int i = 0; i < strings.length; i++) {
            assertThat(set.tailSet(strings[i], true)).containsExactlyElementsIn(sortedNumberNames(i, strings.length))
                    .inOrder();
        }
    }
    ...
    public void testTailSetExclusive() {
        ...
        for (int i = 0; i < strings.length; i++) {
            assertThat(set.tailSet(strings[i], false))
                    .containsExactlyElementsIn(sortedNumberNames(i + 1, strings.length)).inOrder();
        }
    }
        ...
        for (int i = 0; i < strings.length; i++) {
            for (int j = i; j < strings.length; j++) {
                assertThat(set.subSet(strings[i], false, strings[j], false))
                        .containsExactlyElementsIn(sortedNumberNames(Math.min(i + 1, j), j)).inOrder();
            }
        }
    }
        ...
        for (int i = 0; i < strings.length; i++) {
            for (int j = i; j < strings.length; j++) {
                assertThat(set.subSet(strings[i], true, strings[j], false))
                        .containsExactlyElementsIn(sortedNumberNames(i, j)).inOrder();
            }
        }
    }
        ...
        for (int i = 0; i < strings.length; i++) {
            for (int j = i; j < strings.length; j++) {
                assertThat(set.subSet(strings[i], false, strings[j], true))
                        .containsExactlyElementsIn(sortedNumberNames(i + 1, j + 1)).inOrder();
            }
        }
    }
        ...
        for (int i = 0; i < strings.length; i++) {
            for (int j = i; j < strings.length; j++) {
                assertThat(set.subSet(strings[i], true, strings[j], true))
                        .containsExactlyElementsIn(sortedNumberNames(i, j + 1)).inOrder();
            }
        }
    }
