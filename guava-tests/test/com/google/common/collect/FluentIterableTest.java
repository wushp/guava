...
public class FluentIterableTest extends TestCase {
    ...
    public void testFromArrayAndAppend() {
        FluentIterable<TimeUnit> units = FluentIterable.from(TimeUnit.values()).append(TimeUnit.SECONDS);
    }
     ...
     * <a href=
     * "https://docs.oracle.com/javase/specs/jls/se7/html/jls-4.html#jls-4.9">intersection</a> of
    ...
    public void testToString() {
        ...
        assertEquals("[yam, bam, jam, ham]", FluentIterable.from(asList("yam", "bam", "jam", "ham")).toString());
    }
    ...
    public void testAppend_toEmpty() {
        FluentIterable<Integer> result = FluentIterable.<Integer>of().append(Lists.newArrayList(1, 2, 3));
    ...
    public void testSkip_simpleList() {
        ...
        assertEquals(Lists.newArrayList("c", "d", "e"), Lists.newArrayList(FluentIterable.from(list).skip(2)));
    ...
    public void testSkip_skipNoneList() {
        Collection<String> list = Lists.newArrayList("a", "b");
        assertEquals(Lists.newArrayList("a", "b"), Lists.newArrayList(FluentIterable.from(list).skip(0)));
    }
    ...
    public void testLimit_illegalArgument() {
        try {
            FluentIterable<String> unused = FluentIterable.from(Lists.newArrayList("a", "b", "c")).limit(-1);
    ...
    public void testToSortedSet() {
        assertThat(fluent(1, 4, 2, 3).toSortedSet(Ordering.<Integer>natural().reverse())).containsExactly(4, 3, 2, 1)
                .inOrder();
    }

    public void testToSortedSet_removeDuplicates() {
        assertThat(fluent(1, 4, 1, 3).toSortedSet(Ordering.<Integer>natural().reverse())).containsExactly(4, 3, 1)
                .inOrder();
    }
    ...
    public void testToMap() {
        assertThat(fluent(1, 2, 3).toMap(Functions.toStringFunction()).entrySet())
                .containsExactly(Maps.immutableEntry(1, "1"), Maps.immutableEntry(2, "2"), Maps.immutableEntry(3, "3"))
                .inOrder();
    }
    ...
    public void testIndex() {
        ImmutableListMultimap<Integer, String> expected = ImmutableListMultimap.<Integer, String>builder()
                .putAll(3, "one", "two").put(5, "three").put(4, "four").build();
        ImmutableListMultimap<Integer, String> index =
                FluentIterable.from(asList("one", "two", "three", "four")).index(new Function<String, Integer>() {
                    @Override
    ...
    public void testIndex_nullKey() {
        try {
            ImmutableListMultimap<Object, Integer> unused = fluent(1, 2, 3).index(Functions.constant(null));
            fail();
        } catch (NullPointerException expected) {
        }
    }
    ...
    public void testIndex_nullValue() {
        try {
            ImmutableListMultimap<String, Integer> unused = fluent(1, null, 2).index(Functions.constant("foo"));
            fail();
        } catch (NullPointerException expected) {
        }
    }
    ...
    public void testUniqueIndex() {
        ...
        ImmutableMap<Integer, String> index =
                FluentIterable.from(asList("two", "three", "four")).uniqueIndex(new Function<String, Integer>() {
                    @Override
                    public Integer apply(String input) {
                        return input.length();
                    }
    ...
    public void testUniqueIndex_duplicateKey() {
        try {
            ImmutableMap<Integer, String> unused = FluentIterable.from(asList("one", "two", "three", "four"))
    ...
    public void testCopyInto_List() {
        assertThat(fluent(1, 3, 5).copyInto(Lists.newArrayList(1, 2))).containsExactly(1, 2, 1, 3, 5).inOrder();
    }
