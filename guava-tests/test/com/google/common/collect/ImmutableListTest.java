...
public class ImmutableListTest extends TestCase {
    ...
    public static Test suite() {
        ...
        suite.addTest(ListTestSuiteBuilder.using(new BuilderReversedListGenerator()).named("ImmutableList, reversed")
                .withFeatures(CollectionSize.ANY, SERIALIZABLE, ALLOWS_NULL_QUERIES).createTestSuite());
        suite.addTest(
                ListTestSuiteBuilder.using(new ImmutableListHeadSubListGenerator()).named("ImmutableList, head subList")
                        .withFeatures(CollectionSize.ANY, SERIALIZABLE, ALLOWS_NULL_QUERIES).createTestSuite());
        suite.addTest(
                ListTestSuiteBuilder.using(new ImmutableListTailSubListGenerator()).named("ImmutableList, tail subList")
                        .withFeatures(CollectionSize.ANY, SERIALIZABLE, ALLOWS_NULL_QUERIES).createTestSuite());
    ...
    public static class CreationTests extends TestCase {
        ...
        public void testCreation_fourteenElements() {
            ...
            assertEquals(Lists.newArrayList("a", "b", "c", "d", "e", "f", "g", "h", "i", "j", "k", "l", "m", "n"),
                    list);
        }
        ...
        public void testBuilderAddArrayHandlesNulls() {
            ...
            /*
             * Maybe it rejects all elements, or maybe it adds "a" before failing. Either way is
             * fine with us.
             */
            if (result.isEmpty()) {
                return;
    ...
    public static class ConcurrentTests extends TestCase {
        ...
        private static void runConcurrentlyMutatedTest(Collection<Integer> initialContents,
            ...
            Iterable<Integer> iterableToCopy =
                    wrap == WrapWithIterable.WRAP ? unmodifiableIterable(concurrentlyMutatedList)
        ...
        private static void runConcurrentlyMutatedTest(WrapWithIterable wrap) {
            /*
             * TODO: Iterate over many array sizes and all possible operation lists, performing adds
             * and removes in different ways.
             */
        ...
        interface ConcurrentlyMutatedList<E> extends List<E> {
            /**
             * The elements of a {@link ConcurrentlyMutatedList} are added and removed over time.
             * This method returns every state that the list has passed through at some point.
             */
        ...
        private static ConcurrentlyMutatedList<Integer> newConcurrentlyMutatedList(
                final Collection<Integer> initialContents, final Iterable<ListFrobber> actionsToPerformConcurrently) {
            ...
            ConcurrentlyMutatedList<Integer> list = (ConcurrentlyMutatedList<Integer>) newProxyInstance(
                    ImmutableListTest.CreationTests.class.getClassLoader(), new Class[] {ConcurrentlyMutatedList.class},
                    invocationHandler);
            return list;
        }
    }
