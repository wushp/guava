...
public class PeekingIteratorTest extends TestCase {
    ...
    /**
     * Version of {@link IteratorTester} that compares an iterator over a given collection of
     * elements (used as the reference iterator) against a {@code PeekingIterator} that *wraps* such
     * an iterator (used as the target iterator).
     *
     * <p>
     * This IteratorTester makes copies of the master so that it can later verify that
     * {@link PeekingIterator#remove()} removes the same elements as the reference's iterator
     * {@code #remove()}.
     */
    ...
    public void testPeekingIteratorDoesntAdvancePrematurely() throws Exception {
        /*
         * This test will catch problems where the underlying iterator throws a RuntimeException
         * when retrieving the nth element.
         *
         * If the PeekingIterator is caching elements too aggressively, it may throw the exception
         * on the (n-1)th element (oops!).
         */
