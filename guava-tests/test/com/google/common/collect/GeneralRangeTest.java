...
public class GeneralRangeTest extends TestCase {
    ...
    public void testLowerRange() {
        for (BoundType lBoundType : BoundType.values()) {
            ...
            for (Integer i : IN_ORDER_VALUES) {
                assertEquals(ORDERING.compare(i, 3) > 0 || (ORDERING.compare(i, 3) == 0 && lBoundType == CLOSED),
                        range.contains(i));
                assertEquals(ORDERING.compare(i, 3) < 0 || (ORDERING.compare(i, 3) == 0 && lBoundType == OPEN),
                        range.tooLow(i));
    ...
    public void testReverse() {
        ...
        assertEquals(GeneralRange.upTo(ORDERING.reverse(), 3, OPEN), GeneralRange.downTo(ORDERING, 3, OPEN).reverse());
