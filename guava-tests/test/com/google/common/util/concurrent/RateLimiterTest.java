...
public class RateLimiterTest extends TestCase {
    ...
    public void testOneSecondBurst() {
        ...
        assertEvents("U1.00", "U1.00", "R0.00", "R0.00", "R0.00", "R0.00", // first request and
                                                                           // burst
                "R0.20");
    }
    ...
    public void testWarmUp() {
        ...
        stopwatch.sleepMillis(2000); // #6: didn't get cold! It would take another 2 seconds to go
                                     // cold
        for (int i = 0; i < 8; i++) {
            limiter.acquire(); // #7
        }
        assertEvents("R0.00, R1.38, R1.13, R0.88, R0.63, R0.50, R0.50, R0.50", // #1
                "U0.50", // #2
                "U4.00", // #3
    ...
    public void testWarmUpWithColdFactor() {
        ...
        assertEvents("R0.00, R1.75, R1.26, R0.76, R0.30, R0.20, R0.20, R0.20", // #1
                "U0.20", // #2
                "U4.00", // #3
    ...
    public void testWarmUpWithColdFactor1() {
        ...
        assertEvents("R0.00, R0.20, R0.20, R0.20, R0.20, R0.20, R0.20, R0.20", // #1
                "U0.34", // #2
    ...
    public void testInfinity_Bursty() {
        ...
        limiter.acquire();
        limiter.acquire();
        assertEvents("R0.00", // First comes the saved-up burst, which defaults to a 1-second burst
                              // (2 requests).
        ...
        limiter.acquire();
        limiter.acquire();
        assertEvents("R0.50", "R0.00", "R0.00"); // we repay the last request (.5sec), then back to
                                                 // +oo
    }
    ...
    /**
     * Make sure that bursts can never go above 1-second-worth-of-work for the current rate, even
    ...
    /**
     * This neat test shows that no matter what weights we use in our requests, if we push X amount
     * of permits in a cool state, where X = rate * timeToCoolDown, and we have specified a
            ...
            for (double coldFactor : coldFactorsToTest) {
                for (double qps : qpsToTest) {
                    ...
                    RateLimiter rateLimiter =
                            RateLimiter.create(stopwatch, qps, warmupMillis, MILLISECONDS, coldFactor);
    ...
    public void testNulls() {
        NullPointerTester tester = new NullPointerTester().setDefault(SleepingStopwatch.class, stopwatch)
                .setDefault(int.class, 1).setDefault(double.class, 1.0d);
    ...
    public void testVerySmallDoubleValues() throws Exception {
        ...
        assertFalse("Should not acquire additional permit even after sleeping", rateLimiter.tryAcquire());
    }
     ...
     * testing with that version or newer, we'll need to suppress this test (or see if Mockito can
     * be changed to support this).
     */
    ...
    private static void doTestMocking(RateLimiter mock) throws Exception {
        for (Method method : RateLimiter.class.getMethods()) {
            if (!isStatic(method.getModifiers()) && !NOT_WORKING_ON_MOCKS.contains(method.getName())
    ...
    private static final ImmutableClassToInstanceMap<Object> PARAMETER_VALUES = ImmutableClassToInstanceMap.builder()
            .put(int.class, 1).put(long.class, 1L).put(double.class, 1.0).put(TimeUnit.class, SECONDS).build();
}
