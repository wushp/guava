 ...
 *
 * <p>
 * This test class generates all of its own test cases in the {@link #suite()} method. Every
...
public class GeneratedMonitorTest extends TestCase {
    ...
    /**
     * How long to wait when determining that a thread is blocked if we DON'T expect it to be
     * blocked.
     */
    ...
    private enum Scenario {
        ...
        SATISFIED_AND_UNOCCUPIED_BEFORE_ENTERING, UNSATISFIED_AND_UNOCCUPIED_BEFORE_ENTERING, SATISFIED_AND_OCCUPIED_BEFORE_ENTERING, SATISFIED_UNOCCUPIED_AND_INTERRUPTED_BEFORE_ENTERING,

        SATISFIED_BEFORE_WAITING, SATISFIED_WHILE_WAITING, SATISFIED_AND_INTERRUPTED_BEFORE_WAITING, UNSATISFIED_BEFORE_AND_WHILE_WAITING, UNSATISFIED_AND_INTERRUPTED_BEFORE_WAITING;
    ...
    private enum Timeout {
        ...
        MIN(Long.MIN_VALUE, "-oo"), MINUS_SMALL(-SMALL_TIMEOUT_MILLIS, "-" + SMALL_TIMEOUT_MILLIS + "ms"), ZERO(0L,
                "0ms"), SMALL(SMALL_TIMEOUT_MILLIS, SMALL_TIMEOUT_MILLIS + "ms"), LARGE(
                        UNEXPECTED_HANG_DELAY_MILLIS * 2,
                        (2 * UNEXPECTED_HANG_DELAY_MILLIS) + "ms"), MAX(Long.MAX_VALUE, "+oo");
    ...
    private enum TimeoutsToUse {
        ...
        ANY(Timeout.values()), PAST(Timeout.MIN, Timeout.MINUS_SMALL, Timeout.ZERO), FUTURE(Timeout.SMALL,
                Timeout.MAX), SMALL(Timeout.SMALL), FINITE(Timeout.MIN, Timeout.MINUS_SMALL, Timeout.ZERO,
                        Timeout.SMALL), INFINITE(Timeout.LARGE, Timeout.MAX);
    ...
    /**
     * Identifies just enterIfXxx methods (a subset of {@link #isAnyEnter}), which are mostly like
     * the enterXxx methods but behave like tryEnterXxx in some scenarios.
     */
    ...
    private static boolean isTimed(Method method) {
        Class<?>[] parameterTypes = method.getParameterTypes();
        return parameterTypes.length >= 2 && parameterTypes[parameterTypes.length - 2] == long.class
    ...
    private static void addTests(TestSuite suite, Method method) {
        ...
        if (isAnyEnter(method)) {
            addTests(suite, method, Scenario.SATISFIED_AND_UNOCCUPIED_BEFORE_ENTERING, TimeoutsToUse.ANY,
                    Outcome.SUCCESS);
            addTests(suite, method, Scenario.UNSATISFIED_AND_UNOCCUPIED_BEFORE_ENTERING, TimeoutsToUse.FINITE,
                    isGuarded(method) ? (isBoolean(method) ? Outcome.FAILURE : Outcome.HANG) : Outcome.SUCCESS);
            addTests(suite, method, Scenario.UNSATISFIED_AND_UNOCCUPIED_BEFORE_ENTERING, TimeoutsToUse.INFINITE,
                    isGuarded(method) ? (isTryEnter(method) || isEnterIf(method) ? Outcome.FAILURE : Outcome.HANG)
                            : Outcome.SUCCESS);
            addTests(suite, method, Scenario.SATISFIED_AND_OCCUPIED_BEFORE_ENTERING, TimeoutsToUse.FINITE,
                    isBoolean(method) ? Outcome.FAILURE : Outcome.HANG);
            addTests(suite, method, Scenario.SATISFIED_AND_OCCUPIED_BEFORE_ENTERING, TimeoutsToUse.INFINITE,
                    isGuarded(method) ? Outcome.HANG : (isTryEnter(method) ? Outcome.FAILURE : Outcome.HANG));
            addTests(suite, method, Scenario.SATISFIED_UNOCCUPIED_AND_INTERRUPTED_BEFORE_ENTERING, TimeoutsToUse.ANY,
            ...
            addTests(suite, method, Scenario.SATISFIED_AND_INTERRUPTED_BEFORE_WAITING, TimeoutsToUse.ANY,
                    Outcome.SUCCESS);
            addTests(suite, method, Scenario.UNSATISFIED_BEFORE_AND_WHILE_WAITING, TimeoutsToUse.FINITE,
                    Outcome.FAILURE);
            addTests(suite, method, Scenario.UNSATISFIED_BEFORE_AND_WHILE_WAITING, TimeoutsToUse.INFINITE,
                    Outcome.HANG);
            addTests(suite, method, Scenario.UNSATISFIED_AND_INTERRUPTED_BEFORE_WAITING, TimeoutsToUse.PAST,
                    // prefer responding to interrupt over timing out
                    isInterruptible(method) ? Outcome.INTERRUPT : Outcome.FAILURE);
            addTests(suite, method, Scenario.UNSATISFIED_AND_INTERRUPTED_BEFORE_WAITING, TimeoutsToUse.SMALL,
                    isInterruptible(method) ? Outcome.INTERRUPT : Outcome.FAILURE);
            addTests(suite, method, Scenario.UNSATISFIED_AND_INTERRUPTED_BEFORE_WAITING, TimeoutsToUse.INFINITE,
                    isInterruptible(method) ? Outcome.INTERRUPT : Outcome.HANG);
        }
    }

    /**
     * Generates test cases for the given combination of scenario and timeouts. For methods that
     * take an explicit timeout value, all of the given timeoutsToUse result in individual test
     * cases. For methods that do not take an explicit timeout value, a single test case is
     * generated only if the implicit timeout of that method matches the given timeoutsToUse. For
     * example, enter() is treated like enter(MAX, MILLIS) and tryEnter() is treated like enter(0,
     * MILLIS).
     */
    private static void addTests(TestSuite suite, Method method, Scenario scenario, TimeoutsToUse timeoutsToUse,
    ...
    private GeneratedMonitorTest(Method method, Scenario scenario, boolean fair, Timeout timeout,
            Outcome expectedOutcome) {
    ...
    private static String nameFor(Method method, Scenario scenario, boolean fair, Timeout timeout,
            Outcome expectedOutcome) {
        return String.format(Locale.ROOT, "%s%s(%s)/%s->%s", method.getName(), fair ? "(fair)" : "(nonfair)",
                (timeout == null) ? "untimed" : timeout, scenario, expectedOutcome);
    }
    ...
    protected void runTest() throws Throwable {
        ...
        long hangDelayMillis =
                (expectedOutcome == Outcome.HANG) ? EXPECTED_HANG_DELAY_MILLIS : UNEXPECTED_HANG_DELAY_MILLIS;
    ...
    protected void tearDown() throws Exception {
        ...
        //
        // Except for (c), every thread should occupy the monitor very briefly, and every thread
        // leaves
        // the monitor with the guard satisfied. Therefore as soon as tearDownLatch is triggered, we
        // should be able to enter the monitor, and then we set the guard to satisfied for the
        // benefit
     ...
     */
    private static TestCase generateGuardWithWrongMonitorTestCase(final Method method, final boolean fair1,
            final boolean fair2) {
        ...
        return new TestCase(method.getName() + (timed ? "(0ms)" : "()") + "/WrongMonitor->IMSE") {
            ...
            protected void runTest() throws Throwable {
                ...
                if (occupyMonitor) {
                    // If we don't already occupy the monitor, we'll get an IMSE regardless of the
                    // guard (see
    ...
    private static TestCase generateWaitForWhenNotOccupyingTestCase(final Method method, final boolean fair) {
        final boolean timed = isTimed(method); // Not going to bother with all timeouts, just 0ms.
        String testName =
                method.getName() + (fair ? "(fair)" : "(nonfair)") + (timed ? "(0ms)" : "()") + "/NotOccupying->IMSE";
