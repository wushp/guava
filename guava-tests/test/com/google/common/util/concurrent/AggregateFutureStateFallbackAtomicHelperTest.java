 ...
 *
 * <p>
 * On different platforms AggregateFutureState uses different strategies for its core
...
public class AggregateFutureStateFallbackAtomicHelperTest extends TestCase {
    ...
    /**
     * This classloader blacklists AtomicReferenceFieldUpdater and AtomicIntegerFieldUpdate which
     * will prevent us from selecting our {@code SafeAtomicHelper} strategy.
     *
     * Stashing this in a static field avoids loading it over and over again and speeds up test
     * execution significantly.
     */
    private static final ClassLoader NO_ATOMIC_FIELD_UPDATER = getClassLoader(
            ImmutableSet.of(AtomicIntegerFieldUpdater.class.getName(), AtomicReferenceFieldUpdater.class.getName()));
    ...
    public static TestSuite suite() {
        ...
        for (Method method : FuturesTest.class.getDeclaredMethods()) {
            if (Modifier.isPublic(method.getModifiers()) && method.getName().startsWith("test")) {
                suite.addTest(
                        TestSuite.createTest(AggregateFutureStateFallbackAtomicHelperTest.class, method.getName()));
            }
        }
        return suite;
    }
