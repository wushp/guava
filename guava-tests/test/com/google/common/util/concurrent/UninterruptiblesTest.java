...
public class UninterruptiblesTest extends TestCase {
    ...
    protected void setUp() {
        ...
        if (Thread.currentThread().isInterrupted()) {
            throw new AssertionError(
                    "Thread interrupted on test entry. " + "Some test probably didn't clear the interrupt state");
        }
    ...
    public void testNull() throws Exception {
        new NullPointerTester().setDefault(CountDownLatch.class, new CountDownLatch(0))
                .setDefault(Semaphore.class, new Semaphore(999)).testAllPublicStaticMethods(Uninterruptibles.class);
    }
    ...
    public void testJoinTimeoutMultiInterruptExpired() {
        /*
         * We don't "need" to schedule a thread completion at all here, but by doing so, we come the
         * closest we can to testing that the wait time is appropriately decreased on each
         * progressive join() call.
         */
        TimedThread thread = TimedThread.createWithDelay(LONG_DELAY_MS);
        repeatedlyInterruptTestThread(20, tearDownStack);
    ...
    public void testTryAcquireTimeoutMultiInterruptExpired() {
        /*
         * We don't "need" to schedule a release() call at all here, but by doing so, we come the
         * closest we can to testing that the wait time is appropriately decreased on each
         * progressive tryAcquire() call.
         */
        TimedSemaphore semaphore = TimedSemaphore.createWithDelay(LONG_DELAY_MS);
        repeatedlyInterruptTestThread(20, tearDownStack);
     ...
     * a {@code Completion} starts the underlying stopwatch.
     */
    private static final class Completion {
        ...
        /**
         * Asserts that at least {@code timeout} has passed but the expected completion time has
         * not.
         */
        ...
        private static void assertAtLeastTimePassed(Stopwatch stopwatch, long expectedMillis) {
            ...
            /*
             * The "+ 5" below is to permit, say, sleep(10) to sleep only 9 milliseconds. We see
             * such behavior sometimes when running these tests publicly as part of Guava. "+ 5" is
             * probably more generous than it needs to be.
             */
            assertTrue("Expected elapsed millis to be >= " + expectedMillis + " but was " + elapsedMillis,
                    elapsedMillis + 5 >= expectedMillis);
        }
    }

    // TODO(cpovirk): Split this into separate CountDownLatch and IncrementableCountDownLatch
    // classes.
    ...
    private static final class TimedPutQueue {
        ...
        /**
         * Creates a {@link EnableWrites} which open up a spot for a {@code put} to succeed in
    ...
    private static final class TimedTakeQueue {
        ...
        /**
         * Creates a {@link EnableReads} which insert an element for a {@code take} to receive in
         * {@code countdownInMillis}.
         */
    ...
    private static final class TimedSemaphore {
        ...
        /**
         * Requests a permit from the semaphore with a timeout and asserts that operation completed
     ...
     * {@code assertTrue(Thread.interrupted())} except that this version tolerates late interrupts.
     */
    private static void assertInterrupted() {
        try {
            /*
             * The sleep() will end immediately if we've already been interrupted or wait patiently
