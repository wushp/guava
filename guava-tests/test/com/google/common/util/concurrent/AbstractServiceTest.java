...
public class AbstractServiceTest extends TestCase {
     ...
     * {@link State#STARTING} more than once, the {@link Listener#stopping(State)} callback would
    ...
    public void testManualServiceFailureIdempotence() {
        ManualSwitchedService service = new ManualSwitchedService();
        /*
         * Set up a RecordingListener to perform its built-in assertions, even though we won't look
         * at its state history.
         */
    ...
    private class ThreadedService extends AbstractService {
        ...
        /*
         * The main test thread tries to stop() the service shortly after confirming that it is
         * running. Meanwhile, the service itself is trying to confirm that it is running. If the
         * main thread's stop() call happens before it has the chance, the test will fail. To avoid
         * this, the main thread calls this method, which waits until the service has performed its
         * own "running" check.
         */
        ...
        public synchronized void terminated(State from) {
            ...
            if (from == State.NEW) {
                try {
                    ...
                    assertNull(expected.getCause());
                    assertTrue(expected.getMessage()
                            .equals("Expected the service " + service + " to be RUNNING, but was TERMINATED"));
                }
            }
