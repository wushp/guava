...
public class JdkFutureAdaptersTest extends TestCase {
    ...
    public void testListenInPoolThreadCustomExecutorInterrupted() throws Exception {
        ...
        listenableFuture.addListener(singleCallListener, directExecutor());
        /*
         * Don't shut down until the listenInPoolThread task has been accepted to run. We want to
         * see what happens when it's interrupted, not when it's rejected.
         */
    ...
    private static final class RuntimeExceptionThrowingFuture<V> implements Future<V> {
        ...
        public boolean isDone() {
            /*
             * If isDone is true during the call to listenInPoolThread, listenInPoolThread doesn't
             * start a thread. Make sure it's false the first time through (and forever after, since
    ...
    public void testListenInPoolThreadRunsListenerAfterRuntimeException() throws Exception {
        ...
        /*
         * The compiler recognizes that "input instanceof ListenableFuture" is impossible. We want
         * the test, though, in case that changes in the future, so we use isInstance instead.
         */
        assertFalse("Can't test the main listenInPoolThread path " + "if the input is already a ListenableFuture",
                ListenableFuture.class.isInstance(input));
        ListenableFuture<String> listenable = listenInPoolThread(input);
        /*
         * This will occur before the waiting get() in the listenInPoolThread-spawned thread
         * completes:
         */
