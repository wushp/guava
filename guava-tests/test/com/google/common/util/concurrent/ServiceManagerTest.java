...
public class ServiceManagerTest extends TestCase {
    ...
    public void testServiceStartupTimes_selfStartingServices() {
        ...
        // 2. service times are recorded when the service is not started by the manager (but they
        // may
        // not be accurate).
        ...
        // TODO(kak): Use assertThat(startupTimes.get(a)).isAtLeast(150);
        assertTrue(startupTimes.get(a) >= 150);
        // Service b startup takes at least 353 millis, but starting the timer is delayed by at
        // least
        // 150 milliseconds. so in a perfect world the timing would be 353-150=203ms, but since
        // either
     ...
     * service fails and something failed during startup before service.start was called on all the
     * services, then awaitStopped would deadlock due to an IllegalStateException that was thrown
     * when trying to stop the timer(!).
     */
    ...
    public void testEmptyServiceManager() {
        ...
        assertTrue(listener.stoppedCalled);
        assertTrue(listener.failedServices.isEmpty());
        // check that our NoOpService is not directly observable via any of the inspection methods
        // or
        // via logging.
    ...
    public void testListenerDeadlock() throws InterruptedException {
        ...
        Service failRunService = new AbstractService() {
            ...
            protected void doStart() {
                new Thread() {
                    ...
                    public void run() {
                        notifyStarted();
                        // We need to wait for the main thread to leave the
                        // ServiceManager.startAsync call to
                        // ensure that the thread running the failure callbacks is not the main
                        // thread.
        ...
        // But
        // we really just want to wait for the thread to be in the failure callback so we wait for
        // that
        // explicitly instead.
        failEnter.await();
    ...
    public void testPartiallyConstructedManager_transitionAfterAddListenerBeforeStateIsReady() {
        // The implementation of this test is pretty sensitive to the implementation :( but we want
        // to
        // ensure that if weird things happen during construction then we get exceptions.
        final NoOpService service1 = new NoOpService();
        // This service will start service1 when addListener is called. This simulates service1
        // being
    ...
    /**
     * This service will shutdown very quickly after stopAsync is called and uses a background
     * thread so that we know that the stopping() listeners will execute on a different thread than
     * the terminated() listeners.
     */
    ...
    public void testNulls() {
        ServiceManager manager = new ServiceManager(Arrays.<Service>asList());
        new NullPointerTester().setDefault(ServiceManager.Listener.class, new RecordingListener())
