...
public class UninterruptibleFutureTest extends TestCase {
    ...
    public void testRegularFutureInterrupted() throws ExecutionException {
         ...
         *
         * 1. The client thread begins to block on a get() call to a future. 2. The client thread is
         * interrupted sometime before the result would be available. 3. We expect the client's
         * get() to throw an InterruptedException. 4. We expect the client thread's interrupt state
         * to be false. 5. The client thread again makes a blocking call to get(). 6. Now the result
         * becomes available. 7. We expect get() to return this result. 8. We expect the test
         * thread's interrupt state to be false.
         */
    ...
    private static void runNInterruptsTest(int times, SettableFuture<String> future,
            FutureTask<Boolean> interruptReporter) throws InterruptedException, ExecutionException, TimeoutException {
    ...
    public void testMakeUninterruptible_timedGetZeroTimeoutAttempted() throws TimeoutException, ExecutionException {
        ...
        future.set(RESULT);
        /*
         * getUninterruptibly should call the timed get method once with a wait of 0 seconds (and it
         * should succeed, since the result is already available).
         */
    ...
    public void testMakeUninterruptible_timedGetNegativeTimeoutAttempted() throws TimeoutException, ExecutionException {
        ...
        future.set(RESULT);
        /*
         * The getUninterruptibly should call the timed get method once with a wait of -1 seconds
         * (and it should succeed, since the result is already available).
         */
        assertEquals(RESULT, getUninterruptibly(future, -1, SECONDS));
    }

    private static FutureTask<Boolean> untimedInterruptReporter(final Future<?> future,
            final boolean allowInterruption) {
