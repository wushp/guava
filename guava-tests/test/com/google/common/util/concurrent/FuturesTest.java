...
public class FuturesTest extends TestCase {
    ...
    public void tearDown() throws Exception {
         ...
         *
         * (Ideally we would perform interrupts only in threads that we create, but it's hard to
         * imagine that anything will break in practice.)
         */
        clearInterrupt();
    ...
    /*
     * TODO(cpovirk): Use FutureSubject once it's part of core Truth. But be wary of using it when
     * I'm really testing a Future implementation (e.g., in the case of immediate*Future()). But
     * it's OK to use in the case of the majority of Futures that are AbstractFutures.
     */
    ...
    public void testImmediateCancelledFutureStack() throws Exception {
        ...
        try {
            ...
            // There should be two CancellationException chained together. The outer one should have
            // the
            // stack trace of where the get() call was made, and the inner should have the stack
            // trace of
    ...
    public void testTransform_StackOverflow() throws Exception {
        {
            /*
             * Initialize all relevant classes before running the test, which may otherwise poison
             * any classes it is trying to load during its stack overflow.
             */
    ...
    public void testTransformAsync_inputCancelButNotInterruptPropagatesToOutput() throws Exception {
        ...
        /*
         * We might like to propagate interruption, too, but it's not clear that it matters. For
         * now, we test for the behavior that we have today.
         */
    ...
    public void testTransformExceptionRemainsMemoized() throws Throwable {
        ...
        /*
         * Try again when the input's value is already filled in, since the flow is slightly
    ...
    private void runExpectedExceptionCatchingAsyncTest(final Exception expectedException, final boolean wrapInFuture)
            throws Exception {
        AsyncFunctionSpy<Throwable, Integer> fallback = spy(new AsyncFunction<Throwable, Integer>() {
            @Override
            public ListenableFuture<Integer> apply(Throwable t) throws Exception {
    ...
    public void testCatchingAsync_fallbackNotReady() throws Exception {
        ...
        ListenableFuture<Integer> derived = catchingAsync(primary, Throwable.class, fallback, directExecutor());
        secondary.set(1);
    ...
    public void testCatchingAsync_resultCancelledAfterFallback() throws Exception {
        ...
        ListenableFuture<Integer> derived = catchingAsync(failingFuture, Throwable.class, fallback, directExecutor());
        derived.cancel(false);
        assertTrue(secondary.isCancelled());
        assertFalse(secondary.wasInterrupted());
        fallback.verifyCallCount(1);
    ...
    public void testCatchingAsync_nullInsteadOfFuture() throws Exception {
        ...
        ListenableFuture<?> chainedFuture =
                catchingAsync(inputFuture, Throwable.class, new AsyncFunction<Throwable, Integer>() {
                    @Override
    ...
    /*
     * catching() uses a plain Function, so there's no
     * testCatching_fallbackReturnsRuntimeException() or
     * testCatching_fallbackReturnsCheckedException().
     */
    ...
    public void testCatching_resultInterruptedBeforeFallback() throws Exception {
        SettableFuture<Integer> primary = SettableFuture.create();
        Function<Throwable, Integer> fallback = unexpectedFunction();
        ListenableFuture<Integer> derived = catching(primary, Throwable.class, fallback, directExecutor());
        derived.cancel(true);
        assertTrue(primary.isCancelled());
        assertTrue(primary.wasInterrupted());
    }
    ...
    public void testCatching_StackOverflow() throws Exception {
        {
             ...
             */
            SettableFuture<Object> root = SettableFuture.create();
            ListenableFuture<Object> unused = catching(root, MyException.class, identity(), directExecutor());
    ...
    public void testCatching_getThrowsRuntimeException() throws Exception {
        ...
        ListenableFuture<Object> output = catching(input, Throwable.class, identity(), directExecutor());
    ...
    public void testCatchingAsync_StackOverflow() throws Exception {
        {
             ...
             */
            SettableFuture<Object> root = SettableFuture.create();
            ListenableFuture<Object> unused = catchingAsync(root, MyException.class, asyncIdentity(), directExecutor());
            root.setException(new MyException());
        }
    ...
    public void testCatchingAsync_getThrowsRuntimeException() throws Exception {
        ...
        // We'd catch only MyRuntimeException.class here, but then the test won't compile under GWT.
        ListenableFuture<Object> output = catchingAsync(input, Throwable.class, asyncIdentity(), directExecutor());
        assertThat(getDone(output)).isInstanceOf(MyRuntimeException.class);
    }
    ...
    public void testCatchingAsync_rejectionPropagatesToOutput() throws Exception {
        SettableFuture<String> input = SettableFuture.create();
        ListenableFuture<String> transformed = catchingAsync(input, Throwable.class,
                constantAsyncFunction(immediateFuture("foo")), REJECTING_EXECUTOR);
        input.setException(new Exception());
        try {
            getDone(transformed);
            fail();
    ...
    public void testTransformAsync_asyncFunction_timeout() throws InterruptedException, ExecutionException {
        ...
        ListenableFuture<Integer> future = transformAsync(SettableFuture.<String>create(), function, directExecutor());
        try {
    ...
    public void testTransformAsync_asyncFunction_error() throws InterruptedException {
        ...
        ListenableFuture<Integer> outputFuture = transformAsync(inputFuture, function, directExecutor());
    ...
    public void testTransformAsync_asyncFunction_nullInsteadOfFuture() throws Exception {
        ...
        ListenableFuture<?> chainedFuture = transformAsync(inputFuture, constantAsyncFunction(null), directExecutor());
        try {
            getDone(chainedFuture);
            fail();
        } catch (ExecutionException expected) {
    ...
    public void testAllAsList_logging_seenExceptionUpdateRace() throws Exception {
        ...
        bulkFuture.addListener(new Runnable() {
            ...
            public void run() {
                 ...
                 * exception in seenExceptions. When we complete secondFuture with the same
                 * exception, we want for AggregateFuture to still detect that it's been previously
                 * seen.
                 */
    ...
    public void testAllAsList_logging_seenExceptionUpdateCancelRace() throws Exception {
        ...
        bulkFuture.addListener(new Runnable() {
            ...
            public void run() {
                /*
                 * This is similar to the above test, but this time we're making sure that we
                 * recognize that the output Future is done early not because of an exception but
    ...
    /*
     * TODO(cpovirk): maybe pass around TestFuture instances instead of ListenableFuture instances
     */

    /**
     * A future in {@link TestFutureBatch} that also has a name for debugging purposes and a {@code
     * finisher}, a task that will complete the future in some fashion when it is called, allowing
     * for testing both before and after the completion of the future.
     */
    @GwtIncompatible // used only in GwtIncompatible tests
    ...
    /**
     * A collection of several futures, covering cancellation, success, and failure (both
     * {@link ExecutionException} and {@link RuntimeException}), both immediate and delayed. We use
     * each possible pair of these futures in {@link FuturesTest#runExtensiveMergerTest}.
     *
     * <p>
     * Each test requires a new {@link TestFutureBatch} because we need new delayed futures each
    ...
    private static final class TestFutureBatch {
        ...
        final ListenableFuture<String> doneRuntimeException = new ForwardingListenableFuture<String>() {
        ...
        final ListenableFuture<String> delayedRuntimeException = new ForwardingListenableFuture<String>() {
            @Override
            protected ListenableFuture<String> delegate() {
        ...
        final ImmutableList<TestFuture> allFutures =
                ImmutableList.of(new TestFuture(doneSuccess, "doneSuccess", doNothing),
                        ...
                        new TestFuture(delayedRuntimeException, "delayedRuntimeException", finishRuntimeException));

        final Function<ListenableFuture<String>, String> nameGetter = new Function<ListenableFuture<String>, String>() {
            @Override
        ...
        boolean hasDelayed(ListenableFuture<String> a, ListenableFuture<String> b) {
            ...
            return intersect(inputs,
                    ImmutableSet.of(delayedSuccess, delayedFailed, delayedCancelled, delayedRuntimeException));
        }
        ...
        void assertHasFailure(ListenableFuture<String> a, ListenableFuture<String> b, Exception e) {
            ImmutableSet<ListenableFuture<String>> inputs = ImmutableSet.of(a, b);
            smartAssertTrue(inputs, e, intersect(inputs,
                    ImmutableSet.of(doneFailed, doneRuntimeException, delayedFailed, delayedRuntimeException)));
        }

        void assertHasCancel(ListenableFuture<String> a, ListenableFuture<String> b, Exception e) {
            ImmutableSet<ListenableFuture<String>> inputs = ImmutableSet.of(a, b);
            smartAssertTrue(inputs, e, intersect(inputs, ImmutableSet.of(doneCancelled, delayedCancelled)));
        }

        void assertHasImmediateFailure(ListenableFuture<String> a, ListenableFuture<String> b, Exception e) {
            ImmutableSet<ListenableFuture<String>> inputs = ImmutableSet.of(a, b);
            smartAssertTrue(inputs, e, intersect(inputs, ImmutableSet.of(doneFailed, doneRuntimeException)));
        }
    ...
    /**
     * {@link allAsList(Iterable)} or {@link successfulAsList(Iterable)}, hidden behind a common
     ...
     *
     * <p>
     * We need this to test the behavior of no-arg get methods without hanging the main test thread
        ...
        for (int i = 0; i < inputCount; i++) {
            for (int j = 0; j < inputCount; j++) {
                for (boolean iBeforeJ : new boolean[] {true, false}) {
                    ...
                    try {
                        List<String> result = conditionalPseudoTimedGetUninterruptibly(inputs, iFuture, jFuture, future,
                                20, MILLISECONDS);
                        assertTrue("Got " + result, asList("a", null).containsAll(result));
                    } catch (CancellationException e) {
                        assertTrue(merger == Merger.allMerger);
     ...
     * then we call it in a separate thread (using pseudoTimedGet). The result is that we wait as
     * long as necessary when the method is expected to return (at the cost of hanging forever if
     * there is a bug in the class under test) but that we time out fairly promptly when the method
     * is expected to hang (possibly too quickly, but too-quick failures should be very unlikely,
     * given that we used to bail after 20ms during the expected-successful tests, and there we saw
     * a failure rate of ~1/5000, meaning that the other thread's get() call nearly always completes
     * within 20ms if it's going to complete at all).
     *
     ...
     */
    @GwtIncompatible // used only in GwtIncompatible tests
    private static List<String> conditionalPseudoTimedGetUninterruptibly(TestFutureBatch inputs,
            ListenableFuture<String> iFuture, ListenableFuture<String> jFuture, ListenableFuture<List<String>> future,
            int timeout, TimeUnit unit) throws ExecutionException, TimeoutException {
        /*
         * For faster tests (that may hang indefinitely if the class under test has a bug!), switch
         * the second branch to call untimed future.get() instead of pseudoTimedGet.
         */
        return (inputs.hasDelayed(iFuture, jFuture)) ? pseudoTimedGetUninterruptibly(future, timeout, unit)
    ...
    private static void doTestSuccessfulAsList_resultCancelledRacingInputDone() throws Exception {
        ...
        /*
         * Complex (combined.cancel -> input.cancel -> other.set -> setOneValue), to show that this
         * isn't just about problems with the input future we just cancelled:
         */
        ...
        future1.addListener(new Runnable() {
            ...
            public void run() {
                assertTrue(future1.isCancelled());
                /*
                 * This test relies on behavior that's unspecified but currently guaranteed by the
                 * implementation: Cancellation of inputs is performed in the order they were
                 * provided to the constructor. Verify that as a sanity check:
                 */
    ...
    public void testSuccessfulAsList_logging_exception() throws Exception {
        ...
        assertEquals(newArrayList(null, null, null), getDone(successfulAsList(immediateFailedFuture(new MyException()),
