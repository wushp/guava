 ...
 *
 * <p>
 * This class supports an optional fairness policy for ordering waiting producer and consumer
...
public class MonitorBasedArrayBlockingQueue<E> extends AbstractQueue<E> implements BlockingQueue<E> {
    ...
    /**
     * Creates an <tt>MonitorBasedArrayBlockingQueue</tt> with the given (fixed) capacity and
     ...
     * specified access policy and initially containing the elements of the given collection, added
     ...
     * immediately without exceeding the queue's capacity, returning <tt>true</tt> upon success and
     * throwing an <tt>IllegalStateException</tt> if this queue is full.
     *
     ...
     * Inserts the specified element at the tail of this queue if it is possible to do so
     * immediately without exceeding the queue's capacity, returning <tt>true</tt> upon success and
     * <tt>false</tt> if this queue is full. This method is generally preferable to method
    ...
    /**
     * Inserts the specified element at the tail of this queue, waiting up to the specified wait
     * time for space to become available if the queue is full.
     *
     * @throws InterruptedException {@inheritDoc}
     * @throws NullPointerException {@inheritDoc}
     */
     ...
     * capacity of this queue less the current <tt>size</tt> of this queue.
     *
     * <p>
     ...
     * formally, removes an element <tt>e</tt> such that <tt>o.equals(e)</tt>, if this queue
     * contains one or more such elements. Returns <tt>true</tt> if this queue contained the
    ...
    private class Itr implements Iterator<E> {
         ...
         * hasNext(), we must return it in the following next() call even if it was in the process
         * of being removed when hasNext() was called.
         */
        private E nextItem;

        /**
         * Index of element returned by most recent call to next. Reset to -1 if this element is
        ...
        public boolean hasNext() {
            /*
             * No sync. We can return true by mistake here only if this iterator passed across
             * threads, which we don't support anyway.
             */
            return nextIndex >= 0;
        }

        /**
         * Checks whether nextIndex is valid; if so setting nextItem. Stops iterator when either
         * hits putIndex or sees null item.
         */
